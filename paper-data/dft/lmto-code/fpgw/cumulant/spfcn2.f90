...
PROGRAM spfcn2
  ...
  REAL(8) xk, xklast, tmp, tmp2, Ek, gamk, kFermi, norml, normg, norm, normqp, Mu, wp,slope,intercept, QPCorr, gamk_user, &
       & EFermi, deltagF, deltalF, domega, deltat, gam, gam0,gamGW, tmax, ETot, ETot2, ETotAH, kocc, EInt(2), EIntAH(2), &
       & EIntKin(2), E0, EKin, EKinGW, Eexch, Rs, &
       & omegaMax, omegaMin, ESpec, totocc, occint(2), ExInt(2), Ex, tol, ETotRPA, Rea, Ima, ETotGW, EIntGW(2), ntot, ntotGW, &
       & ImSigF, betap, betapp, ntot0, E0Test, MuGW, &
       & alpha, HFMult, Sigc, wmin, wmax, w0, dw0, dxHigh, dxLow
  REAL(8),ALLOCATABLE :: t(:), betal(:), betag(:), betag2(:), omega(:), omega0(:), ReSig(:), ImSig(:), ReSigInt(:), &
       & ImSigInt(:), omegaOut(:), ERPA(:), kocc0(:), E0T(:), &
       &  dos(:), dosOcc(:), dosGW(:), ampband(:), gamband(:), scale2(:), wtk(:)
  COMPLEX*16,ALLOCATABLE :: Cl(:), Cg(:), Cg2(:), CInt(:), CInt2(:),Gg(:), Gl(:), Gtg(:), Gtl(:), qpl(:), G_GW(:), G_SB(:), &
    & G0(:), G0t(:), EkSCF(:), deltal(:), deltag(:), deltaTot(:), koccArr(:), koccGW(:)
  INTEGER ik, nk, iomega,  nt, it, iSCF, nscf, kMax, iFermi, iFermiGW, itmp, ik2, ik3, nomega, nomega_fine, nomega0, iprint, &
    ibroad, ioerr, nLow, nHigh, nband, iband
  ...
  LOGICAL CTilde, GRet, Formal_a, Formal_del, MixedForm, PrintPercent, ImAxis, UserMu, G0Test, CalcSpfcn, AddHF, &
  SelfConsistentZ, CalcExtInf, &
       & GWCorr
  CALL par_begin
  ...
  allocate(cl(2*nt),cg(2*nt), cg2(2*nt),cint(max(nomegamax,2*nt)),cint2(max(nomegamax,2*nt)),t(2*nt),gg(nomegamax), &
  qpl(nomegamax), g_gw(nomegamax), g_sb(nomegamax),scale2(nomegamax),wtk(nk))
  allocate(gl(nomegamax),gtg(2*nt),gtl(2*nt),g0(nomegamax),g0t(2*nt),ekscf(nk),deltal(nk),deltag(nk),deltatot(nk),erpa(nk), &
  e0t(nk),koccarr(nk),koccgw(nk),kocc0(nk))
  allocate(omega(nomegamax), omega0(nomegamax), omegaout(3*(nomegamax)), betal(nomegamax), betag(nomegamax), &
  betag2(nomegamax),resig(nomegamax),imsig(nomegamax),resigint(nomegamax),imsigint(nomegamax),dos(nomegamax), &
  dosocc(nomegamax),dosgw(nomegamax))
  ...
  open(unit=21,file='sigc.dat',status='replace')
  ...
  do ik = 1, nk
     ...
     ! get sigc
     !call terp(omega,resig,nomega,3,efermi-dble(ek),sigc)
     ...
     !   Sigc = 0.d0
     !END IF
     WRITE(21,*) ik, DBLE(deltaTot(ik))*hart, DBLE(deltal(ik))*hart
     IF(.NOT.GWCorr) THEN
        IF(GRet) THEN
           deltaTot(ik) = 0.d0
        ELSE
...
!     deltaTot(ik) = deltaTot(ik) + coni*tmp*pi
     ...
     al = 0.d0
     DO iomega = 2, nomega
        IF((omega(iomega)+DBLE(EKSCF(ik))).GT.EFermi) ag = ag + 0.5d0*(ABS(ImSig(iomega))/omega(iomega)**2 &
            + ABS(ImSig(iomega-1))/(omega(iomega-1)**2))*domega/pi
        IF((omega(iomega)+DBLE(EKSCF(ik))).LT.EFermi) al = al + 0.5d0*(ABS(ImSig(iomega))/omega(iomega)**2 &
            + ABS(ImSig(iomega-1))/(omega(iomega-1)**2))*domega/pi
     END DO
     !IF(.NOT.GRet) THEN
     ...
     DO it = 1, 2*nt
        ...
        DO iomega = 1, nomega
           ...
           ELSE
              ...
              ELSEIF(Formal_del) THEN
                 CInt(iomega) = betag(iomega) * (EXP(-coni*omega(iomega) * t(it)) &
                    + coni*omega(iomega)*t(it) - 1.d0)/(omega(iomega)-coni*tol)**2
              ELSE
                 CInt(iomega) = betag(iomega) * (EXP(-coni*omega(iomega) * t(it)) &
                    + coni*omega(iomega)*t(it) - 1.d0)/(omega(iomega)-coni*tol)**2
              END IF
              CInt2(iomega) = betag2(iomega) * (EXP(-coni*omega(iomega) * t(it)) &
                + coni*omega(iomega)*t(it) - 1.d0)/(omega(iomega)-coni*tol)**2
           END IF
        END DO
        IF(Formal_a.AND.Formal_del) THEN
           Cg(it) = CIntegrate(CInt,nomega,domega) - atot
        ELSEIF(Formal_del) THEN
           Cg(it) = CIntegrate(CInt,nomega,domega)
        ELSE
           Cg(it) = CIntegrate(CInt,nomega,domega)
        END IF
        Cg2(it) = CIntegrate(CInt2,nomega,domega)
     END DO

     ! Create omega0 grid.
     wmin = omega(1) + Ek
     wmax = omega(nomega) + Ek
     w0 = DBLE(Ek)
     dw0=gam0/10.d0
     nLow=INT(nomega*ABS(wmin-DBLE(Ek))/(wmax-wmin))
     nHigh=INT(nomega*ABS(wmax-DBLE(Ek))/(wmax-wmin))
     ...
     DO iomega = 1, nomega
        IF(master) THEN
           iprint = iprint + 1
           IF(MOD(iomega*20,nomega).EQ.0) THEN
              WRITE(*,'(i3,1a)') iomega*100/nomega,'%'
           END IF
           IF(iprint*10.GT.nomega) THEN
        ...
        DO it = 1, 2*nt
           ...
           IF(Formal_del) THEN
              ...
              !Ek=0.d0
              !gamk=0.d0
              Gtg(it) = -coni*EXP(coni*(omega0(iomega) - Ek - DBLE(deltaTot(ik)) - QPCorr)*t(it) - Abs(gamk*t(it)) &
                - (gam0*Abs(t(it)))**ibroad + Cg(it))
              Gtl(it) = -coni*EXP(coni*(omega0(iomega) - Ek)*t(it) - Abs(gamk*t(it)) - (gam0*Abs(t(it)))**ibroad + Cg2(it))
           ELSE
              Gtg(it) = -coni*EXP(coni*(omega0(iomega) - Ek - DBLE(deltaTot(ik)) - QPCorr)*t(it) &
                - (gam0*Abs(t(it)))**ibroad + Cg(it))
              ! DEBUG below
           ...
           CInt(it)  = -coni*EXP(coni*(omega0(iomega) - Ek)*t(it) - ABS(DIMAG(deltaTot(ik)))*Abs(t(it)) - (gam0*ABS(t(it)))**ibroad)
           ! Debug below
...
15   CONTINUE
     ...
     IF((.NOT.master).AND.CalcSpfcn) THEN
        ...
        CALL par_send_dbl(omega0,nomega,0,this_process)
        CALL par_send_dbl(xk,1,0,this_process)
        CALL par_send_dc(EkSCF(ik),1,0,this_process)
        CALL par_send_dbl(wtk(ik),1,0,this_process)
        CALL par_send_dc(Gg,nomega,0,this_process)
        CALL par_send_dc(Gl,nomega,0,this_process)
        CALL par_send_dc(G0,nomega,0,this_process)
        CALL par_send_dc(G_GW,nomega,0,this_process)
        CALL par_send_dc(Zk,1,0,this_process)
        CALL par_send_dc(ZkGW,1,0,this_process)
        CALL par_send_dc(ZkSC,1,0,this_process)
        CALL par_send_dc(qpl,nomega,0,this_process)
        ...
        DO ik2 = ik, MIN(ik + numprocs - 1, nk)
           ...
           IF((ik2.NE.ik).AND.CalcSpfcn) THEN
              ...
              IF(ik2.NE.ik3) THEN
                 ...
                 CALL par_stop
              END IF
              CALL par_recv_dbl(omega0,nomega,ik2-ik,ik2-ik)
              CALL par_recv_dbl(xk,1,ik2-ik,ik2-ik)
              CALL par_recv_dc(EkSCF(ik2),1,ik2-ik,ik2-ik)
              CALL par_recv_dbl(wtk(ik2),1,ik2-ik,ik2-ik)
              CALL par_recv_dc(Gg,nomega,ik2-ik,ik2-ik)
              CALL par_recv_dc(Gl,nomega,ik2-ik,ik2-ik)
              CALL par_recv_dc(G0,nomega,ik2-ik,ik2-ik)
              CALL par_recv_dc(G_GW,nomega,ik2-ik,ik2-ik)
              CALL par_recv_dc(Zk,1,ik2-ik,ik2-ik)
              CALL par_recv_dc(ZkGW,1,ik2-ik,ik2-ik)
              CALL par_recv_dc(ZkSC,1,ik2-ik,ik2-ik)
              CALL par_recv_dc(qpl,nomega,ik2-ik,ik2-ik)
           ...
           DO iomega = 2, nomega
              ...
              IF(omega0(iomega).GT.omegaMin) THEN
                 ...
                 IF(omega0(iomega).LT.MuGW) THEN
                    EIntGW(1) = EIntGW(1) + 0.5d0*DIMAG(G_GW(iomega-1)*(omega0(iomega-1)-EFermi+MuGW) &
                        + G_GW(iomega)*(omega0(iomega)-EFermi+MuGW))*domega/pi
           ...
           IF(GRet) THEN
              IF(omega0(1).LT.Mu) THEN
                 ...
                 ERPA(ik2) = ERPA(ik2) + 0.5d0*(ABS(DIMAG(CInt(1)))*Mu &
                    + ABS(DIMAG(Gg(iFermi)))*omega0(iFermi))*(Mu-omega0(iFermi))/pi
              END IF
           END IF
           IF(omega0(1).LT.EFermi) THEN
              CALL terpc(omega0,G_GW,nomega,1,EFermi,CInt(1))
              koccGW(ik2) = koccGW(ik2) + 0.5d0*(ABS(DIMAG(CInt(1))) + ABS(DIMAG(G_GW(iFermiGW))))*(EFermi-omega0(iFermiGW))/pi
              EIntGW(1) = EIntGW(1) + 0.5d0*(ABS(DIMAG(CInt(1)))*MuGW &
                + ABS(DIMAG(G_GW(iFermiGW)))*(omega0(iFermiGW)-EFermi+MuGW))*(EFermi-omega0(iFermiGW))/pi
              CALL terpc(omega0,G0,nomega,1,EFermi,CInt(1))
              kocc0(ik2) = kocc0(ik2) + 0.5d0*(ABS(DIMAG(CInt(1))) + ABS(DIMAG(G0(iFermiGW))))*(EFermi-omega0(iFermiGW))/pi
              E0T(ik2) = E0T(ik2) + 0.5d0*(ABS(DIMAG(CInt(1)))*EFermi &
                + ABS(DIMAG(G0(iFermiGW)))*omega0(iFermiGW))*(EFermi-omega0(iFermiGW))/pi
           END IF
           ...
           domega = omega(2) - omega(1)
           DO iomega = 1, nomega
              tmp = (iomega-nomega/2)*domega
              IF((tmp.GT.omega0(1)).AND.(tmp.LT.omega0(nomega))) THEN
                 CALL terpc(omega0,Gg,nomega,1,tmp,CInt(1))
                 ...
                 CALL terpc(omega0,G_GW,nomega,1,tmp,CInt(1))
           ...
           IF(ik2.EQ.nk) THEN
              ...
              DO iomega = 1, nomega
                 ...
                 WRITE(29,'(20e20.10)') domega*(iomega-nomega/2), dos(iomega), dosOcc(iomega), dosGW(iomega), tmp, DBLE(EkSCF(ik2))
              END DO
              WRITE(29,*)
              dos = 0.d0
              dosGW = 0.d0
