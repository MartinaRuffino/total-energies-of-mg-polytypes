...
C#endif
      ...
      character*8 alabl
      character(len=recln0),allocatable:: recrd(:)
...
C --- Read input file ---
C#ifndefC LM67
CC     Ridiculously large s_site and s_spec are initially allocated;
CC     later they are reduced to actual size.  rdctrl checks to ensure
CC     that the initial s_site0 and s_spec0 are sufficiently large.
C      allocate(s_site0(nbmx),s_spec0(nbmx))
C
CC     Initialize strux
C      call bcast_strx(-(2**9-1),s_bz,s_ctrl,s_ham,s_pot,s_lat,
C     .  s_mix,s_spec0,s_site0,s_str,nbmx,nbmx)
C
CC     Read input
C      call rdctrl(recrd,recln0,nrecs,prgnam,vrsion,ivn,1,slabl,
C     .  s_bz,s_ctrl,s_ham,s_pot,s_lat,s_mix,s_spec0,s_site0,nbmx,
C     .  s_str,s_move,s_tb,s_optic,s_gw,s_dmft,s_strn)
C
CC     Resize s_site, s_spec
C      j = s_ctrl%nsite
C      allocate(s_site(j))
C      s_site(1:j) = s_site0(1:j)
C      j = s_ctrl%nspec
C      allocate(s_spec(j))
C      s_spec(1:j) = s_spec0(1:j)
C      deallocate(s_site0,s_spec0)
C      allocate(s_ctrl%spid(j))
C      do  j = 1, s_ctrl%nspec
C        s_ctrl%spid(j) = slabl(j)
C      enddo
C#else
      call rdctrlchk(recrd,recln0,nrecs,'LM',vrsion,ivn,F,slabl)
      deallocate(recrd)
      call rx0('lm67')
C#endif
...
C          if (a2vec(strn,len(strn),i,2,', ',2,-3,9,ix,iplx) /= 9) then
C            print *, 'failed to read 9 values from string ... redo'
C            goto 88
C          endif
CC          do  j = 1, 3
CC          do  i = 1, 3
...
C        else
C          i = 0
C          if (a2vec(strn,len(strn),i,4,', ',2,-3,9,ix,plx) /= 9) then
C            print *, 'failed to read 9 values from string ... redo'
C            goto 88
C          endif
C        endif
...
C      lc = 10
C#endif
C#ifndef LMFGWD | LMFGWS | LMFDMFT
...
C#ifdef LM
        if (prgnam == 'lm') then
          call info0(0,0,0,
     .      '%N%1f--rs=#1,#2 --band[~option...] --pdos[~option...] '//
     .      '-mix=#1[,#2] --onesp --weula --rsedit --efrnge')
          call info0(0,0,0,' For --band mode options,'//
     .      ' see doc/generating-energy-bands.html ')
        endif
C#endif
...
C          call info0(0,0,0,
C     . '%N%1f-ef=#%N Exchange-mode-specific options:'//
C     . '%N  --sites[:pair]:site-list'//
C     . '  : to calculate exchange for a subset of sites')
C          call info0(0,0,0,
C     . ' Further switches for exchange printout mode:'//
C     . '%N --wrsj[:j00][:amom][:sscl][:[g]scl=#][:tol=#] --wmfj '//
C     . '--rcut=# --tolJq=# --amom[s]=mom1,mom2,... --2xmsh'//
C     . '%N See documentation in doc/gf.html'
C     .      )
C        endif
C#endif
...
C
C          call info0(0,0,0,
C     .    '%N  Linear scalling playground options'
C     .    //'%N --linscl {msi|ls++}'
C     .    //'%N                Switch to linear scalling branch:'
C     .    //'%N                  msi : matrix sign'
...
C     .      '%N --ctrl=fn%6fname the input file fn (default = actrl).  Extension is appended'//
...
C     .      '%N --xshftx=#,#,# Shift site positions by a uniform translation, multiples of PLAT'//
...
C     .      '%N%1f--wsitex[~opt]%1fsame as --wsite, but positions '//
C     .      'written as fractional multiples of PLAT'//
C     .      '%N%16fOptional ~quad1 shifts site positions to first quadrant'//
C     .      '%N --wpos=fn%6fwrite site positions to file fn.ext'//
C     .      '%N --nfile%8fmodify ctrl file so site file name can be controlled with variable ''file'''//
