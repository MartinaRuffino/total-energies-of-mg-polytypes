C --- Least-squares fit using cubic splines ---
      subroutine fmain
      implicit none
      integer iarg,i,j,n,ifi,i1mach,ipr,frdops,icwgt,icy
      logical cmdstr,a2bin
      integer nr,nc,fopng,rdm,chrln,mkdlst,nlst,odat,oy,oc,ose,owk,osum,
     .  of,odf,oxlst,ier
      double precision xx,var
      parameter (chrln=256)
      character*(chrln) first,string

C Heap allocation
      integer wksize
      parameter(wksize=10000000)
      integer w(wksize)
      common /w/ w
...
C --- Setup ---
   ...
   20 print 333
  333 format(
     .  ' usage: sfit [switches] file'/
     .  1x,'Least-squares fit of data to cubic splines'/
     .  1x,'Switches:'/
     .  3x,'-pr#      sets output verbosity'/
     .  3x,'-qr       quick file read'/
     .  3x,'-br       binary file read'/
     .  3x,'-df=#     use column # for relative standard deviations'/
     .  3x,'-y=#      use column # independent data'/
     .  3x,'-t xlist  tabulates fit at table of points'/
     .  3x,'-var=#    sets variance, if known'/
   ...
   15 continue
      ...
      if (first(1:3) == '-pr') then
        if (.not. a2bin(first(4:40),j,2,0,' ',i,-1)) goto 20
        call pshpr(j)
      else if (first(1:3) == '-qr') then
        frdops = mod(frdops,10) + 1
      else if (first(1:4) == '-df=') then
        if (.not. a2bin(first(5:40),icwgt,2,0,' ',i,-1)) goto 20
      else if (first(1:3) == '-y=') then
        if (.not. a2bin(first(4:40),icy,2,0,' ',i,-1)) goto 20
      else if (first(1:3) == '-br') then
        frdops = mod(frdops,10) + 2
      else if (first(1:5) == '-var=') then
        if (.not. a2bin(first(6:40),var,4,0,' ',i,-1)) goto 20
      ...
      else if (first(1:2) == '-t') then
        iarg = iarg+1
        if (.not. cmdstr(iarg,string)) goto 20
        nlst = mkdlst(string,0d0,-1,w(oxlst))
        call defrr(oxlst,nlst)
        nlst = mkdlst(string,0d0,nlst,w(oxlst))
      endif
      iarg = iarg+1
      goto 15
...
C --- Read data from disk ---
      if (nr == 0) then
        j = 1
        if (mod(frdops,10) == 2) j = 4+1
        ifi = fopng(first,-1,j)
        call defask(i)
        i = i/i1mach(18) - 1
        call defdr(odat,i)
        i = rdm(ifi,frdops,i,' ',w(odat),nr,nc)
        if (i < 0)
     .    call fexit(-1,9,'sfit failed to parse file '//first,0)
...
C --- Spline fit ---
      ...
      call defdr(oc,3*nr)
      call defdr(ose,nr)
      call defdr(owk,7*(nr+2))
      if (icy > nc) call rx1('sfit: missing y column %i',icy)
      of = odat+(icy-1)*nr*i1mach(18)
      if (icwgt == 0) then
        call defdr(odf,nr)
        call dvset(w(odf),1,nr,1d0)
      elseif (icwgt > nc) then
        call rx1('sfit: missing weights column %i',icwgt)
      else
        odf = odat+(icwgt-1)*nr*i1mach(18)
      endif
      call cubgcv(w(odat),w(of),w(odf),
     .  nr,w(oy),w(oc),nr,var,1,w(ose),w(owk),ier)
      if (ier /= 0) call rxi('sfit: cubgcv returned error no. ',ier)

C --- Printout ---
      call defdr(osum,nr)
      call prsfit(nr,w(odat),w(of),w(oy),w(oc),w(owk),var,w(ose),
     .  w(oxlst),w(osum),nlst)
      end

      subroutine prsfit(n,x,f,y,c,wk,var,se,xlst,sum,nlst)
      implicit none
      integer n,nlst
      double precision var,x(n),f(n),y(n),c(n,3),wk(7*(n+2)),
     .  se(n),sum(n)
      double precision xlst(nlst),d,yx,yp,ypp,err
      integer i,j,low,i1mach
      character*120 outs

      if (nlst == 0) then
C      print 30, var,wk(3),wk(4),wk(2)
C   30 format('# Variance =',f7.4,t42,' mean square residual =',f7.4/
C     .       '# Generalized cross validation =',f7.4,t42,
C     .       ' residual degrees of freedom =',f7.2)
      call awrit2(
     .  '# Variance = %,3;3g%42p mean square residual = %,3;3g',
     .  ' ',100,i1mach(2),var,wk(3))
      call awrit2('# Generalized cross validation = %,3;3g%42p'//
     .  ' residual degrees of freedom = %,3;3g',' ',100,
     .  i1mach(2),wk(4),wk(2))

      print 40
   40 format('#'/'# Spline fit at point x is ((c3*d+c2)*d+c1)*d+y(i)'/
     .  '# where x(i) < x < x(i+1) and d = x - x(i)'/'#'/
     .  '#',9x,'input data',17x,'Spline fit',17x,'Spline coffs',
     .  14x,'I(x)-I(x1)'/
     .         '#  i    x',12x,'f',11x,' fit',7x,'std dev',
     .         '     c_1         c_2         c_3')
      sum(1) = 0d0
      do  60  i = 1, n-1
C      print 50, i,x(i),f(i),y(i),se(i),(c(i,j),j=1,3),sum(i)
C   50 format(i4,1p,2e12.4,2x,e12.4,e10.2,3e12.4,e14.5)
      d = x(i+1)-x(i)
      sum(i+1) = (((c(i,3)/4*d+c(i,2)/3)*d+c(i,1)/2)*d+y(i))*d + sum(i)
      outs = ' '
      call awrit8('%,4i%5p%:-1,5;5g%17p%:-1,5;5g%31p%:-1,5;5g'//
     .  '%43p%:-1,3;3g%53p%:-1,5;5g%65p%:-1,5;5g%77p%:-1,5;5g',
     .  outs,120,0,i,x(i),f(i),y(i),se(i),c(i,1),c(i,2),c(i,3))
      call awrit1('%90p%:-1,5;5g',outs,120,-i1mach(2),sum(i))
   60 continue
      outs = ' '
      call awrit6('%,4i%5p%:-1,5;5g%17p%:-1,5;5g%31p%:-1,5;5g'//
     .  '%43p%:-1,3;3g%54p0%66p0%78p0%90p%:-1,5;5g',
     .  outs,120,i1mach(2),n,x(n),f(n),y(n),se(n),sum(n))
C      print 51, n,x(n),f(n),y(n),se(n),sum(n)
C   51 format(i4,1p,2e12.4,2x,e12.4,e10.2,2x,'0',11x,'0',11x,'0',9x,
C     .  e14.5)
      return
      endif

C ... If to tabulate on a list of specified points
      low = 0
      print 331, nlst
  331 format('% rows',i5,'  cols 5  l=x,f,f'',f'''',err')
      do  100  i = 1, nlst
        call huntx(x,n,xlst(i),0,low)
        if (low < 1) low = 1
        if (low > n) low = n
        d   = xlst(i)-x(low)
        if (d > 0 .and. low < n) then
          err = se(low)*(1-d/(x(low+1)-x(low))) +
     .          se(low+1)*(d/(x(low+1)-x(low)))
        elseif (d < 0) then
          err = se(low)
        elseif (low >= n) then
          err = se(n)
        ...
        if (low > n-1) low = n-1
        d   = xlst(i)-x(low)
        yx  = ((c(low,3)*d + c(low,2))*d + c(low,1))*d + y(low)
        yp  = ((3*c(low,3)*d + 2*c(low,2))*d + 1*c(low,1))
        ypp = (2*3*c(low,3)*d + 1*2*c(low,2))
        call awrit5('%,8;8g %,8;8g %,8;8g %,8;8g %,3;3g',' ',100,
     .    i1mach(2), xlst(i), yx, yp, ypp, err)
  100 continue
      end
