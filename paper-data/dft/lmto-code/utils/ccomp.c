...
29 Jun 2005 (version 4) Add switch to suppress automatic appending of extension
...
char *fexts(register char *fname);
void ccomp(FILE *Source, FILE *Dest, char *lpdlist);
void movmem(char *ps, char *pd, int len);
void errchk (char *string);
void next_word(char **psC, char *nam);
int sw(char *s);
int among(char *list, int nlist, char *name);
int match(char *psC);
...
int main(int argc, char *argv[]) {
  char srcnam[128],dstnam[128],olddir[128],*lpdlist=NULL;
...
/*--- get source file name and switches --- */
  ...
  if (!(Options & FILTER)) strcpy(srcnam,argv[iarg++]);

#ifdef MSDOS
  if (!fexts(srcnam) && !(Options & FULLNAME)) strcat(srcnam,".for");
#endif
#ifdef unix
  if (!fexts(srcnam) && !(Options & FULLNAME)) strcat(srcnam,".f");
#endif
...
void ccomp(FILE *Source, FILE *Dest, char *lpdlist) {
  ...
  char cntrl[4096],name[4096],*ps,*psC,*psO; /* cntrl and name shall be no shorter than s to avoid overflow in sscanf later */
...
/*Prints out all #define to stdout and sets up lpdlist */
  ...
  while(ps=s+1,fgets(ps,4095,Source)) {
...
void Error() {
  printf("usage: ccomp [-cdou] source-file [dest-file]\n%s%s%s%s%s%s%s"
        ...
        ,"              -e     File has full name (do not append extension)\n"
