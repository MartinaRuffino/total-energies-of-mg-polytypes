...
C#ifdefC DEBUG
CC     open(111, file='x0qw.allqb',form='unformatted')
C      ifi = fopnx('x0qw.allqb',2,2+4,-1)
C      write (ifi) nq,nmbas,nw,nwx,qp
C      write (ifi) freq(1:nw)
C      do  iq = 1, nq
C      do  iw = 1, nw
C        write(ifi) dble(x0et(1:nmbas,1:nmbas,iq,iw))
C        write(ifi) dimag(x0et(1:nmbas,1:nmbas,iq,iw))
C      enddo
C      enddo
C#endif
...
C#ifdefC DEBUG
CC     open(112,file='etUet.allw')
CC     ifi = fopnx('etUet.allw',2,2,-1)
C      ifi = fopna('etUet',-1,0)
C      do  iw = 1, nwx
C        write(ifi,'(f21.13,3x,255d18.10)')
C     .    freq(iw),(uu(ix,iw),ix=1,nmbas)
C      enddo
C      call fclose(ifi)
C#endif
...
C#ifdefC DEBUG
C      ifi = fopnx('evl_xh.allq',2,2,-1)
C      do  iw = 1, nwx
C        write(ifi,"( f13.5, 100d15.7)") rymev*freq(iw),
C     .    (mxevl_xinvh(iq,iw),iq=1,nq)
C      enddo
C      call fclose(ifi)
C#endif
...
C#ifdefC DEBUG
CC     open(109,file='dxidw.allq')
CC     ifi = fopna('dxidw',-1,0)
C      ifi = fopnx('dxidw.allq',2,2,-1)
C      do  iw = 1, nwx
C        write(ifi,"(f8.2,100d15.7)") rymev*freqm(iw),
C     .    (dxidw_eb(1,1,iq,iw), iq=1,nq)
C      enddo
C      call fclose(ifi)
C#endif
