...
C     implicit real*8 (a-h,o-z)
     ...
     .  ifichkv,ifigwa,ifigwb,ifigwb_,ifigwx1,ifigwx2,isp,ispx,
     .  ispxx,ix,kkk,kkkdummy,l,ldummy,lmxa,lxx,n1q,n2q,n3q,nclassx,m,n,
     .  ncore,ndimh,ngp,nnc,nqbz,nspdummy,IKP,NR_A,procid,iq
C#ifndefC CODE2
C      integer ngc,ngcmx
C#endif
      real(8) qlat(3,3),ovv(20),ef0,z,
     .  a_a,b_a,rofi_anr
      character*120  ext,extn
...
c----------------------------------------------
     ...
     &nbas,     ! Number of atoms in the primitive cell
     &nat,      ! Number of atom in the primitive cell, with augmentation spheres (should be = nbas)
...
c
     ...
     .nqcalc,   ! = number of qp generated by lmfgwd.  Should be nqbze
     .nrank,    ! = number of MPI threads used to generate gwb,gw1,gw2
     .nqirr,    ! = number of qp where bands are actually calculated
     .nbandmx   ! = maximum hamiltonian dimension
      integer, allocatable :: iprocq(:) ! procid that generated data for given iq
      character*256,allocatable:: extp(:) ! thread-dependent extension
      real(8) ::
...
C!!
C      integer:: procid,nrank,ifigwb_,ifigwx1_,ifigwx2_
C      character*256:: extn,aaa
C      character*256,allocatable:: extp(:)
C      integer:: ifiproc,nqixx,nspxx,numprocxx,ixxx
...
C     Read extension, number of threads, thread block array
      ifi = fopnx('mpiqlst',2,1,-1)
      read(ifi,*) ext
      read(ifi,*) ext, ix, nqcalc, nrank
      ix = fext(trim(ext))     ! Store file extension
      print *, 'lmf2gw: read files from ext=',trim(ext),
     .  nqcalc, ' qp made by',nrank, ' threads'
...
C     &              nqbz,plat(1:3,1:3),nqbze
C#else
C     nbas here is number of atoms with augmentation spheres
      read (ifigwb) nbas,nsp,ldim2,ndimh,alat,qlat,ef0,
     &              nqbz,plat(1:3,1:3),nqbze, nqirr
      nqbze=nqirr !nqbze means number of calculated bands (irreducible only) feb2012takao
C#endif
      if (nqbze /= nqcalc) call rx('lmf2gw: file mismatch')
...
!!
      ...
      enddo
C      Not used any more ... move ipbi to m_hamindex
CC     Read map, sites with augmentation spheres to sites in lmf
C      allocate(ipbi(nbas))
C      forall (ibas=1:nbas) ipbi(ibas) = ibas
C      read(ifefclass,*,err=230,end=230) nat
C      if (nat /= nbas) call rx('lmf2gw_2: mismatch in CLASS data')
C      read(ifefclass,*) ipbi
C  230 continue
      close(321)
...
!! Band Part
     ...
     &  cphi(ldim2, nbandmx), evl(nbandmx) )
      ...
      do ikp = 1, nqbze
        do isp = 1, nsp
C#ifndefC CODE2
C          read (ifigwb) qbze(:,ikp),nband
C#else
          ifigwb_ = fopnx('gwb'//trim(extp(iprocq(ikp))),2,5,-1)
...
!! Plane wave part
C#ifndefC CODE2
C        read (ifigwb_) ngp,ngc
C#else
         read (ifigwb_) ngp !,ngc
C#endif
C#ifndefC CODE2
C          allocate( geig(ngp, nbandmx),
C     &              ngvecp(3,ngp),ngvecc(3,ngc) )
C#else
...
C#endif
          geig=1d20
C#ifndefC CODE2
C        read(ifigwb_) ngvecp, ngvecc,geig(:,1:nband)
C#else
          read(ifigwb_) ngvecp,geig(:,1:nband)
C#endif
          write(ificg) ngp
          write(ificg) geig(1:ngp,1:nbandmx)
C         write(6,'(" geig sumcheck= ",2d13.6)')sum(abs(geig(1:ngp,1:nband)))
...
C          if(ikp>nqbz) goto 888
C#else
          deallocate( geig,ngvecp)!takao feb2012 ,ngvecc )
cc          if(iq>nqbz) goto 888 !comment out this line July2012takao
C#endif

!! --- VXCFP
          ifigwx1 = fopnx('gw1'//trim(extp(iprocq(ikp))),2,5,-1)
          ifigwx2 = fopnx('gw2'//trim(extp(iprocq(ikp))),2,5,-1)
...
!! ... augmentation indexing information
      ...
      ncoremx = ncoremx/nsp
      allocate(ec_d(ncoremx, nclass, nsp),gx_d (nrmx, 0:lmxamx, nphimx,
     .  nclass,nsp),gcore_d(nrmx, ncoremx, nclass,nsp)  )
      gx_d = 0
      rewind ifigwa
      nnc =0
      konf =0
...
!! --- ECORE PHICU
     ...
     .    '(''  l  k isp       ecore      gc(rmax)     <gc gc>'')')
        ...
        do l = 0, lmxa
          ...
          do  kkk = l+1, mod(konf(l+1,ibas),10)-1
            ...
            call gintxx(gcore_A(1,icore),gcore_A(1,icore),a_A,b_A,nr_A,
     .        ovv(1))
            write(6,'(3i3,f15.6,f12.6,f15.10,3i3)')
     .        l,kkk,isp,ec_A(icore),gcore_A(nr_A,icore),ovv(1),icore,
     .        icor1
          enddo
        enddo
        enddo
 ...
 3001 continue
      ...
      if(.not.allocated(cphi_d)) allocate(cphi_d(ldim2,  nband, nqbze,nsp))
      if(.not.allocated(geig_d)) allocate(geig_d(ngpmx,  nband, nqbze,nsp))
     ...
     & qbze,efermi,ec_d,evl_d,vxclda,gx_d,gcore_d,nocore)
      close(ifi)
...
C plen: path length; extlen: ext length; maxsiz: name (including ext)
      ...
      character*(plen) path
      integer tabsw(mxnam), tabuni(mxnam),nnam
...
C       end
   ...
   12 continue
     ...
     &  zz,aa,bb,bas,alat,plat,
     &  qbze,efermi,ec,evl,vxclda,gx,gcore,nocore)
c
