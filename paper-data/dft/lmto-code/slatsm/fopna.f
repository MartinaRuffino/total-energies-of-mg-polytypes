...
Cr                 Returns 1 if file exists, 0 otherwise
Cr
Cr   ffnam(fnam,fnam) returns full file name
Cr
Cr   fopni(fnam,lunit,ista) opens a file associated with logical unit,
Cr     if available a name is associated with lunit
Cr     If this is the case, fopni stores name in fnam and returns 0
Cr     If not, fopni does nothing and returns -1.
Cr
Cr   fopnig(fnam,lunit,ista) is like fopni, but returns full file name.
Cr
...
C Passed parameters
      ...
      integer ista,lunit
C Local parameters
      integer fext,fextg,fopn,fopno,fopnn,fhndl,fadd,fxst,fpath,fpathg,fopng,fopnx,fopni,fopnig
      integer mode
...
C#ifndef AIX
      ...
      entry fopni(fnam,lunit,ista)
      fopni = fopnx(fnam,194,0,lunit) ! File name belonging to lunit, sans extension
      if (fopni < 0) return
      mode = 100 + 70*mod(ista/8,2)
      fopni = fopnx(fnam,mode,mod(ista,8)+bitand(ista,32),lunit) ! Open the file
      return

      entry fopnig(fnam,lunit,ista)
      fopni = fopnx(fnam,192,0,lunit) ! Full name belonging to lunit
      if (fopni < 0) return
      mode =   2 + 70*mod(ista/8,2)
      fopnig = fopnx(fnam,mode,mod(ista,8)+bitand(ista,32),lunit) ! Open the file
      return
...
C ... Passed parameters
      ...
      integer, intent(in) :: ista,lunit,mode
C ... Local parameters
      integer j1,j2,fextg,fopng,fopnx
      character(len=1023) :: pfnam
      character(len=127) :: ext
      character(len=1023), save :: tmpdir = ' '
      ...
      call sanrg(.true.,mode,0,4,'fopnt','mode')
...
C#ifdef unix
      parameter (extlen=127, plen=1023, maxsiz=2047)
C#ifdefC DEBUG
C      parameter (IDBG=10)
C#else
      parameter (IDBG=110)
...
C- Concatenates two names, optionally lowering the case
     ...
     .    '" was truncated to "',dest,'"'
        call rx('namcat: dest shorter than expected.')
      endif
...
C      character fnam*20,strn*120
C*      call pshpr(110)
C
CC     Check fopnT
CC      i = fopnT(' ' ,-1,0,12)
CC      i = fopnT('tmp' ,-1,0,0)
CC      ifi = fopn('tmp')
CC      call fshow
CC      i = fhndl('tmp')
CC      print *, i
CC      call dfclos(fopn('TMP'))
CC      i = fopnT('tmp' ,-1,0,2)
CC      print *, i
CC      call fshow
CC      i = fhndl('tmp')
CC      print *, i
...
C
C      ifi = 10
C      print "(' ... test fopna(fnam=''pqr'',ifi,0), ifi=',i3)", ifi
...
C      ifi = fhndl('xyz')
C      jfi = fhndl('pqr')
C      print "(/' ... test fhndl(''xyz.123''),fhndl(''pqr.dat'') : ',2i4)", ifi,jfi
C
C      print "(/' ... Close, reopen file ''pqr.dat'' with position=append, ifi='i4)", jfi
C      call fclose(ifi)
C      call fshow
C      ifi = fopna('pqr' ,ifi,32)
C      call fshow
C      print "(' ... Append ''this is last line'' to file ''pqr.dat''')"
C      write(ifi,'(a)') 'this is the last line'
C
C      print "(' ... Close, reopen file ''pqr.dat'' with rewind, ifi='i4)", jfi
C      call fclose(jfi)
C      ifi = fopna('pqr' ,ifi,0)
C      print "(/' ... Print out file ''pqr.dat''')"
C      do i = 1,4
C        read(ifi,'(a)') strn
C        print '(a)', trim(strn)
C      enddo
C
C      print "(/' ... retest fopng(fnam=''xyz.123'',-1,4)')"
C      ifi = fopng(fnam,-1,4)
C      call fshow
C      print "(' ... read array from binary file xyz.123, ifi=',i4)", ifi
C      call rdm(ifi,0002,8,' ',s,2,2)
C      call ywrm(0,' testing fopng',1,lgunit(1),'(5f12.2)',s,2,2,2,2)
C
C      print "(/' ... test fopng(fnam=''xyz.123'',-1,4), file already open')"
C      ifi = fopng(fnam,-1,4)
C      call fshow
C
C      print "(' ... test fadd(''abc'',ifi,0)')"
C      ifi = 14
C      ifi = fadd('abc',ifi,0)
C      call fshow
C
C      print "(' ... call fext(''.dat2'')')"
C      ifi = fext('.dat2')
C      call fshow
C
C      print "(' ... test fadd(''abc'',ifi,0) with different extension')"
C      ifi = fadd('abc',-1,0)
C      call fshow
C      print "(' ... test fopn(''abc'')')"
C      ifi = fopn('abc')
...
C      call fshow
C      jfi = fopna('abc',-1,0)
C      if (ifi /= jfi) stop 'bug in fopna'
C
CC      call awrit0('%N ... test ftflsh (stdout, abc.dat2)',' ',80,lgunit(1))
CC      write(*,"(' ... testing ftflsh(6) ... ')",advance='no')
CC      call ftflsh(6)
CC      write(*,*) 'after ftflsh(6)'
CC      write(*,*) '... calling ftflsh(-1)'
CC      write(ifi,*) '... before ftflsh(-1)'
CC      call ftflsh(-1)
CC      write(*,*) '... calling ftflsh(ifi)'
CC      write(ifi,*) '... before ftflsh(ifi)'
CC      call ftflsh(ifi)
CC      write(ifi,*) '... after ftflsh(ifi)'
CC
CC      print *, '... invoking system  call ''cat abc.dat2'' ...'
CC      call ftflsh(6)
CC      call fsystm('cat abc.dat2',i)
CC      call awrit1('%N ... system  call ''cat abc.dat2'' returned... %i',' ',80,lgunit(1),i)
C
C      call awrit0('%N ... restore ext to dat',' ',80,lgunit(1))
C      ifi = fext('.dat')
C
C      print "(/' ... test fclr(''abc'',-1) --- refer to file by name')"
C      call fclr('abc',-1)
C      call fshow
C
C      print "(' ... test fclr('' '',10) -- refer to file ''pqr'' by logical unit')"
C      call fclr(' ',10)
C      call fshow
C
C      print "(/' ... test fopna(''xyz'',-1,8) to show internal file table not affected')"
C      ifi = fopna('xyz',-1,8)
C      write (ifi,*) 'test'
C      call fclose(ifi)
C      call fshow
C
C      print "(/' ... reopen with fopna(''xyz'',-1,0) to show modified internal file table')"
C      ifi = fopna('xyz',-1,0)
C      read(ifi,'(a10)') fnam
C      print *, 'after opening, read line from file (should be ''test''): ',trim(fnam)
C      ifi = fxst('xyz')
C      jfi = fxst('zzz')
C      kfi = fopnx('xyz.123',172,-1,-1)
C      call awrit3(' ifi for xyz is %i, for zzz is %i, for xyz.123 %i',' ',80,lgunit(1),ifi,jfi,kfi)
C      call fshow
C
C      call awrit0('%N ... show fopn works without fadd',' ',80,lgunit(1))
C      ifi = fopn('TMP')
C      call fshow
C
C      call awrit0('%N ... call fopnx to test locase, "NEW","OLD"',' ',80,lgunit(1))
