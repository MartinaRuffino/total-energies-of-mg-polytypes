...
Ci        : It is delimited by a double-comma, e.g.
...
Ci        :        4=> printout mode, but nothing added to string prs.
Ci        :            Reset ip to len_trim(prs); write and reset prs(1:) if ifipr>0
Ci        :            It uses only swt(5) and swt(6)
...
Cb   arrays
Cu Updates
Cu   30 Dec 16 Pass prs as argument; new mode swt(5)=4
Cu   06 Nov 01 Initially created
C ----------------------------------------------------------------
      implicit none
C ... Passed parameters
      integer, parameter :: recl=500
      integer swt(7),cast,n1,count
      character*(1) instr(*)
      character*(*) token,descr,resc(1),sep0,sep
      character(len=recl) :: prs
      double precision resn(1)
C ... Local parameters
      logical logval,l1,l2,lhelp,lshow,lread
      integer i,ifipr,ifmt,io,ip,is,isave,it(99),ival,k,ks,
     .  linstr,retval,ltok,quswt,lfixl,lopt,lmult,lio
      procedure(integer) :: rdtk2,a2vec,awrite
      procedure(logical) :: parstr
      character(len=recl) :: aa
      character*8 nmcast(0:4),fmt*25
      save ip
      data nmcast /'logical','char','integer','real','double'/
      data ip /0/

C ... Setup
      if (swt(5) == 4) then  !Special write mode
        ip = len_trim(prs)
      ...
      ifipr = quswt('lunit',swt)
      ltok = len(token)
      lhelp = quswt('io,help',swt) == 1
      lshow = quswt('io,w',swt) == 1
      lread = quswt('io,r',swt) == 1
      lio   = quswt('io',swt)
...
C --- Try and find token ---
      ...
      if (token == ' ') then
        ...
        is = rdtk2(instr,linstr,token,sep0,lopt == 1,swt(1))
      endif
...
C ... Parse for vector of numerical arguments
      ...
      swt(7) = is
      is = is-1
      k = a2vec(instr,linstr,is,cast,sep,len(sep),-99,count,it,resn(n1))
      if (lfixl == 0 .and. k < -1) then ! successfully read at least one expression
        is = swt(7)
        is = is-1
        k = a2vec(instr,linstr,is,cast,sep,len(sep),-99,-k-1,it,resn(n1))
      endif
      retval = k
...
C       ip = ip+2
        if (i < k) then
          if (prs(ip:ip) /= sep(1:1)) then
            ip = ip+1
            write(prs(ip:ip),'(a)') sep(1:1)
          endif
        elseif (retval < 0 .and. lio /= 2) then
          ip = ip+1
...
C     3.  token matches
      do  k = i1, linstr
        ...
        if (instr(k) == ct) then
          ...
          if (iterm > 0) then
            do  is = 1, ltok
              if (instr(k+is-1) /= token(is:is)) goto 31
            enddo
C           A match was found
            rdtk2 = ltok+k
            return
