...
C     call zprm('zqinv',2,a,lda,n,n)
      ...
      if (n > lncut .and. lncut > 0) then
        ...
        if (n > 1 .and. lwrk < n) call rx('zqinv: work too small')
        if (cs == 'h') then
          call zhetrf('L',n,a,lda,w,w(1+n),lwrk,ierr)
          if (ierr /= 0) call rx('zqinv: singular matrix')
          call zhetri('L',n,a,lda,w,w(1+n),ierr)
          do  i = 1, n
          do  j = i+1, n
            a(1,i,j) = a(1,j,i)
            a(2,i,j) =-a(2,j,i)
          ...
          if (ierr /= 0) call rx('zqinv: singular matrix')
          if (n == 1) then
            call zgetri(n,a,lda,w,xx,1,ierr)
          else
            call zgetri(n,a,lda,w,w(1+n),lwrk,ierr)
          endif
        endif
