      subroutine zpmblk(nblk,ipm,ioffo,ioffn,lds,SRC,mode,alfa,nlma,ldd,
     .  DST,nmax)
C- Clips and optionally permutes subblocks of a double complex array
C ----------------------------------------------------------------------
Ci Inputs
...
Ci          Thus DST subblock has size ioffn(i+1)-ioffn(i)
...
C ... Passed parameters
     ...
     .  ipm(nblk),nmax
      double complex dst(ldd,1),src(lds,1),alfa(1)
C ... Local parameters
...
C     if (mode3 /= 0) call awrit2('pm ipm  %n,3i',' ',80,6,nblk,ipm)
...
C     call awrit2('pm iofn %n,3i',' ',80,6,nblk+1,ioffn)
     ...
     .  call rx('zpmblk: must permute one of SRC or DST')
      if (mode1 >= 2 .and. mode3 == 1)
     .  call rxi('zpmblk: imcompatible switches, mode=',mode)
      if (mode2 >= 2 .and. mode3 == 2)
     .  call rxi('zpmblk: imcompatible switches, mode=',mode)
      call sanrg(.true.,mode3,0,2,'zpmblk:','100s digit mode')
...
C      subroutine fmain
C      implicit none
C      integer nblk,ioffs(5),ioffd(5),iprm(4)
C      double precision s(10),d(10)
C      integer nmax,i,mode
C
C      ioffs(1) = 0
C      ioffs(2) = 1
C      ioffs(3) = 3
...
C      print 332, iprm
C
CC     Do the permutation
CC     NB: 4000 should have no effect; just tests different branch.
C      call dpzero(d,10)
C      mode = 4100
C      call pmblk(nblk,iprm,ioffs,ioffd,1,s,mode,0d0,1,1,d,nmax)
C
C      print 333, (s(i), i=1,10)
C      print 333, (d(i), i=1,10)
C  333 format(10f8.1)
C
CC     Undo the permutation
C      mode = 0200
C      call dpzero(s,10)
C      call pmblk(nblk,iprm,ioffd,ioffs,1,d,mode,0d0,1,1,s,nmax)
C      print 333, (s(i), i=1,10)
C
CC     Same permutation, but use mode for sequential DST order
C      call dpzero(d,10)
C      mode = 4120
C      call pmblk(nblk,iprm,ioffs,ioffd,1,s,mode,0d0,1,1,d,nmax)
C      print 333, (d(i), i=1,10)
