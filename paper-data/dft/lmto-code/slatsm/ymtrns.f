...
C ... Local parameters
      ...
      call sanrg(.true.,mode,0,1,'ymtrns','mode')
      do  n = n1, n2
...
C      do  ir = 1, lda
C        do  ic = 1, ldb
C          if (yy(ir,ic,1) /= yy0(ir,ic,1)) stop 'oops'
C          if (yy(ir,ic,2) /= yy0(ir,ic,2)) stop 'oops'
C        enddo
C      enddo
C
...
CC      do  ir = 1, lda
CC        do  ic = 1, ldb
CC          if (yy2(ir,ic,1) /= yy0(ir,ic,1)) print *, ir,ic
CC          if (yy2(ir,ic,1) /= yy0(ir,ic,1)) stop 'oops'
CC          if (yy2(ir,ic,2) /= yy0(ir,ic,2)) print *, ir,ic
CC          if (yy2(ir,ic,2) /= yy0(ir,ic,2)) stop 'oops'
CC        enddo
CC      enddo
C
C
