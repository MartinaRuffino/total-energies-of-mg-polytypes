...
C ... Local parameters
      ...
      n = n+1
      call sanrg(.true.,nrow,nrowr,nrowr,'crdfb: ','nrow'//outs(1:n))
      call sanrg(.true.,ncol,ncolr,ncolr,'crdfb: ','ncol'//outs(1:n))
      call sanrg(.true.,cast,castr,castr,'crdfb: ','cast'//outs(1:n))

      n = nrowr*ncolr

      if (mod(cast,10) == 2) then
        if (.not. lddump(array,2*n,ifi)) goto 99
      elseif (mod(cast,10) == 1) then
