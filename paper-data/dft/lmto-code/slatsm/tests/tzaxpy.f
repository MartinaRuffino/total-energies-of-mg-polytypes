      ...
      character outs*80
      INTEGER   lda, ldb, ldc, ldw, n, m, i, ic, count
...
C     call ywrm(0,'b',1,6,'(4f16.10)',sb,1,ldb,l,m)
     ...
     .    call rxs('cannot parse',outs)
      ...
      if (count == 0) then
    5 CALL scalar_zaxpy( sc, sa, sb, ldc*ldc, lda*lda, ldb*ldb, n, m )
      count = count+1
...
C --- In-line matrix multiplication ---
  ...
  125 format(' starting in-line call ... elapsed wall time',F9.4)
      temp = cpusec()
      do  ic = 1, count
      CALL scalar_zaxpy(sc, sa, sb, ldc, lda, ldb, n, m )
      enddo
...
C --- zgemm matrix multiplication ---
  ...
  129 format(' starting zaxpy call ... total elapsed wall time',F9.4)
      do  ic = 1, count
        call zaxpy(n*m,pb,pa,1,pc,1)
      enddo
c      call mmul(n, m, l, pa, lda, pb, ldb, pc, ldc)
      p_time = (cpusec() - temp)/count
      pwalltime = delwc()
      print 130, pwalltime
  130 format(' finished zaxpy call ... elapsed wall time',F9.4)
      if (p_time == 0d0) p_time = 1
      p_mflops = mflops / p_time
      CALL diff('compare in-line to zaxpy', sc, pc, ldc, n, m)
  ...
  110 format(/1X,'Serial CPU time: ',F7.3,'  Serial MFlops: ',F7.0,
     ...
     .       /1X,' zaxpy CPU time: ',F7.3,'   zaxpy MFlops: ',F7.0,
     .                 '   Wall time: ',F7.3,' (',f14.8,' /product)':
...
C- Compare the two arrays for differences
  ...
  110 FORMAT(1X,a,'... arrays agree to within tol=',1pe8.1)
      END
C      SUBROUTINE fast_matrix_multiply(c, a, b, ldc, lda, ldb, n, m, l)
CC- Matrix Multiplication with zgemm
C      integer lda, ldb, ldc, n, m, l
C      double complex a (lda,*), b(ldb,*), c(ldc,*)
C      call zgemm ('N','N',n,m,l,(1d0,0d0),a(1,1),lda,b(1,1),ldb,
C     .  (0d0,0d0),c(1,1),ldc)
C      end

      SUBROUTINE scalar_zaxpy(c, a, b, ldc, lda, ldb, n, m)
C- In-line matrix zaxpy
