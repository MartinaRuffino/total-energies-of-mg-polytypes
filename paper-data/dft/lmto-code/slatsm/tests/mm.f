...
C Get dimension
      ...
      if (myid == master) print *, 'Usage: mm -n=#'
...
C     send b to each other process
        call MPI_BCAST(b, n*n, mpi_real8, master,
     .                 MPI_COMM_WORLD, ierr)
C        send a row of a to each other process; tag with row number
        do  i = 1,numprocs-1
          do  j = 1,n
            buffer(j) = a(i,j)
          enddo
          call MPI_SEND(buffer, n, mpi_real8, i,
     .      i, MPI_COMM_WORLD, ierr)
          numsent = numsent+1
        enddo

        do  i = 1,n
          call MPI_RECV(ans, n, mpi_real8,
     ...
     .                  status, ierr)
          ...
          if (numsent < n) then
            ...
            enddo
            call MPI_SEND(buffer, n, mpi_real8,
     .                    sender, numsent+1, MPI_COMM_WORLD, ierr)
            numsent = numsent+1
          else
            call MPI_SEND(1.0, 1, mpi_real8, sender,
     .                    0, MPI_COMM_WORLD, ierr)
          endif
        enddo
      else
C     slaves receive B, then compute rows of C until done message
        call MPI_BCAST(b, n*n, mpi_real8,
     .                 master, MPI_COMM_WORLD, ierr)
    1   continue
        call MPI_RECV(buffer, n, mpi_real8, master,
     .                MPI_ANY_TAG, MPI_COMM_WORLD, status, ierr)
        if (status(MPI_TAG) == 0) then
          ...
          do  i = 1,n
            ...
            enddo
          enddo
          call MPI_SEND(ans, n, mpi_real8, master,
