...
C ----------------------------------------------------------------
      implicit none
      integer n,bit
      bittst = (mod(n,bit+bit) - mod(n,bit) == bit)
      end
...
C#endif
   ...
   10 s = 10*s
C#ifdefC QUAD
C      if (is > 0) dtrunc = int(s*dble(x))/s
C      if (is < 0) dtrunc = int(dble(x)/s)*s
C#else
      if (is > 0) dtrunc = int(s*x)/s
      if (is < 0) dtrunc = int(x/s)*s
C#endif
      end
...
C#ifdefC QUAD
C      if (is > 0) dround = xnint(s*dble(x))/s
C      if (is < 0) dround = xnint(dble(x)/s)*s
C#else
