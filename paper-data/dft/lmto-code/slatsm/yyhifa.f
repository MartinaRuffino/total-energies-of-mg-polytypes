...
C ---  Determine the largest off-diagonal element in row imax. ---
   ...
   50   continue
        if (dcabsr(ar(imax,imax),ai(imax,imax)) < alpha*rowmax)
     .    go to 60
          kstep = 1
          swap = .true.
          go to 80
...
c                   if  c /= 0, the inverse is computed,
c                   if  b /= 0, the determinant is computed,
c                   if  a /= 0, the inertia is computed.
c
...
c        and  yhico  has set rcond == 0.0
c        or  yyhifa  has set  info /= 0 .
c
c     Adapted from linpack's version dated 08/14/78 .
c     james bunch, univ. calif. san diego, argonne nat. lab
c
