...
C     cm = 'N'
      ...
      do  2  i = 1, j
        ...
        elseif (cs(i:i) == 'b') then
          lsx = .true.
        elseif (cs(i:i) == 'h') then
          lherm = .true.
        elseif (cs(i:i) == '4') then
C         cm = 'N4'
        elseif (cs(i:i) == '1' .or. cs(i:i) == '2') then
        elseif (cs(i:i) == 'l') then
          lapack = .true.
        elseif (cs(i:i) /= ' ') then
...
C       if (cs(1:1) /= 't') call yyprm('b in zqinvb',2,b,ldb,2*ldb,n,nb)
C       if (cs(1:1) == 't') call yyprm('t in zqinvb',2,b,ldb,2*ldb,nb,n)
        call yyqnvb(cs,a,a(1+lda),2*lda,n,nb,w,ldw,w2,b,b(1+ldb),2*ldb,
     .    ierr)
C       if (cs(1:1) /= 't') call yyprm('a^-1 b',2,b,ldb,2*ldb,n,nb)
C       if (cs(1:1) == 't') call yyprm('b a^-1',2,b,ldb,2*ldb,nb,n)
        if (ierr /= 0) return
