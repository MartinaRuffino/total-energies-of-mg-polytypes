...
Ci         narg<0 : sign used as a flag to skip past leading terminators
Co Outputs:
...
C ----------------------------------------------------------------------
      implicit none
C Passed Parameters
      integer iarg,ip,cast,narg,itrm,it(*),nread
      character*(*) tok,sep
      double precision res(abs(narg))
C Local Variables
...
C --- Read argument, possible error if missing ---
      ...
      nread = -999
      if (iarg <= 0) then
        if (iarg < 0) ltmp = nxargs(nxargs(-1)-1) == 0
      ...
      ip = 0
      nread = 0
      ltmp = lsequ(strn,tok,len(tok),' ',ip) .or. tok == ' '
      if (.not. ltmp) then
        goto 99
      endif
      ip = ip-1
      ...
      it(1) = 1
C     Move past leading terminators
      if (narg < 0) then
        do
          call chrps2(strn,sep,nsep,0,ip,it)
          if (it(1) == 0 .or. ip+1 >= len(strn)) exit
          ip = ip+1
        enddo
      endif
      nread = a2vec(strn,len(strn),ip,cast,sep,nsep,itrm,abs(narg),it,res)
...
C          parsing for arguments following token
Ci  narg:  number of values to parse.
Cio Inputs/Outputs
Co   ip:   on input, position in strn where to start parsing
Co         string begins at ip=0
Co         on output, position in strn on exit.
Co Outputs
Co   res:  Vector of numbers that were converted
...
Co        -n if error on conversion of argument n
Cu Updates
Cu   16 Jul 09  Allow itrm <  0
C ----------------------------------------------------------------------
      implicit none
C Passed Parameters
      integer lstr,ip,cast,narg,itrm,it(*)
      character*(*) tok,sep,strn
      double precision res(narg)
C Local Variables
      logical parstr
      character term*1
      integer jp,np,nsep,lentok,a2vec,jtrm
...
C --- Find end of string ---
      ...
      it(1) = 0
      if (jtrm <= nsep)
     .  call chrps2(strn,sep(jtrm:nsep),nsep-jtrm+1,lstr,jp,it)
      if (it(1) /= 0) then
        np = jp
      else
        np = lstr
