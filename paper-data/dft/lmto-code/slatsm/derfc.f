...
Cc
C   10 if (x > xsml) goto 20
Cc
Cc === erfc(x) = 1.0 - erf(x)  for  x < xsml ===
C      derfc = 2.0d0
C      return
Cc
C   20 if (x > xmax) goto 40
C      y = dabs(x)
C      if (y > 1.0d0) goto 30
Cc
Cc === erfc(x) = 1.0 - erf(x)  for abs(x) <= 1.0 ===
C      if (y < sqeps) derfc = 1.0d0 - 2.0d0*x/sqrtpi
C      if (y >= sqeps)
C     . derfc = 1.0d0 - x*(1.0d0 + dcsevl (2.d0*x*x-1.d0,erfcs, nterf))
C      return
Cc
Cc === erfc(x) = 1.0 - erf(x)  for  1.0 < abs(x) <= xmax ===
C   30 y = y*y
C      if (y <= 4.d0) derfc = dexp(-y)/dabs(x) *
C     .  (0.5d0 + dcsevl ((8.d0/y-5.d0)/3.d0, erc2cs, nterc2))
C      if (y > 4.d0) derfc = dexp(-y)/dabs(x) *
C     .   (0.5d0 + dcsevl (8.d0/y-1.d0, erfccs, nterfc))
C      if (x < 0.d0) derfc = 2.0d0 - derfc
C      return
Cc
...
c
C --- derfc(x) = 1.0d0 - erf(x) for x < xsml ---
      derfc = 2.d0
      return
c
...
c
   20 continue
c     if (i == nos) call errmsg('INITDS: eta may be too small',1)
      initds = i
c
      return
      end
...
C#ifdefC TEST
CC tests the complement of the error function
C      program test
C      double precision x,derfc,z,c,dsqrt
C      print *, 'x=?'
C      read(*,*) x
C   10 c = derfc(x)
C      print *, sngl(x),c,1-c
C      x = x*10.d0**.1d0
C      if (dabs(x) < 50) goto 10
C      stop
C      end
C#endif
