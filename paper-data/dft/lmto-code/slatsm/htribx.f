...
c
C#ifdef BLAS
            call yydotc(l,ar(1,i),ai(1,i),1,zr(1,j),zi(1,j),1,s,si)
C#elseC
C            do 110 k = 1, l
C               s = s + ar(k,i) * zr(k,j) + ai(k,i) * zi(k,j)
C               si = si + ar(k,i) * zi(k,j) - ai(k,i) * zr(k,j)
C  110       continue
C#endif
...
c
C#ifdef BLAS
            call yyaxpy(l,-s,-si,ar(1,i),ai(1,i),1,zr(1,j),zi(1,j),1,
     .                 .true.)
C#elseC
C            do 120 k = 1, l
C               zr(k,j) = zr(k,j) - s * ar(k,i) + si * ai(k,i)
C               zi(k,j) = zi(k,j) - si * ar(k,i) - s * ai(k,i)
C  120       continue
C#endif
c
  130    continue
