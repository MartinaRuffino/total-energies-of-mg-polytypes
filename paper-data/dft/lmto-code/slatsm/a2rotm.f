...
C- Generate rotation matrix from a sequence of rotations, ascii input
C ----------------------------------------------------------------------
Ci strn  :Sequence of elemental rotation matrices.  Syntax: rot1[,rot2...],
...
Co Outputs
Co rotm   :rotation matrix.  Follow with a call to rm2eua to find Euler angles.
Cr Remarks
Cr   Rotations defined in this sense, for a right-handed coordinate system
Cr   (meaning positive rotation around z is counterclockwise) rotate the
Cr   coordinate system.  Sometimes this convention is called intrinsic or passive.
Cr
Cr   The opposition convention (sometimes called extrinsic, or active) rotates the
Cr   rotates vectors abount a fixed coordinate system.  rotm for the active
Cr   convention is the inverse or transpose of that for the passive one.
Cr
Cr   Nevertheless, the rotations actually carried out for orbital and spin angular
Cr   momentum use the "active" sense.  See rotspu
C ----------------------------------------------------------------------
      implicit none
      logical linv
...
C --- Entry point to accumulate new rotation ---
    5 continue
      ...
      if (itrm > 1) then
        call dpzero(r,3)
        if (ss(ich+2:ich+2) /= ':') goto 999
        itrm = itrm/2
        if (itrm > 3) call rx('bug in a2rotm')
   11   r(itrm) = 1
        ich = ich+2
      else
        ich = ich+1
        if (a2vec(ss,ls,ich,4,',)',2,2,3,ix,r) /= 3) goto 999
      endif
      if (a2vec(ss,ls,ich,4,', ',2,2,1,ix,angle) /= 1) goto 999
      call dpzero(rotj,9)
C ... Rotation about r
      call dscal(3,1/dsqrt(ddot(3,r,1,r,1)),r,1)
      ct = r(3)
      phi = 0
      if ( ct > 1 .and.  ct < 1-fuzz) ct = 1
      if (-ct > 1 .and. -ct < 1-fuzz) ct = -1
      if (dabs(ct) < 1) phi = datan2(r(2),r(1))
      theta = dacos(ct)
...
C ... rotm <- rotj*rotm
     ...
     .    irot,alpha,beta,gamma)
        if (ipr > 40) print 335, ((rotm(i,j),j=1,3),i=1,3)
  335   format((3f15.9))
...
C        if (dabs((phi-(alpha-gamma-pi)/2)) > 1d-10) then
C          print *, 'warning: phi off', need fix this
C          phi = phi - pi
C        endif
C        if (dabs((phi-(alpha-gamma-pi)/2)) > 1d-10)  stop 'phi off'
C        gp = (alpha+gamma)/2
C        print *, 'gp=',gp
C        print *, 'angle=', angle,
...
C- Generate Euler angles from a rotation matrix
      ...
      pi = 4*datan(1d0)
      cb = r(3,3)
      if (dabs(cb) > 1 .and.  dabs(cb) < 1+fuzz) cb = sign(1d0,cb)
C --- Handle case beta is near zero or pi ---
      if (dabs(dabs(cb)-1) < dsqrt(fuzz)) then
        xx = r(1,3)**2 + r(2,3)**2
        yy = r(3,1)**2 + r(3,2)**2
      ...
      if (dsqrt(r(3,2)**2 + r(3,1)**2) < fuzz .or.
     .    dsqrt(r(2,3)**2 + r(1,3)**2) < fuzz) then
        if (cb > 0) then
          apg = datan2(r(1,2),r(2,2))
          if (r(3,1)**2+r(3,2)**2 /= 0d0) then
            alpha = datan2(r(3,2),r(3,1))
            gamma = apg - alpha
          elseif (r(1,3)**2+r(2,3)**2 /= 0d0) then
          ...
          if (r(3,1)**2+r(3,2)**2 /= 0d0) then
            alpha = datan2(r(3,2),r(3,1))
            if (dsqrt(r(3,1)**2+r(3,2)**2) < d1mach(3)*2) alpha = 0
            gamma = alpha - amg
          elseif (r(1,3)**2+r(2,3)**2 /= 0d0) then
            gamma = datan2(r(2,3),-r(1,3))
            alpha = amg + gamma
...
C ... Check validity of entire rotation matrix
      ...
      cg = dcos(gamma)
      sg = dsin(gamma)
C      print *,   dabs(r(1,1)-(ca*cb*cg-sa*sg)) > fuzz*10 ,
C     .    dabs(r(1,2)-(sa*cb*cg+ca*sg)) > fuzz*10 ,
C     .    dabs(r(1,3)-(-sb*cg)) > fuzz*10 ,
C     .    dabs(r(2,1)-(-ca*cb*sg-sa*cg)) > fuzz*10 ,
C     .    dabs(r(2,2)-(-sa*cb*sg+ca*cg)) > fuzz*10 ,
C     .    dabs(r(2,3)-(sb*sg)) > fuzz*10 ,
C     .    dabs(r(3,1)-(ca*sb)) > fuzz*10 ,
C     .    dabs(r(3,2)-(sa*sb)) > fuzz*10 ,
C     .    dabs(r(3,3)-cb) > fuzz*10
      if (dabs(r(1,1)-(ca*cb*cg-sa*sg)) > fuzz*10 .or.
     .    dabs(r(1,2)-(sa*cb*cg+ca*sg)) > fuzz*10 .or.
     .    dabs(r(1,3)-(-sb*cg)) > fuzz*10 .or.
     .    dabs(r(2,1)-(-ca*cb*sg-sa*cg)) > fuzz*10 .or.
     .    dabs(r(2,2)-(-sa*cb*sg+ca*cg)) > fuzz*10 .or.
     .    dabs(r(2,3)-(sb*sg)) > fuzz*10 .or.
     .    dabs(r(3,1)-(ca*sb)) > fuzz*10 .or.
     .    dabs(r(3,2)-(sa*sb)) > fuzz*10 .or.
     .    dabs(r(3,3)-cb) > fuzz*10) then
...
C        fi = dasin(ay/st)
C      endif
C      if (x > 0d0 .and. y < 0d0) fi = 2d0*pi - fi
C      if (x < 0d0 .and. y > 0d0) fi =      pi - fi
C      if (x < 0d0 .and. y < 0d0) fi =      pi + fi
C      be = 2d0*dasin(st*dsin(om2))
C      if (dabs(om/pi-0.5d0) < 0.001d0) then
C        if (ct /= 0d0) then
...
CC- Test rm2eua
C
C      double precision rot(3,3),r(3,3),alpha,beta,gamma
C
C      rot(1,1) =  0.908248d0
C      rot(1,2) = -0.091752d0
C      rot(1,3) = -0.408248871390969d0
C      rot(2,1) = -0.091752d0
C      rot(2,2) =  0.908248d0
C      rot(2,3) = -0.408248871390969d0
C      rot(3,1) =  0.408248871390969d0
C      rot(3,2) =  0.408248871390969d0
C      rot(3,3) =  0.816496d0
C      call rm2eua(rot,alpha,beta,gamma)
C
C      print *, alpha,beta,gamma
C
C      call eua2rm(alpha,beta,gamma,r)
C      print *
C      print *, rot-r
C
C      end
