...
c     lsequ = .true.
c     do  10  i = 1, len
c       lsequ = (lsequ .and. s1(i) == s2(i))
c       if (s1(i) == term) return
c  10 continue
      lsequ = .false.
      do  10  i = 1, len
        if (s1(i) /= s2(i)) return
...
C- Parses string for first nonblank character
      ...
      character(len=1), intent(in) :: t(0:nt-1)
      do while (i < nt)
        if (t(i) /= ' ') exit
        i = i + 1
      end do
...
C- Parses string for first blank character
      ...
      integer, intent(in) :: nt
      integer, intent(inout) :: i
      character(len=1), intent(in) :: t(0:nt)
      do while (i < nt)
        if (t(i) == ' ') exit
        i = i + 1
      end do
