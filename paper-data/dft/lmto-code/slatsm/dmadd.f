...
CML      double precision a(0:1), b(0:1), c(0:1), scalea, scaleb
      double precision a(0:nra*nca-1), b(0:nrb*ncb-1), c(0:nrc*ncc-1)
      double precision scalea, scaleb
...
C --- Case unit stride length for each column ---
   ...
   20     continue
          return
        elseif (scalea == 1 .and. scaleb == 1) then
          do  30  j = 0, m-1
            ia = nca * j
            ib = ncb * j
            ic = ncc * j
