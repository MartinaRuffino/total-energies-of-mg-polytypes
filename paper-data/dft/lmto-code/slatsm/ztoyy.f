...
C ----------------------------------------------------------------------
      ...
      integer lbak,kcplxl
C     real(8) ,allocatable :: wkr(:),wkc(:)
      double precision wkr(nr),wkc(nc)

C     allocate(wkr(nr),wkc(nc))
      if (kcplxi == kcplxf) return
      call sanrg(.true.,kcplxi,0,2,'ztoyy','kcplxi')
      call sanrg(.true.,kcplxf,0,2,'ztoyy','kcplxf')
...
C ... Convert kcplx=0 -> kcplx=2
      ...
      if (kcplxl == kcplxf) goto 100

C ... Convert kcplx=1 -> kcplx=2 or kcplx=2 -> kcplx=1
      if (kcplxl /= 0 .and. (kcplxl == 1 .or. kcplxf == 1)) then
        lbak = kcplxl-1
...
C ----------------------------------------------------------------------
      ...
      if (nr > lda) call rx('ztoy2: subblock exceeds lda')
      if (lbak /= 0) goto 20
...
C      if (abs(diff) > 1d-10) lerr = lerr+1
C  333 format(' diff',i2,' ->',i2,' =',f10.2,2x,3f10.2)
C      kcplxi = kcplxf
CC     call cplxdm(kcplxi,lda,ldb,ld1,ld2,ldr,oi)
CC     call ywrm(0,'y',kcplxi+2,i1mach(2),'(6f8.2)',yy,oi,lda,nr,nc)
C
CC ... 2 -> 0
C      kcplxf = 0
C      call ztoyy(yy,lda,ldb,nr,nc,kcplxi,kcplxf)
