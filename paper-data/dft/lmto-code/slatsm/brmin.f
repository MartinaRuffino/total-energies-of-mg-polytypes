...
C --- Limit the shift, depending on dxmx and hmax ---
   ...
   82 diff = max(diff,dabs(w(i,ns)))
      ...
      if (diff > dxmx) then
        factor = dxmx/diff
C#ifdef AWRITE
        call info(20,0,0,' brmin: max shift =%;9,5D'//
     .    ' is larger than dxmx.  Scale by%;8,4D',
     .    diff,factor)
C#elseC
C        print 334, diff,factor
C  334   format(' brmin: max shift =',f9.5,
C     .    ' is larger than dxmx.  Scale by',f8.4)
C#endif
        do  84  i = 1, n
...
C --- Cleanup ---
  ...
  110 diff = max(diff,dabs(w(i,ns)))
     ...
     .  '|g|=%1;4g  max gi=%1;4g  scale H by %1;3g',
     .  iabs(ir),diff,dsqrt(ddot(n,g,1,g,1)),gmax,scale,0,0,0)
      else
        call info8(-30,0,0,' brmin: iter %i  max shift=%1;4g  '//
     .    '|g|=%1;4g  max gi=%1;4g%?#n#  evmin=%1;2g#%j#',iabs(ir),
