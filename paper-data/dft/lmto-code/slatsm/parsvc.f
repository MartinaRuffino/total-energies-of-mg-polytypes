...
C ----------------------------------------------------------------------
      ...
      character*(1) t(0:*)
C Local variables
      double precision tiny,x,y,d
      character rchr*9, sout*50
      integer itrm,awrite,ich,iopt,m,i,iz,id
C     integer a2vec,ix(3)
...
c         parsvc = a2vec(t,ip+100,ip,4,',)',2,3,3,ix,v) == 3
...
C --- Convert vec to t ---
      elseif (mod(iopt,4) == 1 .or. mod(iopt,4) == 2) then
        parsvc = .true.
        do  10  i = 1, 3
        lveq0(i) = dabs(v(i)) < tiny
   10   lveq1(i) = dabs(v(i)-1) < tiny
C   ... Exclude shorthand
