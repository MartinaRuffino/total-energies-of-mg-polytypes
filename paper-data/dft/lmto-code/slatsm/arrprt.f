      subroutine arrprt(hfmt,fmt,casts,ntab,ifi,ncol,nskk,sep,
     .  a1,a2,a3,a4,a5,a6,a7,a8)
C- Write out a table in multi-column format
C ----------------------------------------------------------------------
Ci Inputs
...
Ci   ifi   :if nonzero, file logical unit; if zero use stdout
...
Cu Updates
Cu   05 Dec 14 If ifi is nonzero, write to logical unit ifi instead of stdo
Cu   01 Jun 01 Input simplified: ifi and nskk no longer used.
C ----------------------------------------------------------------------
      implicit none
C ... Passed parameters
      character*(*) hfmt,fmt,casts,sep
      integer ncol,ifi,nskk,ntab
      double precision a1(1),a2(1),a3(1),a4(1),a5(1),a6(1),a7(1),a8(1)
C ... Local parameters
      integer i1,i2,ia,ip,ic,is,ir,lgunit,nrow,awrite,nmemb,jfi
      character*(150) sout,head
      double precision xx(8)
C ... External calls
      external awrit0,vwrt,word

      jfi = lgunit(1); if (ifi /= 0) jfi = ifi
      nrow = (ntab+ncol-1)/ncol
      call word(casts,1,i1,i2)
      nmemb = i2-i1+1
      if (ncol <= 0 .or. nmemb <= 0) return
...
C  ...  this argment is of cast double
   ...
   20 continue
      ...
      if (ir == 1) call awrit0(head,' ',-len(head),jfi)
      call awrit0(sout,' ',-len(sout),jfi)
   10 continue
      end
