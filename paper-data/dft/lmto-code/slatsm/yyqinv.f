...
C#ifdefC DEBUG
C      print 334, 'entering yyqinv: cs=',cs,nlev,n
C  334 format(1x,a,a,': nlev=',i1,'  n=',i4)
C#endif
      m1 = n/2
      m2 = n-m1
...
C#ifdefC DEBUG
C        print *, 'exit yyqinv, LU decomposition'
C#endif
        goto 99
      endif

      if (lda < n) call rx('yyqinv: lda lt n')
      if (ldw < n) call rx('yyqinv: ldw lt n')

C --- R1 = a11^-1 in a11 ---
      if (m1 < nmin .or. nlev == 0) then
        if (cs == 'h') then
...
C#ifdefC DEBUG
C        print *, 'recursive call to yyqinv n,m1,m2=',n,m1,m2
C#endif
C#ifdefC AIX | SUN-ULTRA
C        call yyqinv(cs,ar,ai,lda,nlev-1,(m1),w,ldw,ierr)
C        m1 = n/2
C        m2 = n-m1
C#else
...
C#endif
C#ifdefC AIX | SUN-ULTRA
C        call yyqinv(cs,ar(1+m1,1+m1),ai(1+m1,1+m1),lda,
C     .    nlev-1,(m2),w(1+m1,1),ldw,ierr)
C        m1 = n/2
C        m2 = n-m1
C#else
...
C#ifdefC DEBUG
CC     call yprm(.false.,'a^-1',2,6,fmt,ar,lda,n,lda,n)
C      print 333, 'exiting  yyqinv',nlev,n,m1,m2
