...
C ... Special treatments
      if (fn == 0) then
        ir = 1
        call info2(31,0,0,' rfalsi:  fn=0 at xn=%1,8;8g; return ir=%i',xn,ir)
        goto 999
      elseif (ir == 0) then
        ir = -1
        xmin = xn
...
C ... T if root bracketed, with constraint satisfied
      cst1  = iabs(ic) /= 1 .or. (f1-fn)*(x1-xn)*ic > 0
      cst2 =  iabs(ic) /= 1 .or. (f2-fn)*(x2-xn)*ic > 0
C ... if only 1 cst satisfied, x0 must be in the middle
      ...
      root1 = fn*f1 < 0 .and. cst1
      root2 = fn*f2 < 0 .and. cst2
...
C ... T if local extremum bracketed
      lextr = (f0-f1)*(x0-x1)*(f1-f2)*(x1-x2) < 0 .and. fpp*f1 >= 0
...
C          print *, '1a',xn-x0, xn-x1, (xn-x0)*(xn-x1) > 0
C         pause
          ir = -2
...
C         print *, '2a',xn-x1, xn-x0, (xn-x0)*(xn-x1) > 0
          ir = -2
        endif
C       call dswap(1,x0,1,x1,1)
...
C      if (ir == -2 .or. ir == -3) then
C        if ((xn-x0)*(xn-x1) > 0) then
...
C      do  j = 1, 10
C        ir = irj(j)
CC       if (j > 1) print *, 'j',j,xn-xnj(j),xn < wk(1),xn < wk(2)
C        if (ir /= 0) print *, xn,xn-xnj(j)
C        xn = xnj(j)
C        fn = fnj(j)
