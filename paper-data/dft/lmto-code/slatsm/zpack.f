...
C ... Local parameters
      ...
      fac = 0
      if (mode == 1) fac = 1
      if (modes == 0 .and. moded == 0) then
        nr = nr2-nr1+1
        nc = nc2-nc1+1
        call zmscop(mode,nr,nc,lds,ldd,0,0,0,0,src(nr1,nc1),
     .              dest(nr1,nc1))
      elseif (modes == 0 .and. moded == 1) then
        if (ldd < 2*kl+ku+1) call xerbla('ZPACK ',11)
        ofc = kl+ku+1
        do  10  j = nc1, nc2
        do  10  i = max(nr1,j-ku), min(nr2,j+kl)
   10   dest(i+ofc-j,j) = fac*dest(i+ofc-j,j) + src(i,j)
      elseif (modes == 1 .and. moded == 0) then
        if (lds < 2*kl+ku+1) call xerbla('ZPACK ',10)
        ofc = kl+ku+1
        do  20  j = nc1, nc2
        do  20  i = max(nr1,j-ku), min(nr2,j+kl)
   20   dest(i,j) = fac*dest(i,j) + src(i+ofc-j,j)
      elseif (modes == 1 .and. moded == 1) then
        if (lds < 2*kl+ku+1) call xerbla('ZPACK ',11)
        if (ldd < 2*kl+ku+1) call xerbla('ZPACK ',10)
        ofc = kl+ku+1
        do  30  j = nc1, nc2
