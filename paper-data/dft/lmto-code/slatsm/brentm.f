...
C Passed Parameters
      ...
      iter = iter+1
      if (ir == -1) x = a
      if (iprint() >= 30) print 333, (ir /= -1),a,fx
...
C Extra checks to keep extrapolated point within limits
          if (u >= xhigh) then
            if (ax == xhigh .or. bx == xhigh .or. cx == xhigh)then
              ir = -2
              if (ax == xhigh) then
                cx = ax
                fc = fa
          ...
          if (u <= xlow) then
            if (ax == xlow .or. bx == xlow .or. cx == xlow) then
              ir = -2
              if (ax == xlow) then
                cx = ax
                fc = fa
...
C ... Return with error if root not bracketed
      if ((fa-fb)*maxmin < 0 .or. (fc-fb)*maxmin < 0 .or.
     .  (ax-bx)*(bx-cx) < 0) call rx('BRENT: root not bracketed')

      maxit = itmax
      if (maxit <= 0) maxit=99999
...
C --- Start of main loop ---
      do  11  iter = 1, maxit
        ...
        tol2 = 2*tol1
C ...   Quit if done
        if ( (dabs(x-xm) <= (tol2-(b-a)/2)) .or.
     .       (maxmin == 0  .and.  dsign(fa,fa) /= dsign(fa,fx))
     .     ) goto 3
...
c ... Check for the acceptability of a parabolic fit:
          if (dabs(p) >= dabs(q*etemp/2) .or.
     .        p <= q*(a-x) .or. p >= q*(b-x)) goto 1
C ... Fit is ok ...  take a parabolic step
          d = p/q
          u = x+d
          if (u-a < tol2  .or.  b-u < tol2) d = dsign(tol1,xm-x)
          goto 2
        endif
...
C ... Function evaluation for this iteration: quit if ir returns -1
        fu = maxmin*f(u,1,ir)
        if (iprint() >= 30) print 333, (ir /= -1),u,fu/maxmin
