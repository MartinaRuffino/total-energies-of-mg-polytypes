...
C local
     ...
     .                 m2,m4,small,vsmall,large,vlarge,ldfac,t2
      ...
      if (n == 1) then
        ...
        if (ir > 0 .and. (ir <= ipm .or. ipm == 0)) ir = 0
        call togprt
        call popprt
        return
      endif
...
C other initialisation
      ...
      dmin = small
C Brent has ..
C     if (h < 100*t .and. .not.lq2) h = 100*t
      ldt = h
      if (iprint() >= 60) then
...
C ---  Minimise along the first direction ---
     ...
     .         h,t,ldt,eps,nits,nf,nl,tvec,ir)
      if (ir == -1) return
      if (n == 1) goto 30000
      if (s <= 0) call dscal(n,-1d0,v,1)
      if (sf <= 0.9*d(1).or.0.9*sf >= d(1)) call dcopy(n-1,0d0,0,d(2),1)
...
C ...     random step to get off resolution valley
          if (ipr > 30) print *,'praxis: making random step ..'
          do  1  i = 1, n
...
c      if (.not. illc .and. (df < dabs(100*eps*fx))) then
c      if (.not. illc .and. (df < eps)) then
        if (.not. illc .and. (df < dabs(10*eps*fx))) then
          illc = .true.
          if (ipr > 30)
     .      print *, 'praxis: problem seems ill-conditioned ..'
          goto 20000
        endif
        if (k == 2 .and. ipr > 50) then
...
C throw away direction kl and minimise along new "conjugate" direction
          if (ipr > 50) print*,'praxis: shuffling directions ..'
          do  5  i = kl-1, k, -1
...
C --- See if step length exceeds half the tolerance ---
        ...
        if (kt > ktm) goto 30000
C --- Check ipm switch ---
        ipl = ipl + 1
        if (ipm > 0 .and. ipl >= ipm) then
          if (ipr >= 30) print*,
...
C --- Singular value decomposition of v - Golub & Reinsch (Eispack) ---
   ...
   14 continue
      ...
      if (ierr /= 0) call fexit(-1,1,'minfit in praxis: ierr=%d',ierr)
      if (scbd > 1) then
C unscaling
        do  15  i = 1, n
...
C estimate 1st derivative at x
      ...
      dz = .true.
C predict minimum ..
C here, Brent has if (d2 <= small) then
      if (d2 <= small*dabs(d1)) then
        if (d1 < 0) then
          x2 = h
        else
          x2 = -h
...
C no success so try again ..
          k = k + 1
          if ((f0 < f1) .and. (x1*x2 > 0)) goto 10000
          x2 = 0.5d0*x2
          goto 20000
        endif
        if (iprint() > 50) write (*,300) f2,f2q
      endif
      if (lq) f2 = f2q
...
C Update x from linear search
      if (j > 0) call daxpy(n,x1,v(1,j),1,x,1)
...
C --- Initialise ---
    ...
    1 continue
      ...
      if (qd0 > 0 .and. qd1 > 0 .and. nl >= 3*n*n) then
        if (iprint() > 30) print*,'quad: trying extrapolation ... '
