...
Ci                      transformation to:
Ci                      x=-x2*log(y),  dx=-x2/y dy,  y=exp(-x/x2)
Ci                   2, Quadrature for int_x1^infty.  Internal
Ci                      transformation to:
Ci                      x=a+\frac{y}{1-y}, dx=\frac{1}{(1-y)^2}dy
Ci                      y=\frac{x-a}{1+(x-a)}.
Ci   ipr: verbosity
Co Outputs
Co   xp,wp
Cu Updates
Cu   10 Aug 11 New mode 20
C ----------------------------------------------------------------------
      implicit none
...
C --- 10s digit of mode = 1 => shift points y to x ---
      if (mod(mode,100)/10 == 1) then
        do  i = 1, n
          ...
          wp(i) = x2*wp(i)/y
        enddo
C --- 10s digit of mode = 2 => shift points y to x ---
      elseif (mod(mode,100)/10 == 2) then
        do  i = 1, n
          y = xp(i)
          xp(i) = x1 + y/(1-y)
          wp(i) = wp(i)/(1-y)**2
...
C- Calculates derivative of Legendre polynomial from recursion relation
C ----------------------------------------------------------------------
Ci Inputs
Ci   n,x
Co Outputs
