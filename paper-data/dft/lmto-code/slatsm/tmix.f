...
C ... Normal equations Make < (d_j d_k) > (+ last column for const)
   10 continue
      t(0) = 1
      if (nmix == 0) goto 40
      if (nmix < 0) call rx('tmix: bad nmix')
...
C --- Printout ---
      ...
      call bin2a('',0,0,nmix,2,0,80,s,is)
      call bin2a('nelts=',2,0,0,1,0,80,s,is)
      call bin2a('',0,0,nelts,2,0,80,s,is)
      ...
      print 147, s(1:is)
  147 format(1x,a)
      if (nmix > 0) print 134, (t(j), j=0,nmix)
  134 format(3x,'tj:',7(f9.5,1x))
      if (ipr < 61 .and. (ipr < 41 .or. nelts > 100)) goto 60
      write(*,110)
      do  12  i = 1, nelts
        if (dabs(wk(i,0,1)) + dabs(wk(i,mmix+1,2)-wk(i,0,2)) >= 5.d-7)
