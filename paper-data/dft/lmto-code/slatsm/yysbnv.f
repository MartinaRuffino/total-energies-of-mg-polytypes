...
C#endif
      ...
      if (nn1 > nn2) return
      cs = ' '
      if (mod(mode,10) == 3) cs = 'h'
      mode0 = mod(mode,10)
      mode1 = mod(mode/10,10)
      call sanrg(.true.,mode1,0,1,'yysbnv:','10s digit mode')
      modl = 10*mod(mode0,3)
...
C#ifdefC DEBUG
C      print 334, 'entering yysbnv ',mode,nlev,nn1,nm1,nn2,m1-n1,m2
C  334 format(1x,a,': mode=',i2,': nlev=',i1,
C     .       '  nn1=',i2,'  nm1=',i2,'  nn2=',i2,
C     .       '  m1=',i3,'  m2=',i3)
C#endif
...
C#ifdefC DEBUG
C        print *, 'exit yysbnv, used standard LU decomposition'
C#endif
        return
      endif

      if (lda < n2) call rx('yysbnv: lda lt n2')
...
C --- R1 = a22^-1 in a22 ---
   ...
   12     ai(i,j) = -ai(j,i)
     ...
     .      w(1,2),w(1,3),1)
      ...
      elseif (nm2 > nnmin .and. nlev > 0) then
        call yysbnv(mode,apr,api,ldap,ija,offs,nlev-1,
     .    nm1+1,(nn2),w,ldw,ar,ai,lda,ierr)
C#ifdefC AIX | SUN-ULTRA
C        mode1 = mod(mode/10,10)
C        call psybnv(mode1,nn1,nn2,ija,offs,nmin,nm1,nm2,n1,n2,m1,m2,nd,
C     .    nnmin)
C#endif
C#ifdefC DEBUG
C        print 333, 'recursive call exit',nlev,n2-n1,m1-n1,m2
C#endif
      else
        call yysp2a(mode0,1+nm1,nn2,1+nm1,nn2,apr,api,ldap,ija,offs,
     .    ar,ai,lda)
...
C#ifdefC DEBUG
CC     call yprm(.false.,'a^-1',2,6,fmt,ar(1+n1,1+n1),lda,nd,lda,nd)
C      print 333, 'exiting  yysbnv',nlev,n2-n1,m1-n1,m2
C  333 format(1x,a,': nlev=',i1,'  ndim=',i3,' partitioned into',2i3)
C#endif
      end
...
C --- Partition into approximately equal dimensions ---
   ...
   32   continue
        if (iabs(offs(nm1)-nd) > iabs(offs(nm1-1)-nd)) then
          nm1 = nm1-1
          if (nm1 > 1) goto 32
        endif
        nm1 = nm1-1
