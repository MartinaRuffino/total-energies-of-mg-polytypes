...
C --- Debugging: check for identical roots ---
      do  30  in = 1, n-1
   30 if (dabs(z(in)-z(in+1)) < smachp) stop 'mklagw: identical roots'
...
C#ifdefC TEST
!      subroutine test
CC shows z and weights integrate x**i from i=0..2n-1 exactly
!      implicit none
...
C
C
CC ...  Do integral from 0 to z0 using mlegw
C        r1 = (z0-0)/2
C        r2 = (z0+0)/2
C        resg = 0
C        do  40  j = 1, nn
...
C        if (i == 2*n-1) print *, ' ------ exact ends here ---- '
C   10 continue
C      end
C#endif
