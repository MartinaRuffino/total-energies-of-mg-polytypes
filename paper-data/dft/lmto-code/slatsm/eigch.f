...
C- Eigenvalues of Hermitian matrix -- partial simulation of IMSL
C ----------------------------------------------------------------
Ci Inputs:
Ci   a,n:  hermitian matrix, declared as a(n,*)
...
C --- job numbers to match IMSL ---
      ijob = mod(jobn,10)
      if (jobn-ijob /= 10 .or. ijob > 1) stop 'eigch: bad jobn'

      if (n == 1 .and. ijob == 0) goto 35
      mdim = 2 * n
      if (ijob == 0) goto 5
      if (n > iz) stop 'eigch: ijob /= 0  and  n > iz'
      if (n == 1) goto 35

C --- Rearrange a if necessary when n > iz and ijob /= 0 ---
      mdim = min0(mdim,2*iz)
      if (n <= iz) goto 5
      l = n - 1
      do  4  j = 1, l
