...
C ... Check links, come back here (84); skip if lfast on
      ...
      goto 83
   84 ipname = ipfree
      if (lerr .and. ipfree+length+2 > limit) then
        ipname = -LENGTH
        if (jpr > 0)  print *,
...
C ... Save as one of the links
      ...
      ncall = ncall + 1
      if (jpr > 0) write(*,100) ndefd,leng,length,ipname,ipfree-1
  100 format(' define array',I4,':   els=',i9,'   length=',i9,',',
     .   i9,'  to',i9)
C ... Check to ensure array not too big
      if (ipfree <= limit) then
        if (iopt /= 0) goto (201,202,203,204,205) jopt
        return
...
C   ... Exit if links messed up
        if (ipx < qqq .or. ipx > limit) goto 83
        if (ipx == ipfree) goto 66
        do  63  j = 4, 1, -1
   ...
   66 continue
      ippplc(-1) = ipfree-1
      ipppln(-1) = ipfree-1
      if (ndefd < nstk+1) call rx('defsw: not enough arrays allocated')
C ... Copy bottom array to temp above the top array
      if (ipfree+size(nstk) > limit)
...
C ... Set i0,i1...i3 to elements 0..3
      i0 = ipppln(0)+1
      i1 = ipppln(1)+1
      if (nstk >= 2) i2 = ipppln(2)+1
      if (nstk >= 3) i3 = ipppln(3)+1
      return

C --- Release data up to pointer ---
      entry rlse(ipname)
      if (ipname > limit) call rx('RLSE: pointer exceeds limit')
      if (ipname < 3) call rx('RLSE: pointer less than 3')
      if (jpr == 0) goto 82
...
C --- Return number of words left in common pool ---
      ...
      if (jpr > 0) print 382, lrest
  382 format(' DEFASK:  space left=',i9,'  words')
      return
      entry wkused(lused)
      lused = ipfree
      return
...
C ... Run through links, come back here (81)
      ...
      goto 83
  81  continue
      if (imod == 6 .and. irnd(ncall) > 0 .or.
     .    imod < 5 .and. ncall > 0) then
      write(lgunit(1),600) irnd(ipmax-1),irnd(limit),irnd(ncall)
      if (lgunit(2) > 0)
     .write(lgunit(2),600) irnd(ipmax-1),irnd(limit),irnd(ncall)
  600 format(' wkinfo:  used',i6,' K  workspace of',i6,
     .  ' K   in',i4,' K calls')
      endif
      if (ipfree == qqq) return
      if (jpr > 0) write(*,602)
  602 FORMAT(/'    array',5X,'begin',7X,'end',6X,'length')
      ipx = qqq
      do  30  i = 1, ndefmx
        ipy = w(ipx-1)
        if (ipx == ipfree) return
        if (ipy < qqq .or. ipy > limit) write(*,*) '   . . . . . '
        if (ipy < qqq .or. ipy > limit) return
        if (jpr > 0) write(*,603) i,ipx,ipy,ipy-ipx
  603   format(4(i9,3x))
        ipx = ipy
   30 continue
      return
