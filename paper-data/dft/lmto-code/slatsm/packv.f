...
C     parameter (nsmx=2048)
C     double precision xx(nsmx)
...
C           if (nel > nsmx) call rxi('packv: increase nsmx to',nel)
            allocate(xx(nel))
...
C ... If nelti is not one, copy elements one spec at a time
        ...
        if (pack) then
          ...
          do  10  is = is1, is2
            off = offi + ssize*(is-1)
            if (mod(lpack/10,10) == 1) offx = 1 + (is-is1)*nelti
            if (casti == 4) then
