      recursive subroutine dqinvb(cs,a,lda,nlev,n,nb,w,ldw,w2,b,ldb,ierr)
...
Cc     character*10 fmt
CC     data fmt /'(8f16.10)'/
CC     call ywrm(0,'a',1,6,fmt,a,1,lda,n,n)
C      print 334, 'entering dqinvb',cs,nlev,n,(n+1)/2,n-(n+1)/2
C  334 format(1x,a,': cs=',a,': nlev=',i1,
C     .       '  n=',i3,'  m1=',i3,'  m2=',i3)
C#endif
      ...
      do  2  i = 1, j
        if (cs(i:i) == 't') then
          ltrns = .true.
        elseif (cs(i:i) == 'b') then
          lsx = .true.
        elseif (cs(i:i) == 's') then
          css = 's'
        elseif (cs(i:i) == 'r') then
          lrecur = .true.
        elseif (cs(i:i) /= ' ') then
...
C --- For n<nmin, do directly by dgefa,dgesl ---
      if (n < nmin) then
        if (.not. lsx) then
          if (css == 's') call dsifa(a,lda,n,w,ierr)
          if (css /= 's') call dgefa(a,lda,n,w,ierr)
          if (ierr /= 0) return
        endif
        do  12  j = 1, nb
          if (.not. ltrns .and. css == 's') then
            call dsisl(a,lda,n,w,b(1,j))
          elseif (.not. ltrns .and. css /= 's') then
...
C       ... Do for special case n<=2: w may not be large enough!
            if (n <= 2) then
              do  116  i = 1, n
              ww(i) = b(j,i)
              if (css == 's') call dsisl(a,lda,n,w,ww)
              if (css /= 's') call dgesl(a,lda,n,w,ww,1)
  116         continue
              do  118  i = 1, n
  118         b(j,i) = ww(i)
            else
              do  16  i = 1, n
   16         w(i,2) = b(j,i)
              if (css == 's') call dsisl(a,lda,n,w,w(1,2))
              if (css /= 's') call dgesl(a,lda,n,w,w(1,2),1)
              do  18  i = 1, n
   18         b(j,i) = w(i,2)
            endif
          endif
   12   continue
      ...
      if (ldw < n) call rx('dqinvb: ldw lt n')
      m1 = (n+1)/2
      m2 = n-m1
...
C --- c11 = (a11 - a12 a22^-1 a21)^-1 in a11 ---
      if (.not. (lrecur .and. nlev > 0 .and. m1 >= nmin))
...
C --- Obtain x1 = c11 (b1 - a12 a22^-1 b2) ---
      if (lrecur .and. nlev > 0 .and. m1 >= nmin) then
C   ... (b1 - a12 a22^-1 b2)
...
C       AIX and gcc require this
        ...
        m2 = n-m1
C#ifdefC DEBUG
CC       print 333, 'recursive call exit',nlev,n,m1,m2
C#endif
      else
...
C#ifdefC DEBUG
C      print 333, 'exiting dqinvb ',nlev,n,m1,m2
C  333 format(1x,a,': nlev=',i1,'  ndim=',i4,' partitioned into',2i4)
C#endif
