...
C  ---------------------------------------------------
      implicit none
      integer neq,mx,ipr,ir,nmpseq(0:*)
...
C     ... Evaluate next diagonal in tableau
          do  214  j = 1, neq
            ...
            wk(nv3+j) = yy
            do  213  k = 2, m
              if (v > 1d10) call rx('dbsstp: bad work array')
              nseq1 = nseq0(imp-k+1)
              if (nmpseq(0) /= 0) nseq1 = nmpseq(imp-k+1)
...
C   ... Normal exit when this step is completed
        if (errmax < abs(tol)) then
          ir = 0
C     ... Tune next h to make last imp come as close to mx as possible
          if (imp == mx) then
            hfac = hshrnk
          elseif (imp == mx-1) then
            hfac = hgrow
          else
            nseq1 = nseq0(mx-1)
            if (nmpseq(0) /= 0) nseq1 = nmpseq(mx-1)
