...
C ... Passed parameters
      ...
      logical lrewnd
      character*(*) cat,descr
C#ifdefC ANSI | FIXEDLENGTH
C      integer recl0
C      parameter (recl0=128)
C      character*(recl0) instr(mxrec)
C#else
...
C#ifdefC ANSI | FIXEDLENGTH
C      if (recln /= recl0) call
C     .  rxi('fgtcat: recompile with recl0=',recln)
C#endif
      lio   = quswt('io',swt)
...
C --- Show what category would have been sought ---
     ...
     .    '%a %?#(n==0)#(optional)#(required)#'
      ...
      elseif (lio /= 1 .and. lio /= 3) then
        call rxi('fgtcat : not implemented, mode',lio)

      endif

C --- Find category and read it in ---
      call sanrg(.true.,recln,1,recl,'fgtcat:','recln')
      if (mxrec <= 0) call rxi('fgtcat: illegal mxrec',mxrec)
...
C ... Parse for new word
   10 continue
      ...
      if (j2 < j1) return
      j10 = j1
      if (instr(j1:j1+1) == 'io') then
   20   continue
        call nwordg(instr,1,', ',2,j1,j3)
        if (j3 >= j1) then
          if (instr(j1:j3) == 'read') then
            swt(5) = 1
          elseif (instr(j1:j3) == 'write') then
            swt(5) = 2
          elseif (instr(j1:j3) == 'rw') then
            swt(5) = 3
          elseif (instr(j1:j3) == 'help') then
            swt(5) = 0
          else
            goto 99
          endif
          if (j3 < len(instr)) then
            if (instr(j3+1:j3+1) == ',') then
              j1 = j3+1
              goto 20
            endif
          endif
      ...
      elseif (instr(j1:j1+4) == 'token' .or.
     .        instr(j1:j1+2) == 'cat') then
        k = 3
C        if (instr(j1:j1+2) == 'cat') k = 4
        i0 = mod(swt(k),4)
        i2 = mod(swt(k)/4,2)
        i3 = mod(swt(k)/8,2)
        i4 = mod(swt(k)/16,2)
   30   continue
        call nwordg(instr,1,', ',2,j1,j3)
        if (j3 >= j1) then
          if (instr(j1:j3) == 'opt') then
            i0 = 0
          elseif (instr(j1:j3) == 'o') then
            i0 = 0
          elseif (instr(j1:j3) == 'reqd') then
            i0 = 1
          elseif (instr(j1:j3) == 'r') then
            i0 = 1
          elseif (instr(j1:j3) == 'alt') then
            i0 = 2
          elseif (instr(j1:j3) == 'a') then
            i0 = 2
          elseif (instr(j1:j3) == 'ignr') then
            i0 = 3
          elseif (instr(j1:j3) == 'i') then
            i0 = 3
          elseif (instr(j1:j3) == 'varlen') then
            i2 = 0
          elseif (instr(j1:j3) == 'fixlen') then
            i2 = 1
          elseif (instr(j1:j3) == 'mxrec') then
            i2 = 1
          elseif (instr(j1:j3) == 'nomult') then
            i4 = 0
          elseif (instr(j1:j3) == 'mult') then
            i4 = 1
          else
            goto 99
          endif
      ...
      elseif (instr(j1:j1+4) == 'lunit') then
        swt(6) = n

      elseif (instr(j1:j1+3) == 'init') then
        call iinit(swt,7)

      elseif (instr(j1:j1+4) == 'start') then
        swt(1) = n

      elseif (instr(j1:j1+3) == 'stop') then
        swt(2) = n
...
C ... Local parameters
      ...
      if (instr(j1:j1+1) == 'io') then
        ...
        if (j3 >= j1) then
          quswt = 0
          if (instr(j1:j3) == 'read' .or. instr(j1:j3) == 'r') then
            if (swt(5) == 1 .or. swt(5) == 3) quswt = 1
          elseif (instr(j1:j3) == 'write' .or. instr(j1:j3) == 'w') then
            if (swt(5) == 2 .or. swt(5) == 3) quswt = 1
          elseif (instr(j1:j3) == 'help') then
            if (swt(5) == 0) quswt = 1
          else
            goto 99
...
C       if (instr(j1:j1+2) == 'cat') k = 4
        ...
        if (j3 >= j1) then
          if (.false.) then
            continue
          elseif (instr(j1:j3) == 'opt' .or. instr(j1:j3) == 'o') then
            quswt = i0
          elseif (instr(j1:j3) == 'reqd' .or. instr(j1:j3) == 'r') then
            if (i0 == 1) quswt = 1
          elseif (instr(j1:j3) == 'alt' .or. instr(j1:j3) == 'a') then
            if (i0 == 2) quswt = 1
          elseif (instr(j1:j3) == 'ignr' .or. instr(j1:j3) == 'i') then
            if (i0 == 3) quswt = 1
          elseif (instr(j1:j3) == 'fixlen') then
            if (i2 == 1) quswt = 1
          elseif (instr(j1:j3) == 'mxrec') then
