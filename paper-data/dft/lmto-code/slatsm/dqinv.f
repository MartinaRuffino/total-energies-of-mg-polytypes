...
C ----------------------------------------------------------------
      ...
      double precision a(lda,*),w(ldw,n)
...
C#ifdefC DEBUG
C      print *, 'entering dqinv cs=',cs,' nlev,n=',nlev,n
C#endif
      m1 = n/2
      m2 = n-m1
...
C#ifdefC DEBUG
C        print *, 'exit dqinv, no Strassen'
C#endif
        return
      endif

      if (lda < n) call rx('dqinv: lda lt n')
...
C#ifdefC DEBUG
C        print *, 'recursive call to dqinv n,m1,m2=',n,m1,m2
C#endif
C#ifdefC AIX | SUN-ULTRA
C        call dqinv(cs,a,lda,nlev-1,(m1),w,ldw,ierr)
C        m1 = n/2
C        m2 = n-m1
C#else
...
C#endif
C#ifdefC DEBUG
C        print *, 'recursive call exit dqinv n,m1,m2=',n,m1,m2
C#endif
        if (ierr /= 0) return
      endif
...
C#endif
C#ifdefC AIX | SUN-ULTRA
C        call dqinv(cs,a(1+m1,1+m1),lda,nlev-1,(m2),w(1+m1,1),ldw,ierr)
C        m1 = n/2
C        m2 = n-m1
C#else
...
C#ifdefC DEBUG
C      call yprm(.false.,'a^-1',1,6,fmt,ar,lda,n,lda,n)
C      print *, 'exiting dqinv',n,m1,m2
C#endif
