...
C ... hang onto dh for 2nd order perturbation correction
      if (mod(iopt/100,10) == 1) then
        allocate(wk2(l2))
        call dcopy(l2*2,dh,1,wk2,1)
      endif
...
C ... Restore w to dh; save z0 in w; add pert. correction to z
      if (mod(iopt/100,10) == 1) then
        call dcopy(l2*2,wk2,1,dh,1)
        call dcopy(l2*2,z,1,wk2,1)
      endif
      call daxpy(l2*2,1d0,wk,1,z,1)
C     call yprm('new z',2,z,ld*ld,ld,ld,ld)

C ... Orthogonalize and/or normalize z
      if (mod(iopt/10,10) > 1) then
        do  50  j = 1, ld
          if (mod(iopt/10,10) > 2) then
          do  55  i = 1, j-1
...
Cc        call pvpert(iopt,ld,e0,dh,ds,wk2,z,wk,de)
C        call rlse(owk2)
        deallocate(wk2)
      endif
      end
