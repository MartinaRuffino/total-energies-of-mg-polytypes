      subroutine potpus(z,rmax,lmxa,v,vdif,grv1,a,nr,nsp,nspc,lso,loptic,nrg,nlg,
     .  facso,rofi,pnu,pnz,ehl,rsml,rs3,vmtz,nab,n0,pp,ppnl,hab,vab,sab,
     .  sodb,rgrad,buf)
...
Ci         :and integrals of <u|1|u> etc (for constant external B field)
Ci         :10s digit nonzero: calculate the SO weighted average of the electric field
...
Ci   grv1  :(need if 10s digit lso nonzero) the gradient of the potential v1
...
Co         :These apply to sodb(:,:,:,:,1)
Co         :Alternatively, SO = 1 (for constant external Bfield)
Co         :These apply to sodb(:,:,:,:,2)
Co         :sodb(1,l+1,isp,:,:)=<u|SO|u>
Co         :sodb(2,l+1,isp,:,:)=<u|SO|s>
Co         :sodb(3,l+1,isp,:,:)=<s|SO|u>
Co         :sodb(4,l+1,isp,:,:)=<s|SO|s>
Co         :sodb(5,l+1,isp,:,:)=<u|SO|z>
Co         :sodb(6,l+1,isp,:,:)=<s|SO|z>
Co         :sodb(7,l+1,isp,:,:)=<z|SO|z>
Co         :sodb(8,l+1,isp,:,:)=<z|SO|u>
Co         :sodb(9,l+1,isp,:,:)=<z|SO|s>
Co         :sodb(:,:,1:2,1,1) = spin diagonal parts of SO coupling
Co         :sodb(:,:,1:2,2,1) = spin off-diagonal parts of SO coupling
Co         :sodb(:,:,1:2,1,2) = spin diagonal parts of v(r)=1
Co         :sodb(:,:,1:2,2,2) = off-diagonal parts of v(r)=1
Co         :These latter 2 may be used for constant external B field
Co         :See Remarks for more info on structure of sodb.
Co  rgrad  :radial matrix elements of gradient operator (phi/phid repres.)
Cl Local variables
...
Cr   sodb(:,:,1:2,1,1) holds spin diagonal parts of SO coupling
Cr   sodb(:,:,1:2,2,1) holds spin off-diagonal parts of SO coupling
Cr   sodb(:,:,1:2,1,2) holds spin diagonal parts of constant potential
Cr                     used for constant external B field
Cr   sodb(:,:,1:2,2,2) holds spin off-diagonal parts of constant potential
Cr                     used for constant external B field
Cu Updates
Cu   24 Aug 15 (Scott McKecknie) SO weighted average of electric field
Cu   22 Jun 14 generate noncollinear optical matrix elements.  New arg list
...
Cu   30 Mar 10 Make sodb(:,:,:,:,2) for external B field
...
C ----------------------------------------------------------------------
      implicit none
C ... Passed parameters
      integer lmxa,lso,loptic,nr,nsp,nspc,n0,nab,nppn,nrg,nlg
      parameter (nppn=12)
      double precision z,rmax,a,facso,rofi(*),v(nr,nsp),ehl(n0),
     .  rsml(n0),pnu(n0,nsp),pnz(n0,nsp),pp(n0,2,5),ppnl(nppn,n0,2),
     .  hab(nab,n0,nsp),sab(nab,n0,nsp),vab(nab,n0,nsp),vdif(nr,nsp),
     .  sodb(nab,n0,nsp,2,2),rs3,vmtz,grv1(nr,3)
      integer, parameter :: nrx=1501, nkap0=4
      real(8) rgrad(nrg,nrg,2,nlg,nsp*nspc)
      character*(*) buf
      double precision rme(1) ! To be calculated in future
C ... Local parameters
      integer stdo,ipr,ir,i,j,k,l,lpz,lpzi(0:n0),nrbig,lorthz,lrel,lwronsk,nphi
...
C     Spin-Orbit related parameters
      double precision vavg(nr),dv(nr),sop(0:lmxa,nsp,nsp,6),
     .                 sopz(0:lmxa,nsp,nsp,6),
     .                 psi(nr,0:lmxa,nsp,nkap0), !dpsi(nr,0:lmxa,nsp),
     .                 pzi(nr,0:lmxa,nsp),ezum(0:8,nsp),
     .                 enum(0:8,nsp),m(2,2,0:lmxa,nsp),
     .                 x21,x11,x22,x12,vx00,vx01,vx10,vx11,
     .                 vx0z,vxz0,vx1z,vxz1,vxzz,vxzu,vxuz,vxzs,vxsz
      procedure(integer) :: awrite,nglob
      common /cc/ cc
...
C      call prrmsh('vbig',rofi,vbig,nrx,nrbig,nsp)
      ...
      if (lso /= 0 .or. mod(loptic,10) > 0) call dpzero(psi,size(psi))
      nphi = 2
...
C --- Loop over spins, lmxa ---
      do  i = 1, nsp
        ...
        if (lso /= 0) then
          call dpzero(sodb(1,1,i,1,1),nab*n0)
          call dpzero(sodb(1,1,i,2,1),nab*n0)
          call dpzero(sodb(1,1,i,1,2),nab*n0)
          call dpzero(sodb(1,1,i,2,2),nab*n0)
        endif

      if (ipr >= 40) then
        call awrit4('%N potpus %?#n==2# spin %i #%j#: pnu =%n:1,6d',buf,
     .    80,stdo,nsp,i,lmxa+1,pnu(1,i))
        if (lpz /= 0) then
          ir = 10; if (nsp == 2) ir = 18
          call awrit3('%nfpz  =%n:1,6d',buf,80,stdo,ir,lmxa+1,pnz(1,i))
        endif
C        write(stdo,698)
C  698   format(' l',8x,'enu',9x,'v',11x,'c',10x,'srdel',
C     .    8x,'qpar',8x,'ppar')
        call awrit0(' l%8fenu%9fv%11fc%10fsrdel%8fqpar%8fppar',buf,80,stdo)
      endif
...
C --- Loop over l ---
      ...
      k = l+1
      lpzi(l) = 0
      if (mod(pnz(k,i),100d0) > 0)   lpzi(l) = 1
      if (mod(pnz(k,i),100d0) >= 10) lpzi(l) = 2
      if (mod(pnz(k,i),100d0) >= 20) lpzi(l) = 3
...
C       Case local orbital deeper than valence
        ...
        call makrwf(1000+j,z,rmax,l,v(1,i),a,nr,rofi,pnz(k,i),-0.50d0,4,
...
C ... Valence w.f. g,gp, and their sphere boundary parameters
      call makrwf(10*lwronsk,z,rmax,l,v(1,i),a,nr,rofi,pnu(l+1,i),0d0,2,g,gp,
     .  ev,phi,dphi,phip,dphip,p)
C     call prrmsh('v+2Z/r',rofi,v,nr,nr,nsp)

C      if (l == 1) then
C        call makrwf(10*lwronsk,z,rmax,l,v(1,i),a,nr,rofi,pnu(l+1,i),-0.5d0,2,g,gp,
C     .  ev,phi,dphi,phip,dphip,p)
C      endif
...
C      if (lpzi(l) > 1 .and. lorthz /= 0) then
...
C ... Keep local copies of phi and phidot for SO coupling
      if (lso /= 0 .or. mod(loptic,10) > 0) then
        call dcopy(nr,g,1,psi(1,l,i,1),1)
        call dcopy(nr,gp,1,psi(1,l,i,2),1)
!!?? Why not lpzi = 1 ??
        if (lpzi(l) > 1) then
          nphi = 3
          call dcopy(nr,gz,1,psi(1,l,i,3),1)
        endif
        enum(l,i) = ev
      endif
...
C      if (lpzi(l) /= 0) then
C
CCC       debugging ... check that mode 0 works properly when g and gp
CCC       are not orthogonal.  The next line
CC        call ortrwf(10,z,l,v(1,i),nr,nr,nr,rofi,rwgt,ev,ev,ez,g,gp,gz,D)
CCC       Should produce the same D as if g,gp were orthogonalized
CCC       and then computed, as done in the next two lines
CC        call ortrwf(12,z,l,v(1,i),nr,nr,nr,rofi,rwgt,ev,ev,ez,g,gp,gz,D)
CC        call ortrwf(11,z,l,v(1,i),nr,nr,nr,rofi,rwgt,ev,ev,ez,g,gp,gz,D)
C
...
C        phzp = phzp/D
C        dphzp = dphzp/D
CC       call prrmsh('gz',rofi,gz,nr,nr,2)
C
C      endif
C#endif
...
C      if (ipr >= 40) write(stdo,699) l,'    ',ev,vl,c,srdel,qpar,ppar
C  699 format(i2,a,f10.6,3f12.6,f12.4,f12.6)
C      call info5(40,0,0,'%,2i    %;10,6D%3;12,6D%;12,4D%;12,6D',
C     .  l,ev,[vl,c,srdel],qpar,ppar)
      if (ipr >= 40) call awrit5('%,2i    %;10,6D%3;12,6D%;12,4D%;12,6D',
     .  buf,80,stdo,l,ev,[vl,c,srdel],qpar,ppar)
      pp(k,i,1) = enu
      pp(k,i,2) = c
      pp(k,i,3) = srdel
      pp(k,i,4) = qpar
...
C     ppars not saved; only printed out
      if (lpzi(l) /= 0 .and. ipr >= 40) then
        ...
        qpar = 1d0/q
        ppar = 1d0/dsqrt(pz)
C       write(stdo,699) l,'(sc)',ez,vl,c,srdel,qpar,ppar
        call awrit5('%,2i(sc)%;10,6D%3;12,6D%;12,4D%;12,6D',buf,
     .    80,stdo,l,ez,[vl,c,srdel],qpar,ppar)
...
CC      <g H gp> = <g (H-e) gp> = 1/sqrt(p)
CC      <gp H gp> = <gp (H-e) gp> + e <gp gp> = <gp g> + e = e
C       ghgp = 1/sqrt(p)
C       gphgp = ev
CC      Scaling of gp -> phip, <gp gp> also scaled
C       phip = phip/sqrt(p)
...
C ... Calculate matrix elements of radial grad operator
      if (mod(loptic,10) > 0) then
        call rgrme(0,nrg,nphi,nphi,nsp,nspc,nlg-1,lmxa,nr,rofi,rwgt,psi,psi,rgrad)
      endif

C ... Calculate spin-orbit parameters
      if (lso /= 0) then
        call soprm(5,lpzi,psi(1,0,1,1),psi(1,0,1,2),pzi,nr,nsp,lmxa,
     .    lmxa,v,dv,enum,ezum,z,rofi,rwgt,facso,sop,sopz,buf)

C       SCOTT: Get SO weighted average of electric field
        if (lso >= 10) then
          call avsoef(lpzi,psi,pzi,nr,nsp,lmxa,v,dv,enum,ezum,z,
     .      rofi,rwgt,facso,sop,grv1,buf)
        endif

C       Overwrite dv with 1 to make ME of constant V ... destroys dv!
        call dvset(dv,1,nr,1d0)
        call soprm(6,lpzi,psi(1,0,1,1),psi(1,0,1,2),pzi,nr,nsp,lmxa,
     .   lmxa,v,dv,enum,ezum,z,rofi,rwgt,1d0,sop(0,1,1,4),sopz(0,1,1,4),buf)
...
C   ... Make the spin diagonal radial integrals
        ...
        do l = 0, lmxa
          ...
          m21 = m(2,1,l,i)
          m22 = m(2,2,l,i)
C         j=1 : SO coupling   j=2: constant potential
          do  j = 1, 2
          v00 = sop(l,i,i,1)
          v01 = sop(l,i,i,2)
          v11 = sop(l,i,i,3)
          ...
          v10 = v01
          sodb(1,k,i,1,j) =
     .      m11*v00*m11 + m11*v01*m12 + m12*v10*m11 + m12*v11*m12
          sodb(2,k,i,1,j) =
     .      m11*v00*m21 + m11*v01*m22 + m12*v10*m21 + m12*v11*m22
          sodb(3,k,i,1,j) =
     .      m21*v00*m11 + m21*v01*m12 + m22*v10*m11 + m22*v11*m12
          sodb(4,k,i,1,j) =
     .      m21*v00*m21 + m21*v01*m22 + m22*v10*m21 + m22*v11*m22
...
C   ...  Make the local orbitals spin diagonal integrals
     ...
     .           phz**2*sodb(1,k,i,1,j) +
     .           phz*dphz*(sodb(2,k,i,1,j)+sodb(3,k,i,1,j)) +
     .           dphz**2*sodb(4,k,i,1,j)
           vuz = vuz - phz*sodb(1,k,i,1,j) - dphz*sodb(2,k,i,1,j)
           vsz = vsz - phz*sodb(3,k,i,1,j) - dphz*sodb(4,k,i,1,j)
           vzu = vzu - phz*sodb(1,k,i,1,j) - dphz*sodb(3,k,i,1,j)
           vzs = vzs - phz*sodb(2,k,i,1,j) - dphz*sodb(4,k,i,1,j)
           sodb(5,k,i,1,j) = vuz
           sodb(6,k,i,1,j) = vsz
           sodb(7,k,i,1,j) = vzz
           sodb(8,k,i,1,j) = vzu
        ...
        enddo ! Loop over j (1 for SO, 2 for constant field)
        enddo ! Loop over l
...
C     ... up-down block
          sodb(1,k,1,2,j) =
     .      m11*v00*x11+m11*v01*x12+m12*vx10*x11+m12*v11*x12
          sodb(2,k,1,2,j) =
     .      m11*v00*x21+m11*v01*x22+m12*vx10*x21+m12*v11*x22
          sodb(3,k,1,2,j) =
     .      m21*v00*x11+m21*v01*x12+m22*vx10*x11+m22*v11*x12
          sodb(4,k,1,2,j) =
     .      m21*v00*x21+m21*v01*x22+m22*vx10*x21+m22*v11*x22
C     ... down-up block
          sodb(1,k,2,2,j) =
     .      x11*vx00*m11+x11*v01*m12+x12*vx10*m11+x12*vx11*m12
          sodb(2,k,2,2,j) =
     .      x11*vx00*m21+x11*v01*m22+x12*vx10*m21+x12*vx11*m22
          sodb(3,k,2,2,j) =
     .      x21*vx00*m11+x21*v01*m12+x22*vx10*m11+x22*vx11*m12
          sodb(4,k,2,2,j) =
     .      x21*vx00*m21+x21*v01*m22+x22*vx10*m21+x22*vx11*m22
...
C  ...  Make the local orbitals spin off-diagonal radial integrals
     ...
     .      +  sodb(1,k,1,2,j)*phz*phz2
     .      +  sodb(2,k,1,2,j)*phz*dphz2
     .      +  sodb(3,k,1,2,j)*dphz*phz2
     .      +  sodb(4,k,1,2,j)*dphz*dphz2
         vxzz = vxzz - vxzu*phz - vxzs*dphz - vxuz*phz2 - vxsz*dphz2
     .      +  sodb(1,k,2,2,j)*phz2*phz
     .      +  sodb(2,k,2,2,j)*phz2*dphz
     .      +  sodb(3,k,2,2,j)*dphz2*phz
     .      +  sodb(4,k,2,2,j)*dphz2*dphz
         vuz  = vuz  - sodb(1,k,1,2,j)*phz2 - sodb(2,k,1,2,j)*dphz2
         vxuz = vxuz - sodb(1,k,2,2,j)*phz  - sodb(2,k,2,2,j)*dphz
         vzu  = vzu  - sodb(1,k,1,2,j)*phz  - sodb(3,k,1,2,j)*dphz
         vxzu = vxzu - sodb(1,k,2,2,j)*phz2 - sodb(3,k,2,2,j)*dphz2
         vsz  = vsz  - sodb(3,k,1,2,j)*phz2 - sodb(4,k,1,2,j)*dphz2
         vxsz = vxsz - sodb(3,k,2,2,j)*phz  - sodb(4,k,2,2,j)*dphz
         vzs  = vzs  - sodb(2,k,1,2,j)*phz  - sodb(4,k,1,2,j)*dphz
         vxzs = vxzs - sodb(2,k,2,2,j)*phz2 - sodb(4,k,2,2,j)*dphz2
         sodb(5,k,1,2,j) = vuz
         sodb(6,k,1,2,j) = vsz
         sodb(7,k,1,2,j) = vzz
         sodb(8,k,1,2,j) = vzu
...
C --- Printout ---
      do  i = 1, nsp
        if (ipr >= 40) then
          call awrit1('%N 3rd gen potential parameters%?#n==2# spin 2##'//
     .      '%N l%8f  a      <phi phi>       a*D        a*Ddot      phi(a)      phip(a)',
     .      buf,120,stdo,i)
          do  l = 0, lmxa
C            call info5(1,0,0,'%,2i  %;12,6D%;12,6D%;13,6D%3;12,6D',
C     .        l,rmax,ppnl(2,l+1,i),ppnl(3,l+1,i),ppnl(4,l+1,i))
            call awrit5('%,2i  %;12,6D%;12,6D%;13,6D%3;12,6D',buf,100,stdo,
     .        l,rmax,ppnl(2,l+1,i),ppnl(3,l+1,i),ppnl(4,l+1,i))
C            write (stdo,3) l,rmax,(ppnl(k,l+1,i),k = 2,6)
C    3       format(i2,2x,2F12.6,f13.6,10F12.6)
          enddo
        endif
        if (ipr >= 60) then
C         call info0(1,1,0,'  l%9fval-val%5fval-slo%5fslo-val%5fslo-slo')
          call awrit0('%N  l%9fval-val%5fval-slo%5fslo-val%5fslo-slo',buf,80,stdo)
C          write (stdo,4)
C    4     format(/'  l',9x,'val-val',5x,'val-slo',5x,'slo-val',5x,
C     .      'slo-slo')

          do   l = 0, lmxa
            k  = l+1
C            write (stdo,7) l,'s',(sab(ir,k,i),ir = 1,4)
C            write (stdo,8) 'h',(hab(ir,k,i),ir = 1,4)
C            write (stdo,8) 'v',(vab(ir,k,i),ir = 1,4)
C            call info2(1,0,0,'%,3i%3fs%4;12,6D',l,sab(1,k,i))
C            call info2(1,0,0,'%6fh%4;12,6D',hab(1,k,i),2)
C            call info2(1,0,0,'%6fv%4;12,6D',vab(1,k,i),2)

            call awrit2('%,3i%3fs%4;12,6D',buf,100,stdo,l,sab(1,k,i))
            call awrit2('%6fh%4;12,6D',buf,100,stdo,hab(1,k,i),2)
            call awrit2('%6fv%4;12,6D',buf,100,stdo,vab(1,k,i),2)

            if (lso /= 0) then
C            write (stdo,9) 'sozz',(sodb(ir,k,i,1,1),ir = 1,4)
C            write (stdo,9) 'sopm',(sodb(ir,k,i,2,1),ir = 1,4)
C            write (stdo,9) 'Bzz',(sodb(ir,k,i,1,2),ir = 1,4)
C            write (stdo,9) 'Bpm',(sodb(ir,k,i,2,2),ir = 1,4)
C              call info2(1,0,0,'%3fsozz%4;12,6D',sodb(1,k,i,1,1),2)
C              call info2(1,0,0,'%3fsopm%4;12,6D',sodb(1,k,i,2,1),2)
C              call info2(1,0,0,'%4fBzz%4;12,6D',sodb(1,k,i,1,2),2)
C              call info2(1,0,0,'%4fBpm%4;12,6D',sodb(1,k,i,2,2),2)

              call awrit1('%3fsozz%4;12,6D',buf,100,stdo,sodb(1,k,i,1,1))
              call awrit1('%3fsopm%4;12,6D',buf,100,stdo,sodb(1,k,i,2,1))
              call awrit1('%4fBzz%4;12,6D',buf,100,stdo,sodb(1,k,i,1,2))
              call awrit1('%4fBpm%4;12,6D',buf,100,stdo,sodb(1,k,i,2,2))
            endif
          enddo

          if (lpz /= 0) then
C            call info0(1,1,0,
C     .        '  l%9f val-sc%5f slo-sc%5f sc-sc %5fsc-val%6fsc-slo')
            call awrit0('%N  l%9f val-sc%5f slo-sc%5f sc-sc %5fsc-val%6fsc-slo',buf,100,stdo)
            do   l = 0, lmxa
              k = l+1
              if (pnz(k,1) > 0) then
C               write (stdo,7) l,'s',(sab(ir,k,i),ir = 5,9)
C               write (stdo,8) 'h',(hab(ir,k,i),ir = 5,9)
C               write (stdo,8) 'v',(vab(ir,k,i),ir = 5,9)
C                call info2(1,0,0,'%,3i%3fs%5;12,6D',l,sab(5,k,i))
C                call info2(1,0,0,'%6fh%5;12,6D',hab(5,k,i),2)
C                call info2(1,0,0,'%6fv%5;12,6D',vab(5,k,i),2)
                call awrit2('%,3i%3fs%5;12,6D',buf,100,stdo,l,sab(5,k,i))
                call awrit2('%6fh%5;12,6D',buf,100,stdo,hab(5,k,i),2)
                call awrit2('%6fv%5;12,6D',buf,100,stdo,vab(5,k,i),2)
                if (lso /= 0) then

C                write (stdo,9) 'sozz',(sodb(ir,k,i,1,1),ir = 5,9)
C                write (stdo,9) 'sopm',(sodb(ir,k,i,2,1),ir = 5,9)
C                write (stdo,9) 'Bzz',(sodb(ir,k,i,1,2),ir = 5,9)
C                write (stdo,9) 'Bpm',(sodb(ir,k,i,2,2),ir = 5,9)

C                call info2(1,0,0,'%3fsozz%5;12,6D',sodb(5,k,i,1,1),2)
C                call info2(1,0,0,'%3fsopm%5;12,6D',sodb(5,k,i,2,1),2)
C                call info2(1,0,0,'%4fBzz%5;12,6D',sodb(5,k,i,1,2),2)
C                call info2(1,0,0,'%4fBpm%5;12,6D',sodb(5,k,i,2,2),2)

                  call awrit2('%3fsozz%5;12,6D',buf,100,stdo,sodb(5,k,i,1,1),2)
                  call awrit2('%3fsopm%5;12,6D',buf,100,stdo,sodb(5,k,i,2,1),2)
                  call awrit2('%4fBzz%5;12,6D',buf,100,stdo,sodb(5,k,i,1,2),2)
                  call awrit2('%4fBpm%5;12,6D',buf,100,stdo,sodb(5,k,i,2,2),2)
                endif
...
C       if (i < nsp) call info0(40,1,-1,' ... spin 2')

      enddo

    6 format(i2,a,f10.6,3F12.6,f12.4,f12.6)
    7 format(i3,3x,a1,6F12.6)
    8 format(3x,3x,a1,6F12.6)
    9 format(3x,a4,6F12.6)
