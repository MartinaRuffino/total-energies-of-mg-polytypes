...
TESTLMF lmfa --no-iactiv cu -vnk=8 -vbigbas=f
        ...
        mv bnds.cu bnds.cu-pwmode11
TMPKLMF lmfa --no-iactiv cu -vnk=8 -vbigbas=f
        lmf  --no-iactiv cu -vnk=8 -vbigbas=f
        rm mixm.cu
        mpirun -n 8 lmf --no-iactiv cu -vnk=8 -vbigbas=t -vpwmode=0 -voveps=0d-7
        mpirun -n 8 lmf --no-iactiv cu -vnk=8 -vbigbas=t -vpwmode=11 -voveps=0d-7 --band:fn=syml
        mv bnds.cu bnds.cu-pwmode11
...
TESTGW  echo -1 | lmfgw cu -vnk=8 -vbigbas=t 
        ...
        echo cu | lmf2gw 
CLEAN   rm -f \{atm,dos,mixm,out.lmf,save,ctrl,log,moms,rst,syml,wkp}.cu bnds.cu-pwmode11
...
BZ      NKABC=nk BZJOB=bzj W=.002 NPTS=1001 SAVDOS=t
        EF0=0 DELEF=.1 TETRA=t DOS=0-1 0+.5 METAL=5
% ifdef hf
        NEVMX=-1
% endif
