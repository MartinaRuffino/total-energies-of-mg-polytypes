C#define FP
C#ifdef FP
      integer function iorsf(mode,s_ctrl,s_site,s_spec,s_lat,s_pot,s_bz,
     .  fid,nbas,nat,nspec,qnu,nit,lbin,ifi)
C#elseC
C      integer function iorsa(mode,s_ctrl,s_site,s_spec,s_lat,s_pot,s_bz,
...
C#endif
      ...
      line = 'header'
C#ifdef FP
      if (mode == 1) call fftz30(n1,n2,n3,k1,k2,k3)
C#endif
      ffmt = '(5f15.10)'
      ifmt = '(20i5)'
C ... Hardwired for now
...
C ... Message
      if (ipr >= 10) then
        ...
        fid0(i:) = ', asa'
C#ifdef FP
        if (mode == -1) fid0(i:) = ' dimensioning data'
        if (mode == 1) fid0(i:) = ', mesh density'
C#endif
        i = len_trim(fid0)+1
        fid0(i:i) = ')'
...
C#ifdef FP
        ...
        if (mode == -2 .or. mode == -3) then
          ...
          n2 = n21
          n3 = n31
          if (mode == -2) s_lat%nabc = ngabc
          call fftz30(n1,n2,n3,k1,k2,k3)
          i = k1*k2*k3
C         Read into dummy; discard
          allocate(smrho(i*nsp)); call dpzero(smrho,2*i*nsp)
          call dpdftr(n1,n2,n3,k1,k2,k3,nsp0,smrho,lbin,jfi)
          deallocate(smrho)
        elseif (n11==n1 .and. n21==n2 .and. n31==n3) then
          call dpdftr(n1,n2,n3,k1,k2,k3,nsp0*nspc0,s_pot%smrho,lbin,jfi)
          if (nsp > nsp0) then
C           call zprm3('smrho before split',0,s_pot%smrho,k1,k2,k3)
            i = k1*k2*k3*2
            call dscal(i,0.5d0,s_pot%smrho,1)
            call dpscop(s_pot%smrho,s_pot%smrho,i,1,1+i,1d0)
C           call zprm3('smrho after split',0,s_pot%smrho,k1,k2,k3*2)
          endif
c   ... or read and remesh
        else
          if (ipr >= 10) write (stdo,3) n11,n21,n31,n1,n2,n3
    3     format(9x,'remesh density from  ',i4,'  *',i4,'  *',i4,
     .       '    to  ',i4,'  *',i4,'  *',i4)
          call fftz30(n11,n21,n31,k11,k21,k31)
          i = k11*k21*k31
          allocate(h(i*nsp)); call dpzero(h,2*i*nsp)
          call dpdftr(n11,n21,n31,k11,k21,k31,nsp0,h,lbin,jfi)
          if (nsp > nsp0) then
C           call zprm3('smrho before split',0,h,k1,k2,k3)
            i = k11*k21*k31*2
            call dscal(i,0.5d0,h,1)
            call dpscop(h,h,i,1,1+i,1d0)
          endif
          call pshpr(1)
          i = 0
          if (n1 == 2*n11 .and. n2 == 2*n21 .and. n3 == 2*n31) i=3
          call chgmsh(i,nsp,plat,n11,n21,n31,k11,k21,k31,h,
     .                      plat,n1,n2,n3,k1,k2,k3,s_pot%smrho)
          call poppr
          deallocate(h)
        endif
        if (mode == -2 .or. mode == -3) goto 5
C   ... If cell volume changed, scale smooth density to maintain charge
        vfac = vol0/vol
        if (dabs(vfac-1d0) > 1d-8) then
          if (ipr >= 10) write (stdo,4) vol0,vfac
    4     format(9x,'volume changed from',f8.2,
     .      ' :  scale smooth density by',f8.4)
          call dpcopy(s_pot%smrho,s_pot%smrho,1,2*k1*k2*k3*nsp,vfac)
        endif
        endif
        call mpibc1(s_pot%smrho,2*k1*k2*k3*ns4,4,mlog,'iors','smrho')
    5   continue
C#elseC
C        call rx('smooth density not implemented')
C#endif
        endif
...
C#ifdef FP
          call fsanrg(rmt,rmt0-2*tolr,rmt0+2*tolr,1d-6,msg//'         '//trim(spid)//':','',.true.)
C#elseC
C          call fsanrg(rmt,rmt0-2*tolr,rmt0+2*tolr,1d-6,msg//'         '//trim(spid)//':','',.false.)
C#endif
...
C#ifdef FP
     ...
     .      write(stdo,16) ib,spid,'kmax',kmax0,kmax
C#endif
...
C       ... Read rho1,rho2,rhoc,v0,v1 from disk
            if (procid == master) then ! Allocate temporary arrays if lattice mismatch
              if (nr == nr0 .and. a == a0 .and. abs(rmt-rmt0) < 1d-8) then
                rho1 => s_site(ib)%rho1
                rho2 => s_site(ib)%rho2
                rhoc => s_site(ib)%rhoc
                p_v0 => v0
                ...
                allocate(rho1(nr0,nlml*2*2),rho2(nr0,nlml*2*2))
                allocate(rhoc(nr0,2),p_v0(nr0,2),p_v1(nr0,2))
              endif

              if (.not. lbin) read(jfi,*)
              call dpdbyl(rho1,nr0,nlml0,nlml,nsp0*nspc0,ns4,lbin,jfi)
              if (.not. lbin) read(jfi,*)
              call dpdbyl(rho2,nr0,nlml0,nlml,nsp0*nspc0,ns4,lbin,jfi)
              if (nlml0 > nlml.and.ipr>=10)
  ...
  202         format(9x,'site',i4,', species ',a,
     .               ': ',a,' local density from nlm=',i3,' to',i3)
              if (.not. lbin) read(jfi,*)
              call dpdbyl(rhoc,nr0,1,1,nsp0,ns4,lbin,jfi)
              if (.not. lbin) read(jfi,*)
              call dpdbyl(p_v0,nr0,1,1,nsp0,nsp,lbin,jfi)
              if (.not. lbin) read(jfi,*)
              call dpdbyl(p_v1,nr0,1,1,nsp0,nsp,lbin,jfi)
              if (nsp0 < nsp) then
                call dscal(nr0*2,2d0,v0,1)
                call dscal(nr0*2,2d0,v1,1)
              endif
C             call prmx('iors: v0',v0,nr0,nr0,nsp)

C             General polynomial interpolation.  Works, but slow.
              if (nr /= nr0 .or. a /= a0 .or. abs(rmt-rmt0) > 1d-8) then
                oldmsh(1,is) = 1; oldmsh(2,is) = nr0; oldmsh(3,is) = a0; oldmsh(4,is) = rmt0
                ...
                do  i = 1, nr
                  ...
                  do  isp = 1, nsp
                    call polint(rofi0,p_v0(1,isp),nr0,npoly,rofi(i),0d0,0,jx,v0(i,isp),dv)
                    dvmx = max(dvmx,abs(dv))
!                    call polint(rofi0,p_v1(1,isp),nr0,npoly,rofi(i),0d0,0,jx,v1(i,isp),dv)
                    dvmx = max(dvmx,abs(dv))
                    call polint(rofi0,rhoc(1,isp),nr0,npoly,rofi(i),0d0,0,jx,s_site(ib)%rhoc(i,isp),dr)
                    drmx = max(drmx,abs(dr))
                    do  ilm = 1, nlml
                      k = ilm+(isp-1)*nlml
                      call polint(rofi0,rho1(1,k),nr0,npoly,rofi(i),0d0,0,jx,s_site(ib)%rho1(i,k),dr)
                      drmx = max(drmx,abs(dr))
                      call polint(rofi0,rho2(1,k),nr0,npoly,rofi(i),0d0,0,jx,s_site(ib)%rho2(i,k),dr)
                      drmx = max(drmx,abs(dr))
                    enddo
                  enddo
                enddo
...
!          call snot
!          print *, associated(v0),associated(v1)
!          call ptr_site(s_site,4+1,'v1',ib,nr,nsp,v1)
!          deallocate(v1)
C                call prrmsh('v0(init) ',rofi0,p_v0,nr0,nr0,nsp)
C                call prrmsh('v0(final)',rofi,v0,nr,nr,nsp)
C                call prrmsh('v1(init) ',rofi0,p_v1,nr0,nr0,nsp)
C                call prrmsh('v1(final)',rofi,v1,nr,nr,nsp)
C                call prrmsh('rhoc(init) ',rofi0,rhoc,nr0,nr0,nsp)
C                call prrmsh('rhoc(final)',rofi,s_site(ib)%rhoc,nr,nr,nsp)
C                call prrmsh('rho1(init) ',rofi0,rho1,nr0,nr0,nlml*nsp)
C                call prrmsh('rho1(final)',rofi,s_site(ib)%rho1,nr,nr,nlml*nsp)
C                call prrmsh('rho2(init) ',rofi0,rho2,nr0,nr0,nlml*nsp)
C                call prrmsh('rho2(final)',rofi,s_site(ib)%rho2,nr,nr,nlml*nsp)
                 call info8(20,0,0,'%8f interpolate (a=%d,nr=%i,rmt=%,6;6d)->'//
     .             '(a=%d,nr=%i,rmt=%,6;6d)  errr=%,3;3g  errV=%,3;3g',
     .             a0,nr0,rmt0,a,nr,rmt,drmx,dvmx)
                 deallocate(rofi,rofi0)
                 if (nr == nr0 .and. a == a0) then
                   nullify(rho1,rho2,rhoc,p_v0,p_v1)
                 else
                   deallocate(rho1,rho2,rhoc,p_v0,p_v1)
                 endif
              endif
            endif
            if (mode == -2) then
              ...
              i = nr*ns4
              call mpibc1(s_site(ib)%rho1,i*nlml,4,mlog,'iors','rho1')
              call mpibc1(s_site(ib)%rho2,i*nlml,4,mlog,'iors','rho2')
              call mpibc1(s_site(ib)%rhoc,nr*nsp,4,mlog,'iors','rho3')
              i = nr*nsp
              call mpibc1(v0,i,4,mlog,'iors','v0')
              call mpibc1(v1,i,4,mlog,'iors','v1')

            endif
C#elseC
C            call rxi('iors: not implemented, mode',mode)
C#endif
...
C#ifdef FP
C       Rotate local density after reading
        if (lshear .and. IAND(s_ctrl%lrs,256) /= 0) then
          call dgemm('N','T',3,3,3,1d0,plat,3,qlat0,3,0d0,wk,3)
C          print *, '!!'
C          print "(9f12.6)", plat
C          print "(9f12.6)", qlat0
C          call dgemm('N','T',3,3,3,1d0,plat0,3,qlat0,3,0d0,wk,3)
          call pvsms2(s_site,s_spec,wk,nbas,nsp)
C         stop
        endif
C#endif
...
C#ifdef FP
          lfail = isanrg(lfoc0,lfoc,lfoc,'file: species '//trim(spid)//':','foca',.false.)
C#endif
...
C#ifdef FP
        if (lbin) then
          ...
          write (jfi,18) 'smooth density'
          write(jfi,ifmt) n1,n2,n3
        endif
        call dpdftr(n1,n2,n3,k1,k2,k3,nsp*nspc,s_pot%smrho,lbin,-jfi)
C#endif
...
C#ifdef FP
          ...
          call dpdbyl(s_site(ib)%rho1,nr,nlml,nlml,ns4,ns4,lbin,-jfi)
          if (.not. lbin) write(jfi,'('' rho2'')')
          call dpdbyl(s_site(ib)%rho2,nr,nlml,nlml,ns4,ns4,lbin,-jfi)
          if (.not. lbin) write(jfi,'('' rhoc'')')
          call dpdbyl(s_site(ib)%rhoc,nr,1,1,nsp,nsp,lbin,-jfi)
          if (.not. lbin) write(jfi,'('' v0'')')
          call dpdbyl(v0,nr,1,1,nsp,nsp,lbin,-jfi)
          if (.not. lbin) write(jfi,'('' v1'')')
          call dpdbyl(v1,nr,1,1,nsp,nsp,lbin,-jfi)
C#elseC
C          if (lbin) then
C            call rx('iorsa not ready for binary mode')
CC           call dpdump(pl,nl*nsp,-jfi)
CC           call dpdump(s_site(ib)%rho1,nl*nsp*3,-jfi)
CC           call dpdbyl(v0,nr,1,1,nsp,nsp,lbin,-jfi)
C          else
C            rh = 0
C            vrmax(1) = 0
C            vrmax(2) = 0
C
C            if (mode == 0) then
C              ltmp1 = aiomom(spid,pnu,qnu(1,1,1+(ib-1)*nsp),[NULLR],idmod,n0,lmxa,nsp,0d0,rh,vrmax,(/0d0,0d0,0d0/),-jfi)
C              vrmax(1) = s_pot%ves(ic)
C            elseif (mode == 1) then
C              call dpzero(qnul,3*n0*nsp)
C              forall (i=1:n0, is0=1:nsp) qnul(1,i,is0) = s_pot%qbyl(i+n0*(is0-1),ib)
C              ltmp1 = aiomom(spid,pnu,qnul,[NULLR],idmod,n0,lmxa,nsp,0d0,rh,vrmax,(/0d0,0d0,0d0/),-jfi)
C              vrmax(1) = s_pot%vesrmt(ib)
C            endif
C            write(jfi,143) vrmax(1)
C  143       format('  Ves(rmax)=',f11.7)
C            if (mode == 0 .and. associated(s_pot%v0)) then
C              ic = s_site(ib)%class
C              ltmp1 = aiopot(nr,nsp,a,rmt,[NULLR],s_pot%v0(1,ic),-jfi)
C            elseif (mode == 1) then
C              ltmp1 = aiopot(nr,nsp,a,rmt,[NULLR],v0,-jfi)
C            endif
C          endif
C#endif
