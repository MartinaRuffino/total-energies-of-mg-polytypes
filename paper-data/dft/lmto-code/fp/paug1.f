...
C ... Passed parameters
      ...
      integer lx1(nf1),lx2(nf2),jcg(*),indxcg(*)
...
C         Set flag for all integrals --- integral not yet calculated
     ...
     .                         nr,rwgt,v2,sam)
                    sum(l1,l2,mlm) = sam
C                   if (sam /= 0) write(*,500) i1,i2,ilm1,ilm2,sam
...
C ... Passed parameters
      ...
      double precision sam,v2(nr,mlm),rwgt(nr),
     .  f1(nr,0:lmx1,i1),f2(nr,0:lmx2,i2)
C ... Local parameters
      integer i
