...
C ----------------------------------------------------------------------
      implicit none
C ... Passed parameters
      integer, intent(in) :: mode,lmxa,nr,nrmax,nsp
      integer, intent(inout) :: nrbig,lpz
      real(8), intent(in) :: z,rmax,a,v(nr,nsp),pnz(1+lmxa),rs3,vmtz
      real(8), intent(inout) :: rofi(nrmax),rwgt(nrmax),vbig(nrmax,nsp)
C ... Local parameters
      logical lmode0,lmode1,lmode2
      integer ir,j,k,l,isp
C     integer idn
...
C              Anyway, extrapolation is somewhat arbitrary.
      ...
      parameter (nrx=2001,nptirp=10,ehv=-1d0)
      double precision dasum,tphi,rbig,phi,dphi,xx
      double precision gzbig(nrx*2),gbig(nrx*2)

C ... Setup
      if (mode == 0) return
      call sanrg(.true.,mode,0,8,'vxtrap:','mode')
      lmode0 = mod(mode,2) /= 0
      lmode1 = mod(mode/2,2) /= 0
      lmode2 = mod(mode/4,2) /= 0
...
C        if (rbig > 2*rmax) then
C          idn = dlog(2d0)/a
C          if (mod(idn,2) == 1) idn = idn-1
...
C        call radwgt(intopt,rbig,a,nrbig,rwgt)
C        if (nr < nrbig) rwgt(nr) = rwgt(nr)/2
        return
      endif

C --- Potential on extended mesh ---
      if (.not. lmode2) return
      call sanrg(.true.,nrbig,nr,nrx,'vxtrap:','nrbig')
...
C ... Local parameters
      ...
      integer idn,info,intopt,ir,mode0,nglob,nxtn
      double precision rbig,rwgt(nrx),wk(2*nrx),xi(nrx,0:l),r2(nrx)
...
C       g = 0
        nxtn = nrbig-nrmt+1
        call radwgt(intopt,rbig,a,nrbig,rwgt)
