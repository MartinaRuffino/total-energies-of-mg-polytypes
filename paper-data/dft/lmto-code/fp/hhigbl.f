...
C ----------------------------------------------------------------------
      ...
      implicit none
C ... Passed parameters
      integer mode,jcg(*),indxcg(*),nlm1,nlm2,kmax,ndim1,ndim2,k0
...
C         l2t = l2
     ...
     .      lm21,lm22,kmax,ndim1,ndim2,k0,cg,indxcg,jcg,cy,s_lat,
     .      s,ds)
        enddo
      enddo
      end
      subroutine phhigb(dr,q,rsm1,rsm2,e1,e2,mlm1,nlm1,mlm2,nlm2,
     .  kmax,ndim1,ndim2,k0,cg,indxcg,jcg,cy,s_lat,s,ds)
C- Integrals between smooth hankels with k-th power of Laplace operator
C ----------------------------------------------------------------------
Ci Inputs
Ci   dr    :p1-p2
...
C ----------------------------------------------------------------------
      ...
      implicit none
C ... Passed parameters
      integer jcg(*),indxcg(*),nlm1,nlm2,kmax,ndim1,ndim2,k0,mlm1,mlm2
      double precision q(3),cg(*),cy(*),dr(3),rsm1,rsm2,e1,e2
      double complex s(ndim1,ndim2,0:k0),ds(ndim1,ndim2,0:k0,3)
C ... For structures
!      include 'structures.h'
      type(str_lat)::   s_lat
...
C --- Set up functions for connecting vector p2-p1 ---
      if (dabs(e1-e2) > 1d-5) then
        ...
        if (dabs(e1) > 1d-6) then
          call hklbl(dr,rsmx,e1,q,ktopp1,nlmxp1,ktop0,cy,s_lat,hkl1)
        else
          if (qq > 1d-6) call rx('hhigbl: e1=0 only allowed if q=0')
          call fklbl(dr,rsmx,ktopp1,nlmxp1,ktop0,cy,s_lat,hkl1)
        endif
        if (dabs(e2) > 1d-6) then
          call hklbl(dr,rsmx,e2,q,ktopp1,nlmxp1,ktop0,cy,s_lat,hkl2)
        else
          if (qq > 1d-6) call rx('hhigbl: e2=0 only allowed if q=0')
          call fklbl(dr,rsmx,ktopp1,nlmxp1,ktop0,cy,s_lat,hkl2)
        endif
        do  ilm = 1, nlmxp1
          do  k = 0, ktopp1
     ...
     .    call rx('hhigbl: case q=0 and e1=e2=0 not available')
        call hklbl(dr,rsmx,e,q,ktopp1,nlmxp1,ktop0,cy,s_lat,hkl2)
        call hsmbl(dr,rsmx,e,q,lmaxx+1,cy,s_lat,hsm,hsmp)
        do  ilm = 1, nlmxp1
