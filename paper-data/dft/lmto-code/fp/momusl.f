...
C ... Passed parameters
      ...
      integer, intent(in) :: lmxa,lmxl,nlml,nr,nsp
      real(8), intent(in) :: rmt,a,z,rofi(nr),rwgt(nr),rsml(n0),ehl(n0),
     .  v0(nr,nsp),v1(nr,nlml,nsp),pnu(n0,nsp),pnz(n0,nsp)
      real(8), intent(out) :: qum(0:lmxa,0:lmxa,0:lmxl,6,nsp),
     .  vum(0:lmxa,0:lmxa,nlml,6,nsp)
C ... Local parameters
      logical lpz1,lpz2
      integer l1,l2,lm,i,mlm,isp
      real(8) :: pi,srfpi,xx,xx1,
     ...
     .  gz(2*nr,0:lmxa,nsp),rss(nr,0:lmxa,2,nsp)

      call tcn('momusl')
      pi = 4d0*datan(1d0)
      srfpi = dsqrt(4d0*pi)
      call dpzero(gz,2*nr*(lmxa+1)*nsp)
!       ul = 0; sl = 0
      call makusp(n0,z,nsp,1,rmt,lmxa,v0,a,nr,xx,xx1,pnu,pnz,rsml,ehl,
...
C      call wrhomt(1,'f1p','f1(pointwise)',0,f1p,f1p,rofi,nr,nlm1,1)
C
C      sum = 0
C      do  ip = 1, np
C      do  ir = 1, nr
...
C      call wrhomt(1,'f2p','f2(pointwise)',0,f2p,f2p,rofi,nr,nlm2,1)
C
CC ... v(lm) > v(sphere)
C      call dgemm('N','T',nr,np,nlmv,1d0,vlm,nr,yl,np,0d0,vp,nr)
C      call wrhomt(1,'vp','v(pointwise)',0,vp,vp,rofi,nr,nlm2,1)
C
C      sum = 0
C      do  ip = 1, np
C      do  ir = 1, nr
