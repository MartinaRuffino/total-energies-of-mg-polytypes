...
Ci         :2 use both components, but use g1**2+g2**2 for normalization
Ci            In this case, z,v,e0,e1,ez are not used
Ci   z     :nuclear charge
...
Cr   relativistic case, mode=0, the inner product depends slightly
Cr   on z,v, and eigenvalue)
Cu Updates
Cu   25 Apr 12 New mode=2
...
C ... Local parameters
      ...
      k = l+1
      lpzi = 0
      if (mod(pnz(k),100d0) >   0) lpzi = 1
      if (mod(pnz(k),100d0) >= 10) lpzi = 2
      if (mod(pnz(k),100d0) >= 20) lpzi = 3
      ...
      elseif (lpzi == 1) then
        ...
        fac = phz*au + dphz*as
        x = 0
        call addrwf(1,x,l,[x],nr,0,nr,rofi,rwgt,x,x,-fac,g0(1,2),gz(1,2),xx)
        call addrwf(1,x,l,[x],nr,0,nr,rofi,rwgt,x,x,-fac,g0,gz,xx)

        fac = phz*bu + dphz*bs
        call addrwf(1,x,l,[x],nr,0,nr,rofi,rwgt,x,x,-fac,g1(1,2),gz(1,2),xx)
...
Ci         :1 use large component of radial w.f. only.
Ci            In this case, z,v,e0,e1,ez are not used
Ci         :2 use both components, but use normalization g1**2+g2**2 for normalization
Ci            In this case, z,v,e0,e1,ez are not used
Ci   z     :nuclear charge
Ci   l     :l quantum number
