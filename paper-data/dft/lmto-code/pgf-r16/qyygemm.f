      subroutine qyygemm(transa,transb,m,n,k,alpha,ar,ai,lda,br,bi,ldb,
     .  beta,cr,ci,ldc)
...
C Remarks:
Cr   Unless to tell it not to, qyygemm may perform the multiplication
...
Cr
Cr   You can make qyygemm do the multiplication in the normal 4 real
...
C Passed parameters
      ...
      real*16   alpha, beta
      real*16   ar(lda,1), br(ldb,1), cr(ldc,1),
...
C ... Check cases where multiplication cannot be done fast way
      lno3 = beta /= 0 .or.
     .       m < ncut .or. n < ncut .or. k < ncut
      if (len(transa) > 1) then
        lno3 = lno3 .or. transa(2:2) == '4'
      endif
      s = alpha
...
C ---   Real part ---
        ls = trnsa == 'C' .eqv. trnsb == 'C'
        if (ls .and. beta /= 0) then
          do  10  in = 1, n
          do  10  im = 1, m
   10     cr(im,in) = -cr(im,in)
        endif

        call qgemm(trnsa,trnsb,m,n,k,s,ai,lda,bi,ldb,beta,cr,ldc)
   ...
   12     cr(im,in) = -cr(im,in)
        ...
        call qgemm(trnsa,trnsb,m,n,k,s,ar,lda,br,ldb,one,cr,ldc)

C ---   Imaginary part ---
        ls = trnsb == 'C'
        if (ls .and. beta /= zero) then
          do  14  in = 1, n
          do  14  im = 1, m
   14     ci(im,in) = -ci(im,in)
        endif

        call qgemm(trnsa,trnsb,m,n,k,s,ar,lda,bi,ldb,beta,ci,ldc)

        if (ls .neqv. trnsa == 'C') then
          do  16  in = 1, n
          do  16  im = 1, m
   16     ci(im,in) = -ci(im,in)
        endif

        call qgemm(trnsa,trnsb,m,n,k,s,ai,lda,br,ldb,one,ci,ldc)
...
C ... ci = ai *  bi ...
      call qgemm(trnsa,trnsb,m,n,k,s,ai,lda,bi,ldb,zero,ci,ldc)

C ... cr = ar *  br ...
      call qgemm(trnsa,trnsb,m,n,k,s,ar,lda,br,ldb,zero,cr,ldc)

C ... cr <- ar*br - ai*bi,  ci <- -ar*br - ai*bi  ...
      if (trnsa == 'C' .eqv. trnsb == 'C') then
        do  20  in = 1, n
        do  20  im = 1, m
          xx = cr(im,in)
...
C ... ci += (ar+ai)(br+bi) ...
  ...
  124   br(im,in) = br(im,in) - bi(im,in)
      ...
      call qgemm(trnsa,trnsb,m,n,k,s,ar,lda,br,ldb,one,ci,ldc)
      kk = k
      mm = m
      if (trnsa == 'C' .or. trnsa == 'T') then
        kk = m
