...
C passed parameters
      ...
      double precision tau(3),dlat(3,nkd),rlat(3,nkr),cy(*),dl(*)
C local parameters
      integer ilm,nlm
...
C local parameters
      ...
      integer :: ilm,ir,l,m,nlm !,tpi
      real(8) :: fpibv,gamma,scalp,tpiba,yyy,eiphi(0:1),xxx,r2,r(3),yl((lmaxx+1)**2)
      external sylm

      if (lmax > lmaxx) call rx('rcnsl0: increase lmaxx')
!       tpi=8d0*atan(1d0)
      gamma = .25d0/(a*a)
      fpibv = 2d0*tpi/vol
      tpiba = tpi/alat
...
!             print '(a,x, 5(x,f20.12),3(x,i0))', "dl:", r, dl(ilm),yl(ilm),l,m,lmax
          enddo
!           m = 2*l+1
!           dl(ilm+1:ilm+m)=dl(ilm+1:ilm+m) + yl(ilm+1:ilm+m)*yyy*eiphi(0)
!           ilm = ilm+m
C eiphi *= (0,-1):
!           xxx = eiphi(0)
!           eiphi(0) = eiphi(1)
...
C passed parameters
      ...
      double precision tau(3),dlat(3,nkd),dl(*)
C local parameters
      integer ilm,ir,ir1,l,m,lmaxx
      parameter(lmaxx=9)
      double precision a2,cc,gl,r1,srpi,ta2,derfc,ddot,dl0
