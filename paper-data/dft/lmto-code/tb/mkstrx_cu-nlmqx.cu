...
// ----------------------------------------------------------------------
   ...
   int mlm, klm, lm, lk, ii, indx, icg1, icg2, llm, lp;
...
//       klm = els%nlm;
   ...
   // --- the following includes extra p terms 'implicitly' ---
      if ((mlm == klm) && (lm == 1) && ((ldip == 2) || (ldip == 3)) && (!mol)) {
         struxel += 1.0/vol;
         if (pv) dstrxel -= 3.0/vol;
      }

      double fac = 4.0 * M_PI * mone_on_l(lk) * sqrt(double((2*lk+1)*(2*lm+1)))/double(df[lk]*df[lm]);

      if (mlm > 0 && mlm < 9) mlm = msm[mlm-1];
      if (klm > 0 && klm < 9) klm = msm[klm-1];

      strux[mlm*nlmq + klm] = struxel *fac;
      if (npv) dstrx[mlm*nlmq + klm] = dstrxel *fac;
   }
//    fclose(fl);
//    __syncthreads();
//    return 0;
...
mkstrx_cu(int ldip, int nbas, int nbas1, int ib0, int nlmq1, int nlmq,
...
//    if (tid == 0) printf("block: %d %d %d speaking\n", blockIdx.x,blockIdx.y,blockIdx.z );
   ...
   int ibr, li, lj, nlx, nlf, lmxst; //lmxf,  lmax
   double tau[3];
...
extern "C"
void mkstrx_hcu(int ldip, int nbas, int nbas1, int ib0, int nlmq1, int nlmq,
                  double *bas, int *ipc, int nclas, int *lmxl, double awld, double alat, double vol,
                  double *dlat, int nkd, double *glat, int nkg,
                  int *indxcg, int *jcg, double *cg, double *cy,
