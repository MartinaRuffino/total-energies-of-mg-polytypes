...
Ci          2 by iterating embedding equation, starting from supplied trial gii
...
Co         :Note: in refresh mode, gii must be set to trial g on input
Cr Remarks
Cb Bugs
Cb  more consistent treatment of complex arithemetic needed
Cb  move ymscop from here to caller
Cu Updates
Cu   13 Jan 18 Iterative refinement of already-given surface gf
...
C ... Dynamically allocated local arrays
      real(8), allocatable :: wk2(:),gsLL(:,:,:),gsRR(:,:,:),gs(:,:,:),wkmix(:,:,:)
...
C     logical :: debug = .false.
      logical lso
      integer PRTG,i,ld0,ld2,lds,modeGF,maxit,mode0,mode1,kcplx,nbas,nbasp,nl,npl,lncol
      integer nspc,ld0x,ldsx,ld2x,lgLL,lgRR,lhdimp,neul,lidimp,ipr,jpr
C     For iterative refinement of the leads
      integer norm(10,10)
      integer :: nmix=2,nit,jmix
      integer, parameter :: nitmx = 200
      real(8) :: beta = .3d0, awk(10,2), rmsdel
...
C     Assemble s_pot%pfnc.  Note parallel lines in pgflu, pgemb
      ...
      elseif (nspc == 2) then
        ...
        call rotspn(230010+100*i,1,nbasp,nbasp,nl,s_ham%iprmb,s_ham%eula,neul,[0d0],[0d0],
...
!       allocate(sll(3*ld2x),snew(3*ld2x)) ! snew is a work array
        allocate(sll(ld0x,ld0x,3),snew(ld0x,ld0x,3)) ! snew is a work array
C       call pshpr(81)
...
C         call rx('pgsif: no bulk gf with decimation yet')
          if (mode1 == 2) then
            call rx('pgsif: no refinement of bulk gf yet')
          endif
          allocate(sllbk(3*ld2x),gsLL(ld0x,ld0x,2),gsRR(ld0x,ld0x,2))
          call dcopy(2*3*ld2x,sll,1,sllbk,1) ! Preserve sll.  Not needed?
          lgii = 1  ! Left surface
C#ifdefC QUAD
C          call qpgsifd(ld0x,sllbk,snew,wk,wk2,lgii,maxit,ld0x,gsLL)
C#else
          call pgsifd(ld0x,sllbk,snew,wk,wk2,lgii,maxit,ld0x,gsLL)
C#endif
          call lgupac(lgLL,'p',1,1,0,3,0,0)
C         call yprmi('gsLL isp=%i',isp,0,2,gsLL,ld0x**2,ld0x,ld0x,ld0x)
          call dcopy(2*3*ld2x,sll,1,sllbk,1) ! Preserve sll for pgembs call
          lgii = 2  ! Right surface
C#ifdefC QUAD
C          call qpgsifd(ld0x,sllbk,snew,wk,wk2,lgii,maxit,ld0x,gsRR)
C#else
          call pgsifd(ld0x,sllbk,snew,wk,wk2,lgii,maxit,ld0x,gsRR)
C#endif
          call lgupac(lgRR,'p',1,2,0,3,0,0)
C         call yprmi('gsRR from pgsifd, spin %i',isp,0,2,gii,ld0x**2,ld0x,ld0x,ld0x)
          call pgembs(ldsx,ld0x,ld0x,ld0x,ld0x,ld0x,ld0x,0,0,0,
     .      sll,30,lgLL,gsLL,lgRR,gsRR,sllbk,lgii,gii)  ! Use sllbk as work array
C         call yprmi('gii after pgembs, spin %i',isp,0,2,gii,ld0x**2,ld0x,ld0x,ld0x)

C   ... Semi-infinite g by decimation
        else
          lgii = mod(mode0,2)+1 ! 1 for left, 2 for right
          if (s_ctrl%refinegstol(1) > 0) then ! preserve sll
            s_ctrl%refinegstol(2) = 0
            allocate(sllbk(3*ld2x))
            call dcopy(2*3*ld2x,sll,1,sllbk,1)
          endif
          if (mode1 == 2) then ! skip generation of gii by decimation
            if (s_ctrl%refinegstol(1) <= 0) call rx('pgsif: refinement method requires tol set')
            call ztoyy(gii,ld0x,ld0x,ld0x,ld0x,kcplx,0) ! pgsif works in kcplx=0 mode now
          else  ! gii by decimation
C#ifdefC QUAD
C          call qpgsifd(ld0x,sll,snew,wk,wk2,lgii,maxit,ld0x,gii)
C#else
          call pgsifd(ld0x,sll,snew,wk,wk2,lgii,maxit,ld0x,gii)
C#endif
          call lgupac(lgii,'p',1,lgii,0,3,0,0)
C          if (debug) then
C          call yprmi('gs from pgsifd, spin %i',isp,0,2,gii,ld0x**2,ld0x,ld0x,ld0x)
C          endif
          endif

C     ... Iterative refinement of gii = gs through embedding
          if (s_ctrl%refinegstol(1) > 0) then
            allocate(wkmix(ld0x**2*2,0:nmix+1,2))  ! For Anderson mixing
            allocate(gs(ld0x,ld0x,2)) ! Use for either gs or gsRR
            call dcopy(ld0x**2*2,gii,1,wkmix(1,0,2),1) ! gs(in) -> wkmix(:,0,2)
            ...
            jmix = 0
            deallocate(wk); allocate(wk(3*ld2x))

            nit = 0
            do
C             gii contains either gsLL or gsRR.
C             Embedding generates gs ... should be same.  Iterate until diff < tol
              call dcopy(2*3*ld2x,sllbk,1,sll,1)
C             call yprm('gs before embedding',2,gii,ld0x**2,ld0x,ld0x,ld0x)
              call pgembs(ldsx,ld0x,ld0x,ld0x,ld0x,ld0x,ld0x,0,0,0,
     .          sll,10*(mod(mode0,2)+1),lgLL,gii,lgRR,gii,wk,lgii,gs)
C             call yprm('gs after embedding',2,gs,ld0x**2,ld0x,ld0x,ld0x)
              call dpscop(gs,wkmix,ld0x**2*2,1,1,1d0)
              call dcopy(ld0x**2*2,gs,1,wkmix,1) ! gs(out) -> wkmix(:,0,1)
              jpr = ipr-20; if (nit > nitmx-10) jpr = 30
              jmix = amix(ld0x**2*2,min(jmix,nmix),nmix,0,beta,jpr,
     .          5d0,norm,awk(1,2),wkmix,awk,rmsdel)
              s_ctrl%refinegstol(2) = max(rmsdel,s_ctrl%refinegstol(2))
              jmix = jmix+1
              call dcopy(ld0x**2*2,wkmix(1,0,2),1,gii,1) ! gs(mix) -> gs(in)
              if (nit == 0) s_ctrl%refinegstol(3) = rmsdel
C              if (nit == 0) print 444, qp, rmsdel
C  444         format('qp',3f12.6,' rmsdel',1pg9.2)
              nit = nit+1
              if (nit > nitmx) call rx1('pgsif: refinement failed to converge for q = %s,(%3;6d)',qp)
              if (rmsdel < s_ctrl%refinegstol(1)) then
                call info5(41,0,0,' pgsif: refinement converged to rms %;4g in %i iter, q = %s,(%3;6d)',
     .            rmsdel,nit,qp,4,5)
                exit
              endif
              call dcopy(ld0x**2*2,wkmix(1,0,2),1,gii,1) ! wkmix(:,0,2) -> gs(in)
              if (nit == nitmx/2) then ! Trouble ... mix more conservatively
                jmix = 0
                beta = .1d0
              endif
            enddo
...
C       deallocate(sll)
      ...
      if (ipr >= PRTG/1) then
        call awrit2('%a (%?#n==0#decimation#diagonalization#), layer %i',strn,len(strn),0,mode1,ipl)
...
C     if (iprint()>75) call yprm('(P-s00)',2,s,ld*ld*3,ld,ld,ld)
...
C   ... sp(1,*) <- (P-S11) (temporary storage)
        ...
        enddo
        enddo
C       if (iprint()>75) call yprm('(P-s11)',2,sp,ld*ld*3,ld,ld,ld)
...
C       if (iprint()>75) call yprm('(P-s11)^-1',2,sp,ld*ld*3,ld,ld,ld)

C   ... sp(3,*) <- s01 (P-s11)^-1 (temporary storage)
        call yygemm('N','N',ld,ld,ld,1d0,s(1,1,L,1),s(1,1,L,2),ld,
     .    sp,sp(1,1,1,2),ld,0d0,sp(1,1,3,1),sp(1,1,3,2),ld)
C       if (iprint()>75) call yprm('s01 (P-s11)^-1',2,sp(1,1,3,1),ld*ld*3,ld,ld,ld)
...
C       if (iprint()>75) call yprm('s01 (P-s11)^-1 s10',2,wk,ld*ld,ld,ld,ld)

C   ... sp(2,*) <- s01 (P-s11)^-1 s01 (updated s01)
        call yygemm('N','N',ld,ld,ld,1d0,sp(1,1,3,1),sp(1,1,3,2),ld,
     .    s(1,1,L,1),s(1,1,L,2),ld,0d0,sp(1,1,2,1),sp(1,1,2,2),ld)
C       if (iprint()>75) call yprm('s01 (P-s11)^-1 s01',2,sp(1,1,2,1),ld*ld*3,ld,ld,ld)

C   ... sp(3,*) <- s10 (P-s11)^-1 (temporary storage)
        call yygemm('N','N',ld,ld,ld,1d0,s(1,1,R,1),s(1,1,R,2),ld,
     .    sp,sp(1,1,1,2),ld,0d0,sp(1,1,3,1),sp(1,1,3,2),ld)
C       if (iprint()>75) call yprm('s10 (P-s11)^-1',2,sp(1,1,3,1),ld*ld*3,ld,ld,ld)

C   ... sp(1,*) <- s10 (P-s11)^-1 s01 (temporary storage)
        call yygemm('N','N',ld,ld,ld,1d0,sp(1,1,3,1),sp(1,1,3,2),ld,
     .    s(1,1,L,1),s(1,1,L,2),ld,0d0,sp,sp(1,1,1,2),ld)
C       if (iprint()>75) call yprm('s01 (P-s11)^-1 s10',2,sp,ld*ld*3,ld,ld,ld)
...
C   ... RMS s10, s11
        ...
        enddo
        if (iprint() >= 70 .or. iter == maxit .or. iprint() == 69 .and. iter == 1)
