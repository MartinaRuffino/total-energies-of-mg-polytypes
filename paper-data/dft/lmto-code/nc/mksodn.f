...
C ... Local parameters
      ...
      lmr = 0
      nlm = nl*nl
      call dpzero(CmEnu,size(CmEnu)); call dpzero(ovl,size(ovl))
      do  ib = 1, nbas
        ic = ipc(ib)
        lmr = nl*nl*(ib-1)
        do  l = 0, nl-1
          ilm = l**2+1
          if (indxsh(lmr+ilm) > ldim) cycle
          do  is = 1, nsp
            CmEnu(l,is) = pp(2,l,is,ic) - pp(1,l,is,ic)
            ovl(l,is) = oalpha(pp(1,l,is,ic),pp(2,l,is,ic),pp(3,l,is,ic)**2,pp(6,l,is,ic),pp(5,l,is,ic))
          enddo
        enddo

        call dpzero(sodi,size(sodi))
        call mksod1(nl,nlm,CmEnu,ovl,sop(0,1,1,1,ic),sodi) ! SO for this calss
        lmr = nl*nl*(ib-1)
        ilm = 0
        do  l = 0, nl-1
          do  m = -l, l
...
C   ... For 3C part  ho <phi Hso phi> oh + 2 ho <phi Hso dot> h + h <dot Hso dot> h
        ...
        do  m = -l, l
          ilm = ilm+1
          sod(ilm,3,isp) = m*sz*sop(l,isp,isp,1)*ovl(l,isp)**2 +
     .                   2*m*sz*sop(l,isp,isp,2)*ovl(l,isp) +
     .                     m*sz*sop(l,isp,isp,3)
        enddo
        enddo
...
C   ... For 2C part  <phi Hso phi> oh + <phi Hso dot> h + h.c.
        ...
        do  m = -l, l
          ilm = ilm+1
          sod(ilm,2,isp) = m*sz*(sop(l,isp,isp,2) + sop(l,isp,isp,1)*ovl(l,isp))
        enddo
        enddo
...
C   ... For 1C  <phi Hso phi>
        ...
        do  m = -l, l
          ilm = ilm+1
          sod(ilm,1,isp) = m*sz*sop(l,isp,isp,1)
        enddo
        enddo
...
C       (See "working directly with D" above)
        ...
        do  m = -l, l
          ilm = ilm+1
          sod(ilm,1,isp) = sod(ilm,1,isp) +
     .      2*sod(ilm,2,isp)*(CmEnu(l,isp)) +
     .      sod(ilm,3,isp)*(CmEnu(l,isp))**2
        enddo
        enddo
        ilm = 0
        do  l = 0, nl-1
        do  m = -l, l
          ilm = ilm+1
          sod(ilm,2,isp) = sod(ilm,2,isp) +
     .      sod(ilm,3,isp)*(CmEnu(l,isp))
        enddo
        enddo
      enddo
...
C ... For 3C contribution to L-S+ block
      ...
      do  l = 0, nl-1
        do  m = -l, l
          ilm = ilm+1
          sod(ilm,4,3) = sp*flmp(m) * (ovl(l,1)*(sop(l,1,2,2) + ovl(l,2)*sop(l,1,2,1))
     .                 + sop(l,2,1,2)*ovl(l,2) + sop(l,1,2,3))
        enddo
      enddo
      sod(ndim,4,3) = 0
...
C ... For 2C contribution to L-S+ block
      ...
      do  m = -l, l
        ilm = ilm+1
        sod(ilm,2,3) = sp*flmp(m)*(sop(l,2,1,2) + ovl(l,1)*sop(l,2,1,1)) ! <phi_lm L-S+ dot_lm+1>_21
        sod(ilm,3,3) = sp*flmm(m)*(sop(l,1,2,2) + ovl(l,2)*sop(l,1,2,1)) ! <phi_lm-1 | L-S+ | dot_lm>_12
      enddo
      enddo
      sod(nlm,2,3) = 0
      sod(1,3,3) = 0
...
C ... For 1C contribution to L-S+ block
      ...
      do  m = -l, l
        ilm = ilm+1
        sod(ilm,1,3) = sop(l,1,2,1)*flmp(m)*sp
      enddo
      enddo
      sod(nlm,1,3) = 0

C ... Add diagonal (C-enu) subtracted from 2- and 3-center; see Remarks
      ilm = 0
      do  l = 0, nl-1
      do  m = -l, l
        ilm = ilm+1
        if (ilm == nlm) cycle  ! sod(nlm,1,3) should be zero
        sod(ilm,1,3) = sod(ilm,1,3) +
     .    (CmEnu(l,1))*sod(ilm,2,3) +
     .    (CmEnu(l,2))*sod(ilm+1,3,3) +
     .    (CmEnu(l,1))*sod(ilm,4,3)*(CmEnu(l,2))
      enddo
      enddo
      ...
      do  m = -l, l
        ilm = ilm+1
        if (ilm == nlm) cycle ! sod(nlm,2,3) should be zero
        sod(ilm,2,3) = sod(ilm,2,3) + (CmEnu(l,2))*sod(ilm,4,3)
      enddo
      enddo
      sod(nlm,2,3) = 0
      ...
      do  m = -l, l
        ilm = ilm+1
        if (ilm == 1) cycle ! sod(nlm,2,3) should be zero
        sod(ilm,3,3) = sod(ilm,3,3) + (CmEnu(l,1))*sod(ilm-1,4,3)
      enddo
      enddo
      sod(1,3,3) = 0

C     call prmx('soph(3,:,:,:)',soph(3,:,:,:),4,4,ldim)
C      call prmx('sod',sod,ndim,nlm,12)
