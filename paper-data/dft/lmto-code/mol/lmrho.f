...
C#endif
C#ifdef MCRHO
      subroutine lmrho(prgnam,s_ctrl,s_ham,s_pot,s_lat,s_bz,s_mix,
...
C#else
      lcor = 0
      orhoic = 1
      if (cmdopt('--ra',4,0,outs)) then
        call mopen(78,'fa','f')
        rewind 78
        if (cmdopt('--ra0',5,0,outs)) then
          call rhdma0(rhoid,spid,nbas,nsp,npan,n0,ips,lmxl,lmxa,
     .    rmt,nr,a,lcor,ltail,lrs(4),nxi,lxi,exi,orhoi,orhoic,orho0,
     .    scale,nri,nvi,pnu,pnuz,idmod,idmoz,orho,orhoc,
     .    pos,f,psav,fsav,it,lrs(3),ipr,78)
        else
     ...
     .    tstep,time,etot0,zeta,lrs,ipr,78)
        ...
        rewind ifi
        if (cmdopt('--rb0',5,0,outs)) then
          call rhdmp0(rhoid,spid,nbas,nsp,npan,n0,ips,lmxl,lmxa,
     .    rmt,nr,a,lcor,ltail,lrs(4),nxi,lxi,exi,orhoi,orhoic,orho0,
     .    scale,nri,nvi,pnu,pnuz,idmod,idmoz,orho,orhoc,
     .    pos,f,psav,fsav,it,lrs(3),ipr,ifi)
        else
...
C#elseif MCRHO
     ...
     .  nphi,ephi,lphi,nel,nhs,w(oevl),w(1))
      call rx0('plro')
C#endif
C ---  Dump to file 'p2' ---
      vint = 0
