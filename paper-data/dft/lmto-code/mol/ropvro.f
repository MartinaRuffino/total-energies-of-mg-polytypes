      subroutine ropvro(dx,dy,dz,pos,nbas,rsm,rcut,rint,radd,nxi,lxi,
...
c  loops over all atoms, makes integrals vxc*h in one xy plane
     ...
     .   vxci(1),pos(3,1),ipd(1),ioff(1),cy(1),ips(1),rhot(1),
...
c --------- gather x,y coordinates and vxc ---------
     ...
     .   rhot,np,mp,w(ox1),w(oy1),w(ov1),w(ox),w(oy),w(oz),w(ovxc))
      if(np == 0) goto 10
      call rlse(ox1)
...
c --------- make and add density in xy-plane --------
      ...
      call defrr(oq,      np)
      ir=ioff(ib)+1
      call ropvho(nxi(is),lxi(1,is),exi(1,is),rsm(is),rcut(is),rint(is),
