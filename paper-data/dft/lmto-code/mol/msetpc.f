      subroutine msetpc(spid,z,rmt,rsm,rint,rcut,nphi,lphi,ephi,nxi,
     .  lxi,exi,nsp,lmxl,lmxa,pin,p,piz,pz,lsc,
     .  nspec,n0,nel,el,atid,pos0,pol,pos,ips,nbas,amp,
     .  ioff,ioffv0,ioffp,nri,nvi,ndim,nhs,nla,nlmax,nll,qval,qsmc)
c  all kinds of setup for m-programs (two-panel)
C  note msetpc differs from msetp2 only in that nlmax is returned.
C  should merge back to avoid proliferation of msetp
      implicit real*8 (a-h,p-z), integer (o)
      dimension rmt(1),rsm(1),z(1),nxi(1),lxi(n0,1),exi(n0,1),
     .  pin(n0,nsp,1),p(n0,nsp,1),pz(n0,nsp,1),piz(n0,nsp,1),
     ...
     .  ndim(nel),konf(10),ioffp(1),pos0(3,1),rcut(1)
      ...
      call getpr(ipr)
      if(ipr >= 10) write(6,330) nbas,nspec,amp
  330 format(/' msetp2:   nbas=',i3,'   nspec=',i3,'    amp=',f10.6)
      write(71,777) amp
