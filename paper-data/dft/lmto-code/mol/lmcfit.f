...
C#endif
C#ifdef MCFIT
      subroutine lmcfit(prgnam,s_ctrl,s_ham,s_pot,s_lat,s_bz,s_mix,
     .  s_spec,s_site,s_str,s_move,s_strn)
C#endif
C#ifdefC MCXBS
...
C#ifdef MCFIT
      integer nn,nph,js,ip,nalftc,ncupl,ndust
      double precision ri1,ri2,adec,wftx,d0
C#endif
C#ifdefC MCA
...
C --- make and store OCF ---
      ...
      do  20  is = 1, nspec
        nn = 0
        rsm(is) = 0d0
        call on2gen(n0,lxi(1,is),exi(1,is),nxi(is),lphi(1,is),el,nel,
     .    llink,elink,tphi(1,1,is),rmt(is),rsm(is),0d0,ri1,ri2,nph,ifi)
   20 continue
      call mclose(ifi)

C --- make and store TCF ---
   29 call query('TCF?',-1,0)
      ifi = 62
      call mopen(ifi,'t2','u')
      rewind ifi
      do  24  is = 1, nspec
...
C ...     Extract TCF parms specific to this pair (of default parms)
          do  27  i = 2, ntcpm
          ip = i
   27     if (is == nint(tcpm(1,i)) .and. js == nint(tcpm(2,i)) .or.
     .        is == nint(tcpm(2,i)) .and. js == nint(tcpm(1,i))) goto 26
          ip = 1
   26     continue
          nbisi(1) = nint(tcpm(3,ip))
          nbisi(2) = nint(tcpm(4,ip))
          nbisi(3) = nint(tcpm(5,ip))
          nalftc   = nint(tcpm(6,ip))
          adec     = tcpm(7,ip)
          wftx     = tcpm(8,ip)
          ncupl    = nint(tcpm(9,ip))
          ndust    = nint(tcpm(10,ip))

          d0 = rmt(is)+rmt(js)
C ... No TCF unless at least 2 atoms of a species or --allp set
          if (.not. cmdopt('--allp',6,0,outs)) then
          i = 0
          j = 0
          do  28  ib = 1, nbas
            if (atid(ib) == spid(is) .and. i == 0) i = ib
            if (atid(ib) == spid(js) .and. (j == 0 .or. j == i)) j = ib
   28     continue
          if (j == i) goto 25
          endif
          if (nbisi(3) == 1) d0 = dsqrt((pos0(1,i)-pos0(1,j))**2+
     .      (pos0(2,i)-pos0(2,j))**2+(pos0(3,i)-pos0(3,j))**2)
          rsm(is) = 0d0
          rsm(js) = 0d0
          call hy2gen(lxi(1,is),exi(1,is),nxi(is),lxi(1,js),exi(1,js),
     .      nxi(js),lphi(1,is),el,nel,lphi(1,js),el,nel,n0,
     .      llink,elink,tphi(1,1,is),elink,tphi(1,1,js),rmt(is),rmt(js),
     .      0d0,0d0,rsm(is),rsm(js),
     .      d0,nbisi(3),adec,wftx,nalftc,
     .      nbisi(1),nbisi(2),rmt(is),rmt(js),ncupl,ndust,ifi)

C --- Uncomment following for plot ---
C          rewind 61
C          npwr = 0
C          is = 1
C          js = 1
...
C          lp1 = lphi(ie,is)
C          lp2 = lphi(je,js)
C          call defrr(otspec,  100*ntx)
C          call defrr(otdata,  ndx)
C          call hyfinp(1,w(otspec),w(otdata),ntx,ndx)
C          print *, 'enter dir,d'
C          read(*,*) dr,d
C          dr1 = dsqrt(dr(1)**2+dr(2)**2+dr(3)**2)
C          do  7  m = 1, 3
C    7     dr(m) = dr(m)*d/dr1
C          nlm1 = (lp1+1)**2
C          nlm2 = (lp2+1)**2
C          nf1 = 0
C          do  17  k = 1, nxi(is)
C   17     nf1 = nf1 + (lxi(k,is)+1)**2
C          nf2 = 0
C          do  18  k = 1, nxi(js)
C   18     nf2 = nf2 + (lxi(k,js)+1)**2
C          call defrr(oc1,     nf1*nlm1*nlm2)
C          call defrr(oc2,     nf2*nlm1*nlm2)
C          call hyfget(dr,rmt(is),el(ie),nlm1,rmt(js),el(je),nlm2,
C     .      w(oc1),w(oc2),nf1,nf2,nxi(is),lxi(1,is),exi(1,is),
C     .      nxi(js),lxi(1,js),exi(1,js),w(otspec),w(otdata))
CC          call ious(is,oul1,osl1,a1,r1,nris,lmxa1,83)
CC          call ious(js,oul2,osl2,a2,r2,nrjs,lmxa2,83)
C          call hy2plt(lxi(1,is),exi(1,is),nxi(is),w(1),npwr,
C     .      lxi(1,js),exi(1,js),nxi(js),w(1),npwr,dr,rmt(is),
C     .      rmt(js),rsm(is),rsm(js),el(ie),el(je),w(oc1),w(oc2),
C     .      nf1,nf2,nlm1,nlm2)
