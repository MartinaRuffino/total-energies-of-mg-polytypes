      subroutine rovlaz(nxi,lxi,exi,n0,rmt,rint,a,nr,lmxl,
     .  nbas,alat,pos,ips,rhoi,ioff,cg,jcg,indxcg,cy,ixp,dxp,orho)
c  Overlap interstitial tail density for all spheres.
...
c ---------- loop over energies, add bessel tails ----
     ...
     .  lmxl(is),1,w(ohl),w(ohl))
      ...
      lb=lxi(je,js)
      nlmb=(lb+1)**2
      if(nlmb > nlmbx) call rx('rovlaz: increase nlmbx')
...
C     .  w(ocf),w(ohl),w(os))
     ...
     .  w(ocf),w(ohl),w(os))
      ...
      call rovlp1(nlml,nlmb,w(os),nr1,rhoi(j1),w(oxi),w(oh),
     .   w(orofi),w(oro))
  11  j1=j1+nlmb
      call rlse(ohl)
  20  continue
  ...
  787 format(' rovlaz:  q1,q2,qtot=',3f12.6)
  10  continue
      call tcx('rovlaz')
      end

c ------ sub rovlp1 ------------------------
      subroutine rovlp1(nlml,nlmb,s,nr1,rhoi,xi,h,rofi,rhol)
      implicit real*8 (a-h,p-z), integer (o)
