...
C ... Local parameters
      integer stdo,ipr,ir,i,j,k,l,lpz,lpzi(0:n0),nrbig,lorthz,lrel,lwronsk
      integer nrx
      parameter (nrx=1501)
      character*80 outs
...
C     Spin-Orbit related parameters
     ...
     .                 pzi(nr,0:lmxa,nsp),ezum(0:lmxa,nsp),
...
C ... Semicore wf gz and its sphere boundary parameters
      if (lpzi(l) /= 0) then
        call makrwf(10*lwronsk,z,rmax,l,v(1,i),a,nr,rofi,pnz(k,i),0d0,4,
...
C ... 2nd generation potential parameters (Methfessel scaling)
      ...
      if (ipr >= 40) write(stdo,699) l,'    ',ev,vl,c,srdel,qpar,ppar
