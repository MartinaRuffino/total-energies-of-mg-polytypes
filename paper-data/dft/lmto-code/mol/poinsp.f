...
C ... Passed parameters
     ...
     .  vhrho(2),rho0(nr)
C ... Local parameters
      integer nsp,ir,ilm,l,ll,lp1,intopt,nglob
      double precision fpi,srfpi,y0,atepi,ea,a2b4,rpb,rmax,fllp1,df,r,
     .  drdi,srdrdi,g,x,f,y2,y3,y4,vnow,vhom,alfa,sum,wt(nr)
...
C --- Add homogeneous solution --- ---
   ...
   10 sum = sum + wt(ir)*rho(ir,ilm)*v(ir,ilm)
      rhves1 = rhves1 + sum
      v(1,ilm) = 0d0
   80 continue
