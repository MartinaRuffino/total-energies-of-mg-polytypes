      subroutine hy2nrm(mmax,lx1,ex1,nx1,lx2,ex2,nx2,nb1,nb2,
     .  lscal,rmt1,rmt2,ri1,ri2,rsmp1,rsmp2,llink,el1,add1,el2,add2,
     .  rsm1,rsm2,lp1,ep1,np1,lp2,ep2,np2,idxj,nph,lsymj,s,b,
...
C- Makes normal matrix for hyfgen (s=overlap and b=rhs)
     ...
     .  nx1,nx2,ndimx,ndist,nrhsx,nsdmx,lscal,llink
      integer lx1(6),lx2(6),lx(20)
      double precision ep1,ep2,ri1,ri2,zc1,zc2,
     .  rsm1,rsm2,rsmp1,rsmp2,rmt1,rmt2,el1,add1(1),el2,add2(1),
...
C --- Start loop over distances ---
  ...
  550 format(' hyfnrm: idist=',i3,' of',i3,'   d=',f9.6,'   wgt=',f9.6)
     ...
     .  ri1,ri2,rsmp1,rsmp2,llink,el1,add1,el2,add2,rsm1,rsm2,rmt1,rmt2,
     .  lscal,1,np,nbisi,lp1,ep1,np1,lp2,ep2,np2,cx,d,
...
C --- Make the 2-center fit integrals at this distance -------
   ...
   50 continue
      ...
      subroutine hy2xim(lx1,ex1,nx1,lx2,ex2,nx2,nb1,nb2,
     .  ri1,ri2,rsmp1,rsmp2,llink,el1,add1,
     .  el2,add2,rsm1,rsm2,rmt1,rmt2,
     .  lscal,lsw,np,nbisi,lp1,ep1,np1,lp2,ep2,np2,cx,d,
     .  lmax,lmxp1,lmxp2,nx,lx,zc1,zc2,oph1,oph2,owp,oxi,oxp,ozp)
C-  Generate mesh and tabulate phi's and xi's there
      implicit none
      double precision cx(1),d,ep1,ep2,ex1(1),ex2(1),ri1,ri2,
     .  rsm1,rsm2,rsmp1,rsmp2,zc1,zc2,rmt1,rmt2,el1,el2,
     .  add1(1),add2(1)
      integer lmax,lp1(1),lp2(1),lx1(1),lx2(1),lscal,lsw,
     .  oidx,oph1,oph2,owk,owp,oxi,oxp,oyl,ozp,np1,np2,llink,
...
C --- Tabulate xi's and phi's for this mesh ----------
      if (iprint() >= 60) call tc('start smoothed h')
      call defrr(oxi,    np*nlm*(nx1+nx2))
      call defrr(oph1,   np*nlmp1*(np1+1))
      call defrr(oph2,   np*nlmp2*(np2+1))
      call defrr(owk,    np*ndCRAY)
      call defrr(oyl,    np*max(6,nlm)*ndCRAY)
      call defrr(oidx,   np*2)
      call hy2xi(np,w(oxp),w(ozp),w(owp),lscal,lsw,lmax,nlm,rmt1,rmt2,
     .  zc1,nx1,lx1,ex1,lmxp1,nlmp1,rsmp1,rsm1,lp1,ep1,np1,el1,add1,
     .  zc2,nx2,lx2,ex2,lmxp2,nlmp2,rsmp2,rsm2,lp2,ep2,np2,el2,add2,
     .  llink,cx,w(oidx),w(owk),w(oyl),w(oph1),w(oph2),nx,lx,w(oxi))
      call rlse(owk)
      end
      subroutine hy2xi(np,xp,zp,wp,lscal,lws,lmax,nlm,rmt1,rmt2,
     .  zc1,nx1,lx1,ex1,lmxp1,nlmp1,rsmp1,rsm1,lp1,ep1,np1,el1,add1,
     .  zc2,nx2,lx2,ex2,lmxp2,nlmp2,rsmp2,rsm2,lp2,ep2,np2,el2,add2,
     .  llink,cx,idx,wk,yl,ph1,ph2,nx,lx,xi)
C- Makes phi's and xi's (linked basis)
...
C  lws=1:   fold in weights; wp are DESTROYED on output.
     ...
     .        nx2,lx2(1),lp2(1),np2,lmxp2,nlmp2,lx(1),nx,llink
      double precision xp(1),zp(1),wp(1),
     .  cx(1),wk(1),idx(1),yl(1),tol,zc,xi(np,nlm,1),
     .  ex1(1),ep1,zc1,rsmp1,rsm1,ph1(np,nlmp1,1),rmt1,el1,add1(1),
     .  ex2(1),ep2,zc2,rsmp2,rsm2,ph2(np,nlmp2,1),rmt2,el2,add2(1)
...
C --- Smoothed Hankels xi and phi centered at origin ---
     ...
     .  tol,cx,idx,wk,yl,10,xi)
      call sll0sr(rsmp1,lmxp1,nlmp1,np1,lp1,ep1,xp,zp,np,np,
     .  tol,cx,rmt1,llink,el1,add1,idx,wk,yl,0,ph1)
...
C --- xi and phi centered off origin ---
     ...
     .  tol,cx,idx,wk,yl,10,xi(1,1,nx1+1))
      call sll0sr(rsmp2,lmxp2,nlmp2,np2,lp2,ep2,xp,zp,np,np,
     .  tol,cx,rmt2,llink,el2,add2,idx,wk,yl,0,ph2)
      if (lscal == 1)
     .  call hyfxsc(nx2,lx2,ex2,lmax,rmt2,np,nlm,1,xi(1,1,nx1+1))

C      jj = jj+1
C      qq = dcos(4*datan(1d0)*(jj-0.5d0)/5)
C      qq = dcos(2*dacos(qq))
C     print *, 'jj,qq=', jj,qq

CC ... Cludge to guarantee exact fit for testing, 1st rhs ...
C      do  66 ip = 1, np
C        ph1(ip,1,1) = dsqrt(qq/1000*xi(ip,1,1) + xi(ip,1,nx1+1))
C        ph2(ip,1,1) = dsqrt(qq/1000*xi(ip,1,1) + xi(ip,1,nx1+1))
C   66 continue
