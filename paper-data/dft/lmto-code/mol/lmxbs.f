...
C#endif
C#ifdef MCXBS
      subroutine lmxbs(prgnam,s_ctrl,s_ham,s_pot,s_lat,s_bz,s_mix,
...
C#ifdef MCXBS
C --------- make bs. file for xbs -----------------
      if (.not. cmdopt('--rs',4,0,outs)) then
        call mkbs(atid,spid,ips,alat,nbas,nspec,z,rmt,pos,
     .            colxbs,radxbs)
        if (cmdopt('--bonds',7,0,strn)) then
          dist0 = 5
          if (cmdopt('-d=',3,0,strn)) then
            j = 3
            if (.not. a2bin(strn,dist0,4,0,' ',j,len(strn)))
     .        call rx('mcxbs: bad value -d')
          endif
          call bonds(dist0,nbas,pos,alat,atid)
        endif
        goto 1
      endif
C#endif
