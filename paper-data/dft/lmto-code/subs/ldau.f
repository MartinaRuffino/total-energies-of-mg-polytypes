...
C      if (lvrs == 4 .or. lvrs == 5) then
C        call dcopy(2*(2*lmaxu+1)**2*2,dmatu(-lmaxu,-lmaxu,1,iblu),1,
C     .    vorb(-lmaxu,-lmaxu,1,iblu),1)
CC       call prmx('vorb, spin 1',vorb,2*lmaxu+1,2*lmaxu+1,2*lmaxu+1)
C      endif
      if (lvrs == 3) then
        do  isp = 1, 2
          aven(isp) = 0d0
          ...
          do  m1 = -l,l
            do  m2 = -l,l
              do  ii = -l,l
                den2(m1,m2) = den2(m1,m2) + den1(m1,ii)*den1(m2,ii)
              enddo
            enddo
          enddo
          num(isp) = 0d0
        ...
        enddo
        if (bot == 0d0) call rx('LDAU: divide by zero bot')
        aaa = nnum/((2*l+1)*bot)
