      subroutine hansr(rsm,lmn,lmx,nxi,lxi,exi,rsq,nrx,nr,idx,wk,job,xi)
...
Ci   wk      double precision work array of length (4+lmx-lmn)*nrx
Ci           Length can be 2*nrx if 10s digit of job set.
Co Outputs
Co   xi      smoothed Hankels for: xi(1..nr, 0..lxi(ie), 1..nxi)
Co           xi is the radial part/r**l, so the solid Hankel is
Co           hl(ilm) = xi(l)*Yl(ilm)
Co           where the Yl are r^l * (spherical harmonics)
Cl Local variables
...
C ---------------------------------------------------------------
      ...
      double precision rsq(nrx),e,exi(nxi)
...
C     ... Normal evaluation of smoothed Hankels
          if (n1 <= nr)
     .      call hansr2(rsq(n1),lmn,lmax,nrx,n2-n1,e,rsm,wk(n1,1),wk(n1,2),xi(n1,lmn,ie))
...
C ---   Polynomial to nmaxl ---
          do  i = nm2+1, nmax
            ...
            cofl(i) = add
            nmaxl = i
            if (dabs(add*r2max) < cof0(l)*tol) goto 2
            rhs = -rhs*a2/i
          enddo
          call info5(1,0,0,' hansr1 (warning) h(l=%i,e=%;1d) converged to %1,3;3g > tol (%1,3;3g)',
     .      l,e,add*r2max/cof0(l),tol,5)
          if (dabs(add*r2max) > cof0(l)*10000*tol) call rx('hansr1 failed to converge')
    2     continue
          forall (ir = 1:nr) xi(ir,l) = cofl(nmaxl)
          do  m = nmaxl, 1,-1
            do  ir = 1, nr
