          ...
          do  i = 1, nrs
            do  j = 1, nrs
              ...
              if (latvec(1,tol1,qlat,dbas)) then
                if (iprm(i) /= 0) call fexit2(-1,111,' Exit -1 '//
...
C     ... No match unless all iprm are nonzero
          do  i = 1, nrs
            if (iprm(i) == 0) then
              if (ipr >= 60) write(stdo,333) ig,trans,is
