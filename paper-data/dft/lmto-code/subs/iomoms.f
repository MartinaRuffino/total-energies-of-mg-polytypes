...
Ci   accwt :archaic: no longer used.
Ci   nspc:  1 for uncoupled spins, 2 for coupled spins
Cio Inputs/Outputs.
Cio    nev: (input file write)
Cio         (output, file read)
Cio          number of evals read at this k-point:
Cio          required that nev <= nevmx
Cio  nfstg: a compound of digits specifying what is to be I/O
Cio       : in moments file; See Remarks
Cio        :1s digit =>  eb I/O
Cio        :10s digit => doswt I/O; specifies number of 'spin' channels to I/O.
Cio                      See Remarks in iomomn for how this number
Cio                      is set, and the meaning of each channel.
Cio        :100s digit => dosw2 I/O
Cio        :1000s digit => I/O in parallel mode
...
C   --- Read data for this qp ---
        elseif (iq <= nkp*nsp .and. nfstgf == 0) then
          call rx('IOMOMS: file is missing bands')
        elseif (iq <= nkp*nsp .and. nfstg >= 0) then
          read (ifi,err=999,end=999) nchani,nev
          if (nchani /= nchan .and. nfstg > 1)
     .      call rx('IOMOMS: nchan mismatch')
          if (nev > nevmx .and. nfstg > 0)
...
C     ... Read, or read over rhos
          if (mod(nfstg/100,10) == 0) then
            if (mod(nfstgf/100,10) == 1) read(ifi,err=999,end=999) xx
          elseif (mod(nfstg/100,10) == 1) then
            if (mod(nfstgf/100,10) == 1) then
              if (.not. lddump(dosw2(1,iq),2*nchan2*nev*3,ifi)) goto 999
C              if (nev /= nevmx) call rx('iomoms can''t read dosw2')
            else
              call rx('IOMOMS: file missing spin-density-matrix')
            endif
          endif
        elseif (iq == nkp*nsp+1) then
