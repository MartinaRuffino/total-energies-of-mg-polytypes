      subroutine stoner(nl,nclass,clabl,idxdn,ef,dr,npts,
...
C Passed Parameters
      logical io
      integer nl,nclass,idxdn(0:nl-1,0:*),npts,mnpts,index(0:*)
...
C Local Variables
     ...
     .               e1,e2,df)
          if (dabs(e2 - e1) < d1mach(3)) then
            Nbar(m) = df
            emag(m) = 0
              cycle
          endif
          if (e2 < e1) call fexit(-1,111,
...
C --- find global minimum in emag and get moment ---
        ...
        enddo
        if(j == mnpts .and. iprint() > 10) call awrit3
...
C --- find bin containing Fermi level ---
      ...
      if (nf-ndown < 5 .or. nup-nf < 6) then
        ekin = 0
        return
      endif
