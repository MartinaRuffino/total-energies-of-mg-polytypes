...
Ci   b     :lattice parameter, required unless isym1 > 3
Ci         :                   (triclinic,monoclinic,orthorhombic)
Ci   c     :lattice parameter, required unless isym1 > 6 or
Ci         :                   isym1 == 5.and.isym3 == 1
...
C Local variables:
     ...
     .                 tb(9,7),parm,vol,wk
      parameter(pi=3.14159265358979324d0,facdeg=180d0/pi,parm=1.d-4)
C External calls:
...
C ----- triple hexagonal cell
        c1 = a1*dsqrt(3d0+6d0*dcos(al))
        a1 = a1*dsqrt(2d0-2d0*dcos(al))
        isym3 = 7
      endif
      if (isym1 > 3) b1 = a1
      if (isym1 > 6) c1 = a1
      if (isym1 > 1) al = 90d0/facdeg
      if (isym1 > 2) be = 90d0/facdeg
      if (isym1 > 1) ga = 90d0/facdeg
      if (isym1 == 5.or.isym1 == 6) ga  = 120d0/facdeg

C --- Sanity checks ---
      call rxx(a1 <= 0,'roplat: unspecified  `a'' lattice parameter')
      call rxx(b1 <= 0,'roplat: unspecified  `b'' lattice parameter')
      call rxx(c1 <= 0,'roplat: unspecified  `c'' lattice parameter')
      call rxx(al <= 0,'roplat: unspecified angle `alpha''')
      call rxx(be <= 0,'roplat: unspecified angle `beta''')
      call rxx(ga <= 0,'roplat: unspecified angle `gamma''')

C --- Choose b as length unit
      alat = b1
      if (isym1 == 5) alat = alat/dsqrt(3d0)
     ...
     .  call rx('LATPAR: bad lattice parameters')
      ...
      wk = 1d0-dcos(al)**2-dcos(be)**2-dcos(ga)**2
     .    +2d0*dcos(al)*dcos(be)*dcos(ga)
...
C Statement functions:
     ...
     .  (1d0/dsqrt(ddot(3,qlatin(1,m),1,qlatin(1,m),1)),m=1,3)
...
c             if ((pr1 < prn1-tollat).or.
c    .            (pr1*pr2 < prn1*prn2-d1mach(3))) then
...
C --- Printout
     ...
     .    prn1,prn2,((platcp(m,k),m=1,3),(qlatcp(m,k),m=1,3),k=1,3)
        if (iprint() > 50) write(stdo,303)
     .    (1d0/dsqrt(ddot(3,qlatcp(1,m),1,qlatcp(1,m),1)),m=1,3)

      else
        if (iprint() >= 40) write(stdo,302)
C        if (iprint() >= 50) write(stdo,301)
