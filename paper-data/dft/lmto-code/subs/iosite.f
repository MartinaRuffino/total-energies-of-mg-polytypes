     ...
     .  nbas,nspec,pos,vel,eula,vshft,ips,ipl,irlx)
...
Ci           4    file nbas must match passed value, if it is read
...
Ci        1000s to 100000s digit: bits indicate what info is read in
Ci           1    I/O alat
Ci                ... alat is optional, if sought; no value of alat
Ci                    is assigned if missing (standard format)
Ci           2    I/O plat required, if sought
Ci           4    I/O nbas read (and required).  
Ci                    If this switch is NOT set, you must supply nbas as input
Ci           8    I/O species, pos information (pos,ips)
Ci                    If bit is not set, pos,ips arrays untouched
Ci                ... these data are optional, if sought, that is
Ci                    on file read, arrays are assigned to zero if
Ci                    missing from file data.
Ci            ... (version 3)
Ci          16    1 return iosite = local file contents index:
Ci                index = 1000s to 100000s digit of lio, as stored in site file
...
Ci         128    (read only) Counts species and loads species labels
Ci                If this switch is set, switches 8-64 are ignored
Ci         256    (read only) Counts nbas
Ci                If this switch is set, switches 1-128 are ignored
Ci            ... (version 2)
...
Cu Updates
Cu   13 Mar 19 New ability to count nbas from file entries
Cu   01 Aug 15 New ability to count species and load slabl (lio345>=128)
...
C Local variables
     ...
     .  vshftl,delta(6),vnx,plati(3,3)
      logical bittst,ltmp,parstr,a2bin,lfast,lxpos,rdstrn
      integer a2vec,ipr,lio0,lio12,lio345,llio345,fopnx,ib,is,
     .  irlxl(3),ipll,lgunit,i,j,nbasl,ix(9),iirlx,j1,j2,getdig,bitand,
     .  isw,mpipid
...
C ... Setup
      ...
      if (bittst(lio345,256)) lio345 = lio345 - mod(lio345,256) ! ignore switches 1-128
C     Remake convention for lio345 if input is earlier than v2
      if (vn < 3) then
        lio345 = mod(lio345,16) + 32*(lio345/16)
      endif
      if (procid == master) then
        if (.not. bittst(lio0,2)) then
C          call rxx(fxst(filnam) /= 1,
C     .    'iosite: file '//trim(filnam)//' does not exist')
          ifi = fopnx(filnam,100,16+8+0,-1)
        endif
...
C         Write tbe data
          if (bittst(lio345,64)) then
            call rx('iosite: fix this branch')
            call dpzero(delta,6)
            call awrit1(' tbe:%9p%6:1;9F',' ',80,ifi,delta)
          endif
        enddo

        call info2(30,0,0,' IOSITE: wrote to file '''//
...
C           For now, sites must be ordered
          ...
          if (bittst(lio12,4))
     .      call sanrg(.true.,nbasl,nbas,nbas,'iosite:','file''s nbas')
...
C         Column 3 is species index
          spid = ' '
          read(a(j1:j2),'(a)',err=98,end=98) spid(1:min(j2-j1+1,len(spid)))
...
C   ... Read version and check compatibility with calling program
        ...
        call sanrg(.true.,int(vnx),2,3,'IOSITE:','file''s version number')
C   ... Read lio345
        i = 0
        readin = ' "io" '
...
C       Remake convention for llio345 if file is v2 but ask v3
        if (vnx < 3 .and. vn >= 3) then
          llio345 = mod(llio345,16) + 32*(llio345/16)
        endif
C   ... Read nbas
        if (bittst(lio12,4) .or. bittst(lio345,4) .or. bittst(lio345,256)) then
          i = 0
          readin = '"nbas" '
          ltmp = parstr(a,'nbas=',len(a)-5,5,' ',i,j)
          if (ltmp) then
            i = j-1
            if (.not. a2bin(a,nbasl,2,0,' ',i,len(a)-5)) goto 99
            if (bittst(lio345,4) .or. bittst(lio345,256)) nbas = nbasl
            if (bittst(lio12,4)) call sanrg(.true.,nbasl,nbas,nbas,'iosite:','file''s nbas')
            if (bittst(lio345,256)) goto 999
          else  ! nbas is missing from input
            if (bittst(lio345,4)) goto 99 ! it is required
            if (bittst(lio345,256)) then  ! iosite will determine nbas from line count
              nbas = 0
              goto 42
            endif
          endif
!        if ( bittst(lio12,4)) continue   ! The last possiblity is innocuous
        endif
        if (nbas <= 0) goto 99
C   ... Read alat (optional if sought), plat (reqd if sought)
   ...
   42   continue ! Re-entry point for lio345=256 : Special mode to return nbas only
...
C         Read another line and exit if file doesn't contain enough data
    ...
    5       continue
            ltmp = rdstrn(ifi,a,len(a),.false.)
            nr = nr+1
            if (ltmp .and. a(1:1) == '#' .or. a == ' ' .and. ib < nbas) goto 5
            if (lio345 >= 64 .and. lio <= 128) then
    6         continue
            ...
            do   ! do loop is used to skip blank lines
            call rdfiln(ifi,rdarg,mxlev,loop0,nlin,list,lstsiz,
     .        ilist,nlist,vnam,ctbl,mxchr,a,recrd,recl,nr)
            ltmp =  nr > 0
            if (ltmp .and. lio345 >= 64 .and. lio <= 128)
     .        call rdfiln(ifi,rdarg,mxlev,loop0,nlin,list,lstsiz,
     .        ilist,nlist,vnam,ctbl,mxchr,aa,recrd,recl,nr)
            ltmp =  nr > 0
            if (len_trim(a) > 0 .or. nr < 0) exit
            enddo
C           print *, 'nr,a', ltmp, nr, ib, len(a), len_trim(a)
          endif

C     ... Special mode: count nbas
          if (bittst(lio345,256)) then
            if (nr < 0) goto 999
            nbas = nbas+1
            goto 4
          endif
...
C         Special mode: reading species table and skip to next atom
          if (is <= 0 .and. lio345 >= 128) then
            nspec = nspec+1
            slabl(nspec) = spid
          endif
          if (lio345 >= 128) then
...
C         No vel,eula,vshft,ipl,irlx available on disk
          if (llio345 < 32) then
            ...
            if (lfast) then
              ...
              i = 0
              if (a2vec(a(j2+1:),len(a)-j2,i,4,', ',2,3,3,ix,posl) /= 3) goto 98
            endif
C         vel,eula,vshft,ipl,irlx are available on disk
     ...
     .        posl,vell,eulal,vshftl,ipll,iirlx
            ...
            i = 0
            if (a2vec(a(j2+1:),len(a)-j2,i,4,', ',2,3,3,ix,posl) /= 3) goto 98
            if (a2vec(a(j2+1:),len(a)-j2,i,4,', ',2,3,3,ix,vell) /= 3) goto 98
            if (a2vec(a(j2+1:),len(a)-j2,i,4,', ',2,3,3,ix,eulal) /= 3) goto 98
            if (a2vec(a(j2+1:),len(a)-j2,i,4,', ',2,3,1,ix,vshftl) /= 1) goto 98
            if (a2vec(a(j2+1:),len(a)-j2,i,2,', ',2,3,1,ix,ipll) /= 1) goto 98
            if (a2vec(a(j2+1:),len(a)-j2,i,2,', ',2,3,1,ix,iirlx) /= 1) goto 98
          endif
          if (lxpos) then
            call dcopy(3,posl,1,plx,1)
...
C ... Error handling
   ...
   99 continue
C     Copy llio345
C     if (bittst(lio345,16)) lio = llio345
  ...
  369 format(1x,'IOSITE ',a,' failed to read ',a,' from file ',a)
      if (errh == 0) call rxs('IOSITE: file mismatch, file ',filnam)

      end
      integer function ioibmap(ifi,nbas,imap)
C- Read/write map in site file, if it can be found
      implicit none
      integer ifi,nbas,imap(nbas)
C ... Local parameters
      integer, allocatable :: kv(:)
      integer i,j,n,nw,a2vec
C ... These are for rdfiln
      integer recl,nr,mxchr,mxlev,lstsiz,ctlen
      parameter (mxchr=20,mxlev=4,lstsiz=200,recl=500,ctlen=120)
      character recrd*(recl),ctbl(mxchr,2)*(ctlen),a*(recl),vnam(mxlev)*16,rdarg*6
      logical loop0(0:mxlev)
      integer nlin(0:mxlev),list(lstsiz,mxlev),ilist(mxlev),nlist(0:mxlev)
      data rdarg /' {}% c'/
   ...
   10 continue
     ...
     .  ilist,nlist,vnam,ctbl,mxchr,a,recrd,recl,nr)
      if (nr <= 0) return
      if (a(1:4) /= '#map') goto 10

      call words(a,nw)
      nw = nw-1
      i = 5
      j = a2vec(a,len(a),i,2,', ',2,3,nw,kv,imap(1+n))
      if (j /= nw) call rx1('ioibmap: failed to read %i elements from file',nw)
      n = n+j
      ioibmap = n
      if (n < nbas) goto 10
      deallocate(kv)
