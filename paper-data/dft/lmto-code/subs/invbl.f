...
C Intrinsic functions
      ...
      call rxx(nsp /= 1,'INVBL: not set up for spin')
...
C Passed Parameters
      ...
      double precision plat(3,3),pos(3,nbas),alat,dclabl(*),
...
C .. get onsite strux
      ...
      do isite = 1, nsite
        ...
        do  lmb = 1, nl**2
          do  lma = 1, nl**2
            if (dabs(hrs(lma,lmb,isite)) > 1d-6) then
              outs = ' '
              ...
              if (lasa) then
                call strcop(outs(j+2:50),orb1(lmb),8,' ',i)
              else
                call strcop(outs(j+2:50),orb2(lmb),8,' ',i)
              endif
              j = j + 2 + i
              ...
              i = j
              if (lasa) then
                call strcop(outs(i+2:50),orb1(lma),8,' ',i)
              else
                call strcop(outs(i+2:50),orb2(lma),8,' ',i)
              endif
              i = i + j + 2
              if (lasa) then
                dsd = srx(lma,lmb,isite)
                h1 = dsd
                if (onsite .and. lmb == lma) then
                  h1 = c(ica,lma) + dsd
                endif
...
C     .              + 2d0*cmenu(icb,lmb)*ob*enu(icb,lmb)
     ...
     .                vec(4),lmb,lma,hrs(lma,lmb,isite),h1,h2,
