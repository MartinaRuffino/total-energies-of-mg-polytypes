...
C ----------------------------------------------------------------------
      implicit none
C ... Passed parameters
      integer opt,ldU,iprmb(*),ldmpa,ldima,ib1,ib2
      double precision rotm(3,3),U(ldU,ldU)
      double complex Uz(ldU,ldU)
C ... Local parameters
      integer n0,nkap0,nlx,nl2,i,ibas,iorb,j,li,nlm,nlmi,norb,offri,offi,morder
      parameter (n0=10,nkap0=4,nlx=9,nl2=nlx*nlx)
...
C     call prmx('rothrm: rYL',rYL,nl2,nl2,nl2)
      ...
      if (opt == 1) then
        call ylmrtg(nl2,rotm,rYL)
        call s2sph(1+100*morder,nlx,nlx,rYL,nl2,nl2,nl2,nl2,rYLz)
...
Ci   q     :k-point for current h, in units 2*pi/alat
Ci   tbas  :set of basis vector shifts, in units of alat; see Remarks
Ci   ldima :points to last orbital in  downfolding block of h to rotate
Ci   iprmb :permutations ordering orbitals in l+i+h blocks (makidx.f)
...
C ----------------------------------------------------------------------
      implicit none
C ... Passed parameters
      integer opt,ldH,iprmb(*),ldima,ib1,ib2
...
C ... Local parameters
      ...
      parameter (n0=10,nkap0=4,PRDBG=80)
      integer ltab(n0*nkap0),ktab(n0*nkap0),offl(n0*nkap0)

      if (q(1) == 0 .and. q(2) == 0 .and. q(3) == 0) return

      twopi = 8d0*datan(1d0)
      nphase = .false.
      call getpr(ipr)
      if (mod(opt/10,10) /= 0) nphase = .true.
...
C   --- For all orbitals belonging to site ib, h->phase*h and/or h->h*phase+
        if (mod(opt,10) == 0 .or. mod(opt,10) == 2) then
          do  j = j1, j2
        ...
        if (mod(opt,10) == 1 .or. mod(opt,10) == 2) then
          phase = dconjg(phase)
          do  i = off+1, off+ndimi
            do  j = j1, j2
              h(j,i) = h(j,i)*phase
