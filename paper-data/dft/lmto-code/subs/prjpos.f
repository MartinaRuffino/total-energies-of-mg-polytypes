...
C ... Case 10s digit mode is >=4
    ...
    4     continue
     ...
     .         pos2(3,ib)*dl2(3,n)
          lshrink = a2 + 2*ap < tol
          lcnst = pos2(1,ib) + dl2(1,n) >= 0d0
     .      .and. pos2(2,ib) + dl2(2,n) >= 0d0
     .      .and. pos2(3,ib) + dl2(3,n) >= 0d0
...
CC ... compare with: mc out.dat posplat2 -- -t
...
CC ... compare with: mc plat -i plat2 out.dat -x pos -- -x -t
...
CC ... check: for above, rename out.dat as out and then:
CC     compare with: mc out.dat out -- -t
C
C      call prjpos(0,ix,plat,plat2,48,pos,pos2)
