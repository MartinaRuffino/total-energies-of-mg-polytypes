...
C Local parameters
      integer k,iprint,m,modeg(3),isw,stdo,lgunit
...
C      write (stdo,350) ((plat0(m,k),m=1,3),(qlat0(m,k),m=1,3),k=1,3)
C  350 format(3f11.6,5x,3f11.6)
      do  k = 1, 3
        call info2(2,0,0,'%3;11,6D     %3;11,6D',plat0(1,k),qlat0(1,k))
      enddo
      if (dabs(gt-1d0) > 1d-5) then
C       call awrit4(' Distorted with gx,y,z= %d %d %d  gt=%d:',' ',80,i1mach(2),g1,g2,g3,gt)
        call info5(2,0,0,' Distorted with gx,y,z= %d %d %d  gt=%d:',g1,g2,g3,gt,0)
C        write(stdo,350) ((plat(m,k),m=1,3),(qlat(m,k),m=1,3),k=1,3)
        do  k = 1, 3
          call info2(2,0,0,'%3;11,6D     %3;11,6D',plat(1,k),qlat(1,k))
        enddo
        call dpzero(one,9)
        one(1,1) = 1
        one(2,2) = 1
  ...
  352   format(t14,'shear matrix'/(3f11.7))
      ...
      call info5(20,0,0,'   alat = %;6d  Cell vol = %1,6;6d'//
     .  '%?#n# (undistorted vol = %1,6;6d)#%j#',
     .  alat,vol,isw(dabs(vol-vol0*(alat**3)) > 1d-9),vol0*(alat**3),0)
   20 continue
...
C --- Printout ---
      ...
      call info5(30,1,0,' LATTC:  as=%;6,3D   tol=%,3e   alat=%;8,5D   awald=%;6,3D   lmax=%i',
     .  as,tol,alat,awald,lmax)
      call info5(30,0,0,'%9pr1=%;7,3D   nkd= %i%36pq1=%;7,3D   nkg= %i',r0+radd,nkd,q0+qadd,nkg,0)
C 340 format(/' LATTC:  as=',f6.3,'   tol=',1p,e8.2,'   alat=',0p,f8.5,'   awald=',f6.3)
C      write (stdo,340) as,tol,alat,awald
C      write (stdo,342) r0+radd,nkd,q0+qadd,nkg
C  342 format(9x,'r1=',f7.3,'   nkd=',i4,'      q1=',f7.3,'   nkg=',i4)
   60 if(dabs(alat0/alat-1d0) > 0.04d0)
...
!DMT: this is a bad way to type 1/sqrt(pi) ...
