...
C     call rfalsi(vnow,dznow,5d-8,0d0,5d-8,dxmx,10,vhold(1),ir)
      call rfalsi(vnow,dznow,5d-8,1d-7,5d-8,dxmx,30,vhold(1),ir)
C     Handle pathological case, e.g. insulator with integer moment.
C     where vnow may fall very near root but not be bracketed
      if (abs(dznow) < 1d-7) then
        ir = 0
      endif
C     print *, ir
      call poppr
