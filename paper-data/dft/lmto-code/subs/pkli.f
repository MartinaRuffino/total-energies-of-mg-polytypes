...
C --- Decomposition of pkl into simple polynomials ---
      if (max(km1,km2,km3) > k0) call rx('s3pkl: kmax gt k0')
      if (max(lm1,lm2,lm3) > l0) call rx('s3pkl: lmax gt l0')
...
C --- Decomposition of pkl into simple polynomials ---
      if (max(kmax,kmax3) > k0) call rx('s3pkl: kmax gt k0')
...
C --- Integrate the pkl pk'l product term by term ---
      do  l = 0, lmax
        ...
        do  k3 = 0, kmax3
          ...
          do  m3 = 0, k3
            qq = 2*(m1+m2+m3+l)+3
            sum = sum + (2d0/rsm**2)**(m1+m2)*(2d0/rsm3**2)**m3/qq
     .        *rmax**qq*aklm(m1,k1,l)*aklm(m2,k2,l)*aklm3(m3,k3,0)
          enddo
          enddo
          enddo
          s(k1,k2,k3,l) = sum
...
C --- Make f(m) = int r^(2m) exp(-a^2r^2) ---
      dfac = 1
      f(0) = srpi/(2*a)
      if (lmax+2*mmax+1 > 200) call rx('p2pkl: f too small')
      do  m = 1, (lmax+2*mmax+1)
        f(m) = (2*m-1)*f(m-1)/ta2
      enddo
