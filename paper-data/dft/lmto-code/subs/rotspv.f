...
Ci
Ci         :Styles are one of the following; they are described in Remarks
Ci         : *a. Standard format as linear combination of Pauli matrices
...
Ci         : *d. Permuted format: (1,2) block stored in v(:,3)
Ci
...
Ci         :6 rin (complex) in permuted noncollinear format (3 elements)
Ci
...
Ci         :0 u is a spinor rotation matrix
...
Ci
Ci         :1000s digit used in cases 100s digit mode = 4,5
...
Cr
Cr  *a. Standard spinor format stores the three components of the density as
...
Cr
Cr  *b, *c. Compressed spinor format is designed to handle collinear
...
Cr   Even if the input v is complex, its imaginary part is not used.
Cr
Cr  *d. Permuted spinor format also designed to handle collinear and
Cr   noncollinear cases in a single form.  Only for complex matrices.
Cr    v(1) = rho++  v(2) = rho--  v(3) = rho+- (complex)
Cr
...
C ... External calls
      external dcopy,rotspu,rxi,zdia22,zinv22
...
C     u passed to this routine is a set of 3 Euler angles
      case (6,7);
        ...
        call rotspu(0,1,1,1,1,eula,1,ul0)
...
C     rin (complex) in permuted format (noncollinear, 4 elements)
      case (6)
        do  i = 1, nr
          r11 = dble(rinc(i,1,1))
          r22 = dble(rinc(i,2,1))
...
C     routc is stored in permuted spinor format (3 elements)
      case (6)
        do  i = 1, nr
          routc(i,1,1) = rwk(i,1,1)*ul(1,1) + rwk(i,1,2)*ul(2,1)
          routc(i,2,1) = rwk(i,2,1)*ul(1,2) + rwk(i,2,2)*ul(2,2)
          cx(1,2) = rwk(i,1,1)*ul(1,2) + rwk(i,1,2)*ul(2,2)
          cx(2,1) = rwk(i,2,1)*ul(1,1) + rwk(i,2,2)*ul(2,1)
          routc(i,1,2) = (cx(1,2) + dconjg(cx(2,1)))/2
        enddo
...
C      call rotspu(0,1,1,1,1,(/pi/4,pi/2,0d0/),1,u)
