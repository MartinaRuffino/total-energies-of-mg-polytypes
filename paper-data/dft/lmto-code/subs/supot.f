...
C ... Local parameters
C#ifdefC FP
C      integer, allocatable :: ipca(:),istab(:,:)
C      real(8), allocatable :: g(:,:),ag(:,:)
C      integer ng,ngabc(3),n1,n2,n3,k1,k2,k3,ngmx
C      double precision gmax,plat(3,3)
...
C#ifdefC FP
CC --- Setup for FT charge density, potential representation ---
C      if (IAND(s_ctrl%lcd,4) /= 0) then
C        plat = s_lat%plat
C        ngabc = s_lat%nabc
C        gmax = s_lat%gmax
C
CC   ... Generate energy cutoff gmax or n1..n3
C        call pshpr(iprint()-0)
C        call mshsiz(s_lat%alat,plat,0,gmax,ngabc,ngmx)
C        call poppr
C        call fftz30(n1,n2,n3,k1,k2,k3)
C        s_lat%nabc = ngabc
C        s_lat%gmax = gmax
C        if (ngmx <= 0) call rx('supot: missing G vectors.  Check HAM_GMAX.')
C
CC   ... Make list of lattice vectors within cutoff
C        call sugvec0(s_lat)
C        ng = s_lat%ng
C        if (ng /= ngmx) call rx('supot: bug in gvlist')
C
C        i = idalloc('irho+v',allocvb()+2,k1*k2*k3,nsp)
C        call ptr_pot(s_pot,8+1,'smrho',k1*k2*k3,nsp*nspc,xx)
C        call ptr_pot(s_pot,8+1,'smpot',k1*k2*k3,nsp*nspc,xx)
CC       allocate(s_pot%rhat(nbaspp),s_pot%rnew(nbaspp))
C        call ptr_pot(s_pot,1,'rhat',nbaspp,1,xx)
C        call ptr_pot(s_pot,1,'rnew',nbaspp,1,xx)
C
CC   ... Setup for symmetrization and possible AFM symmetrization
C        i = 1; if (iabs(s_lat%nsafm) /= 0) i = 2
C        call ptr_lat(s_lat,1,'ips0',ng*i,0,0,0)
C        call ptr_lat(s_lat,1,'bgv',ng*i,0,0,0)
C        call sgvsym(s_lat%nsgrp,s_lat%symgr,s_lat%ag,ng,s_lat%gv,s_lat%ips0,s_lat%bgv)
CC   ... Setup for AFM symmetrization
C        if (iabs(s_lat%nsafm) /= 0) then
C          allocate(ipca(nbas),istab(nbas,2),g(9,2),ag(3,2))
C          call info0(20,0,0,'%9f... setup for AFM symmetrization')
C          call suafmsym(s_lat,nbas,ipca,istab,g,ag)
C          call sgvsym(-2,g,ag,ng,s_lat%gv,s_lat%ips0(1+ng),s_lat%bgv(1+ng))
C        endif
C      endif
C#endif
      end
