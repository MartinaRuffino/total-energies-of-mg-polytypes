...
C --- Begin loop over sets of BZ planes/energy windows ---
   10 continue
      read (nfilqp,*,err=999,end=999) nwin,np0
      call rxx(nwin < 1,'GETBZP: bad nwin')
      call rxx(nwin > nwmx,'GETBZP: nwin gt nwmx')
      call rxx(nptot+np0 > npln,'GETBZP: nptot gt npln')
      do  iw = 1, nwin
        read (nfilqp,*,err=999,end=999) (e(i,iw,nptot+1),i=1,2)
        call rxx(e(1,iw,nptot+1) > e(2,iw,nptot+1),'GETBZP: e1 gt e2')
      enddo

C --- Begin loop over BZ planes in this set ---
      do  ip = nptot+1, nptot+np0
        nw(ip) = nwin
        if (ip > nptot+1)
     ...
     .    (vy(i,ip),i=1,3),(y(i,ip),i=1,2),nq(2,ip),z(ip)
        call rxx(nq(1,ip) < 2,'GETBZP: bad n1')
        call rxx(nq(2,ip) < 2,'GETBZP: bad n2')
        nq0 = nq(1,ip)*nq(2,ip)
        nqtot = nqtot + nq0
        call rxx(nqtot > nqp,'GETBZP: nqtot gt nqp')

C --- Orthonormalize axes, construct normal vector ---
        rx = ddot(3,vx(1,ip),1,vx(1,ip),1)
        call rxx(dabs(rx) < 1.d-06,'GETBZP: bad vx')
        ...
        call rxx(dabs(ry) < 1.d-06,'GETBZP: vx and vy are colinear')
        ...
        do  iy = 1, nq(2,ip)
          ...
          do  ix = 1, nq(1,ip)
            ...
            do  i = 1, 3
              ...
              if (dabs(q(i,iq)) < 1d-12) q(i,iq) = 0d0
            enddo
            if (iprint() > 50) write (*,3) iq,(q(i,iq),i=1,3)
    3       format(i5,3F10.6)
          enddo
        enddo
      enddo
...
C --- Get next set ---
      ...
      goto 10
  999 continue
      call rxx(nqtot == 0,'GETBZP: empty or badly formed BZPL file')
      call rxx(nptot /= npln,'GETBZP: file mismatch, nptot ne npln')
      call rxx(nqtot /= nqp,'GETBZP: file mismatch, nqtot ne nqp')
