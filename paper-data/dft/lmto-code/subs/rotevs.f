...
Ci             U = Z^QP (Z^LDA)^-1
Ci         :3  Make QP hamiltonian from Z^LDA, evl, U
Ci         :5  Make effective static coulomb interaction n^-1 sigm
Ci             sigm is in the QP basis; n is diagonal.
Ci
...
Ci         :(mode 2) not used
Ci         :(mode 3) QP evals
Ci         :(mode 5) occupation number array
Cio Inputs/Outputs
...
Cio        :(mode 5,input)  self-energy in QP basis
...
Cio        :(mode 2,input)  density matrix
Cio        :(mode 5,input)  Z^QP = LDA  eigenvectors
Cio  h     :(mode 1) work array
Cio        :(mode 2,output) Z^QP (Z^LDA)^-1
Cio        :(mode 3) work array
Cio        :(mode 2,output) Z^-1 sigm
Cio        :(mode 5,output) (1/2-n) sigm
Cl Local variables
Cl         :
Cr Remarks
Cr
...
C       call zprm('Z_LDA^-1',2,Z,ndimz,ndimz,ndimz)
        call rxx(ierr < 0,'rotevs: failed to invert Z^LDA')
C
C       Step 2a
...
C        call rxx(ierr < 0,'rotevs: failed to invert Z^LDA')
...
C        call phmbls(32+64,ndimz,ndimz,evl,xx,Z,xx,h,h,sigm)
CC       call zprm('Hqp',12,sigm,ndimz,ndimz,ndimz)
C
CC     Mode 4:  Make sigma(orbital basis)
CC     Define U = Z^QP (Z^LDA)^-1
CC     Note: Hqp(LDA bas) =
C      elseif (mode == 4) then
C
C     Mode 5:  Make Z^-1 sigma
      elseif (mode == 5) then
C      call zprm('sigm',2,sigm,ndimz,ndimz,ndimz)
C      call zprm('Z',2,Z,ndimz,ndimz,ndimz)
       ...
       do  i = 1, ndimz
         do  j = 1, ndimz
           xx = 1/(0.5d0-evl(j))
           h(i,j) = sigm(i,j)/2 + sigm(i,j) * xx/2
         enddo
       enddo

C      call zprm('(1/2-0) sigma',2,h,ndimz,ndimz,ndimz)
      else
...
C   ... Return here to resume parsing for arguments
   ...
   50   continue
     ...
     .        dc//' ',1,1,i,lwevec) < 0) goto 999
            call sanrg(.true.,lwevec,0,2,'EVCFLG:','mode')
