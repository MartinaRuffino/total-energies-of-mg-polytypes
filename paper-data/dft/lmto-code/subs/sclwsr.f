...
Ci         :         is required:
Ci         :-1 no resizing done; printout only
...
Ci         : 3 Only sites with z=0 are resized
Ci         :100s digit
Ci         : 1 restore lock before returning
...
Cu   08 Sep 12  bug fix, occured in lmf-MPIK
...
C ... Local parameters
      integer i,k,nrspec(nspec),ib,is,opt1,opt2
      integer niax,mxcsiz,mxnbr,nttab,ipr,stdo,llock(nspec),locksave(nspec)
      logical les
      double precision facr,tiny,avw,vol,avwsr,volnew,volnes,volold,
     .  volsph,dovl1(3),dovl2(3),gamma,range(nbas),wsrs(nspec)
...
C --- Setup ---
      ...
      if (ib > nspec) call rx('sclwsr: wrong number of species')
      opt1 = mod(opt/10,10)
      opt2 = mod(opt/100,10)
      call dcopy(nspec,wsr,1,wsrs,1) ! Preserve starting wsr
      avw = avwsr(plat,alat,vol,nbas)
      if (opt2 /= 0) call icopy(nspec,lock,1,locksave,1)
...
C --- Make a neighbor table and adjust llock to freeze ES sites ---
      ...
      allocate(iax(niax*mxnbr)); call iinit(iax,niax*mxnbr)
      allocate(wk(3*mxnbr))
      nttab = mxnbr
      call pshpr(ipr-20)
      call pairc(1,nbas,nbasp,modep,20,0,alat,plat,basp,basp,range,-1,1,
     .  nttab,ntab,iax,wk,k)
      deallocate(wk)
      call poppr
...
C --- Scale the sphere radii, freezing all but empty spheres ---
      elseif (opt1 == 3) then
        do  is  = 1, nspec
          if (z(is) /= 0) lock(is) = 2
        enddo
...
C     ... Scale wsr with z=0 sites locked at wsr=0
          ...
          volnes = volfac
          call sclws2(nbas,nspec,alat,plat,basp,slabl,iax,ips,
     .      ntab,z,nrspec,omax1,omax2,gamma,wsmax,llock,volnes,
     .      dovl1,dovl2,wsr)
          if (ipr >= 30) then
...
C   ... 10s digit opt=2 : freeze wsr(Z>0) or floating sites
        if (opt1 >= 2 .and. les) then
          do  is  = 1, nspec
            if (z(is) /= 0 .or. wsr(is) == 0) lock(is) = 2
          enddo
        endif
...
C --- Scale the sphere radii (2nd pass for les) ---
      ...
      volnew = volfac
      call sclws2(nbas,nspec,alat,plat,basp,slabl,iax,ips,ntab,
     .  z,nrspec,omax1,omax2,gamma,wsmax,lock,volnew,dovl1,dovl2,wsr)

C --- Printout ---
C      if (ipr >= 10) write(stdo,309)
C     .  vol,100*volold,100*volnew
C  309 format(/' SCLWSR: vol=',f11.3,
C     .  '  sphere fraction=',f5.1,
C     .  '%(initial)  ',f5.1,'%(scaled)')

      call info5(15,0,0,' SCLWSR:  mode = %i  vol = %,5g a.u.%3f'//
     .  'Initial sphere packing = %;1d%%  '//
     .  'scaled to %;1d%%',opt,vol,100*volold,100*volnew,0)
  ...
  312       format(i4,3x,a,3f12.6)
      ...
      if (dabs(volnew-volfac) > tiny) then
        if (mod(opt,10) >= 1 .and. ipr >= 20) then
          write(stdo,321) int(volfac*100)
  321     format(/' SCLWSR (warning): failed to reach target vol (',i3,'% of cell vol)')
        endif
      ...
      deallocate(ntab,iax)
      if (opt2 /= 0) call icopy(nspec,locksave,1,lock,1)
...
C       fin=T if volume change is small or all spheres are locked
        fin = fin .or. dabs(ratio-volfac) < tiny .or. nloop == nspec+1
...
C              if (dovl1(ip) > omax1(ip)+tiny .or.
C     .            dovl2(ip) > omax2(ip)+tiny)
