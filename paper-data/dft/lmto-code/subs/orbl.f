...
Ci         :iprmb should be dimensioned iprmb(nl*nl*nkaph,1:ib)
Ci         :Default values for nl and nkaph are taken from nglob,
Ci         :but they can be set manually:   call setorblp(nl,nkaph)
Co Outputs
...
Cr   Each orbital corresponds to a unique radial partial wave at the
Cr   site where the orbital is centered.  There can be multiple 'k'
Cr   for a particular l.
Cr
...
C ----------------------------------------------------------------------
      implicit none
C ... Passed parameters
      integer ib,ldimp,ldim,iprmb(*),norb,ndim
      integer, parameter :: n0=10,nkap0=4
      integer off,ltab(n0*nkap0),ktab(n0*nkap0),offl(n0*nkap0)
C ... Local parameters
      integer ik,l,lmr
      integer ltabi(n0*nkap0),ktabi(n0*nkap0),offli(n0*nkap0+1),iwk(n0*nkap0)
      integer :: nl=-1,nkaph=-1
      procedure(integer) :: nglob

C     if (mxorb == -1) mxorb = nglob('mxorb')
      if (nl == -1) nl = nglob('nl')
...
C     lmr = mxorb*(ib-1)
      ...
      do  l = 0, nl-1
        offli(norb+1) = -1
        if (iprmb(lmr+1) > ldimp .and. iprmb(lmr+1) <= ldim) then
          offli(norb+1) = iprmb(lmr+1) - 1
          if (off == -1) off = offli(norb+1)
...
C --- Permute to downfolding order ---
      if (norb > n0*nkap0) call rx('orbl: norb > n0')
...
C     .  '  offl=%n:1i',' ',80,6,ib,ndim,norb,ltab,norb,offl)
      ...
      entry setorblp(ldimp,ldim)  ! ldimp, ldim are new values for nl,nkaph
...
C ----------------------------------------------------------------------
      ...
      integer mode,ib1,ib2,nbas,indxsh(*),indxi(*),
     .  offh(n0H,nkap0,nbas+1)
      integer offl(*),norb
      integer hdimg,hdimi,horbi,idimg,idimi,iorbi,ldimg,ldimi,lhdimg,
     .  lidimg,lmr,lorbi,mxorb,nlmr,offlm,ib,i,nlm,
...
C ... Make indxi
      if (mod(mode,10) == 1) then
        ...
        do  lmr = 1, nlmr
          if (indxsh(lmr+offlm) <= ldimg) then
            lorbi = lorbi+1
            indxi(lmr) = lorbi
          else if (indxsh(lmr+offlm) <= lidimg) then
            iorbi = iorbi+1
            indxi(lmr) = iorbi
          else if (indxsh(lmr+offlm) <= lhdimg) then
            horbi = horbi+1
            indxi(lmr) = horbi
          else
            norbi = norbi+1
...
C ... Local parameters
      ...
      if (mxorb == -1) mxorb = nglob('mxorb')
      ld = 0

C ... Brute force (no assumption about site ordering)
      if (lfast == 0 .or. lfast < 2 .and. ib1+1 >= ib2) then
        if (lfast == 2) return
...
C  which satisfy ilow<=idx(1)<=ihigh
      ...
      ix = 1
      do  i = 1, n
        if (ilow <= idx(ix) .and. idx(ix) <= ihigh) then
...
C      lmr = mxorb*(ib-2)
C      do  10  l = 0, nl-1
C        if (iprmb(lmr+1) > ldimp .and. iprmb(lmr+1) <= ldim) then
C          if (off == -1) off = iprmb(lmr+1) - 1
C          ndim = ndim + 2*l+1
C        endif
C        lmr = lmr + 2*l+1
C   10 continue
