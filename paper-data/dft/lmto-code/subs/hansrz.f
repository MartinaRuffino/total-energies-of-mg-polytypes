      subroutine hansrz(rsmh,lmin,lmax,e,rsq,nrx,nr,job,xi,fi)
C- Vector of smoothed Hankel functions, any e
C ----------------------------------------------------------------------
Ci Inputs
Ci   rsmh    : vector of l-dependent smoothing radii of Hankels
Ci           : EITHER must be specified for lmin..lmax
Ci           : OR     rsmh(lmin) < 0. Implies rsmh(l) = -const for all l
...
Cr Remarks
Cr  *Scaling of xi and fi.
Cr   If 1s digit job is 0, xi and fi are the radial part/r**l, so the solid Hankel is
Cr   hl(ilm) = xi(l)*Yl(ilm)
Cr   where the Yl are r^l * (spherical harmonics)
Cr   If 1s digit job is nonzero xi and fi are the radial parts proper.
Cr
...
C     Negative 1st smoothing radius => constant rsm
      ...
      n2p = nr + 1
      do  ill = lmax, lmin, -1
        if (.not. lrsm0) rsm = rsmh(ill)
        if (dabs(rsm-rsx) < tol) goto 60
        rsx = rsm
...
C     ... Power series for points within rc1
          if (n1 > 0) call hansz1(rsq(1),lmin,llmax,nrx,n1-1,e,rsm,
     .      dsqrt(rc1),whan(1,lmin))
C     ... Normal evaluation of smoothed Hankels
          if (n2 > n1) call hansz2(jerf,rsq(n1),lmin,llmax,nrx,n2-n1,e,rsm,gs(n1),
     .      wk(n1),whan(n1,lmin))
...
Cr
Cr  *Case e=0.
Cr   xi(-1) for e=0 is infinite.
Cr   This routine merely returns a large number for xi(-1) if lmin is -1
C ---------------------------------------------------------------
      implicit none
      integer nrx,nr,lmin,lmax
      double precision rsq(nrx),e,xi(nrx,lmin:lmax),rsm,rmax
C Local variables
      integer, parameter :: nmax=40,nm1=14,nm2=20
      real(8), parameter :: rzero = 1d-40, tol=1d-20
      double precision cof0(-1:20),cofl(0:nmax)
      double precision a,a2,add,akap,cc,fac,rhs,ta,ta2l,y0,r2max,x
      integer i,il,l,ir,m,nmaxl
      logical lpos
      procedure(complex(8)) :: zerfc
      procedure(real(8)) :: derfc
...
c 333   format(' hansz1:  not converged to tol=',1pe8.1)
   ...
   24   continue
        do  53  ir = 1, nr
   53   xi(ir,l) = cofl(nmaxl)
        do  54  m = nmaxl, 1, -1
        do  54  ir = 1, nr
   54   xi(ir,l) = xi(ir,l)*rsq(ir) + cofl(m-1)
        endif
        endif
        ta2l = ta2l*(ta*a)
   20 continue
...
C ---------------------------------------------------------------
Ci Inputs
Ci   jerf  : 1 - approximate erfc as a ratio of polynomials (e < 0)
...
C ... If the mesh starts in the origin, set xi(1) to lim xi(r -> 0)
      n1 = 1
      if (rsq(1) < rzero) then
        n1 = 2
...
C   ... am1 = xi(-1) = (vm + vp) * i/(2*akap)
            if (.not. lzero) am1 = -dimag(vm + vp) * 0.5d0/akap
C   ... xi(0) = (vm - vp)/(2*r)
...
C   ... am1 = xi(-1) = (um + up)/(2*akap)
            if (.not. lzero) am1 = (um + up) * 0.5d0/akap
C   ... xi(0) = (um - up)/(2*r)
...
C --- xi(ir,l) for l>1 by upward recursion ---
      ...
      facgl = a2
        do  il = lmin+2, lmax
          xx = 2*il-1
          do  ir = n1, nr
            xi(ir,il) = (xx*xi(ir,il-1) - e*xi(ir,il-2) - wk2(ir)) / rsq(ir)
            wk2(ir) = facgl*wk2(ir)
          enddo
        enddo
      endif
