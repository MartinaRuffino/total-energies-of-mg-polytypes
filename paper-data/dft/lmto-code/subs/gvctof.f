...
Ci              3 use cutoff for largest vector
...
C --- Get shortest mesh pts where ki /= 0, gmax is pi/(longest) ---
      h1 = 1d10
      h2 = 1d10
      h3 = 1d10
      do  k1 = -5, 5
      ...
      if (ipr >= ipr0) call awrit6('%N GVCTOF: mesh has '//
     .  ' %i * %i * %i divisions  (length = %,3;3d * %,3;3d * %,3;3d)',
     .  ' ',80,stdo,n1,n2,n3,h1,h2,h3)
      voln = (4*pi/3)*gmax**3
      ipv = int(100*voln/volgs)
      if (ipr >= ipr1) write (stdo,1) gmax,voln,ipv
    1 format('   Nyquist cutoff pi/h    ',f10.3,'    (volume',f10.2,
     .        ',',i4,'%)')
...
C --- Alternative: non-overlapping spheres on recip superlattice ---
      ...
      if (ipr >= ipr1) write (stdo,2) gmax1,vol1,ipv
    2 format('   largest sphere within BZ ',f8.3,'    (volume',f10.2,',',i4,'%)')

      if (ipr >= ipr1) write (stdo,3) gvol,volgs,100
    3 format('   cutoff for largest vector',f8.3,'    (volume',f10.2,',',i4,'%)')
...
C        if (abs(gs(1)+7) < tol .and. abs(gs(2)+1) < tol) then
C          print *, gs
C        endif
        gg = (tpiba*tpiba)*(gs(1)**2+gs(2)**2+gs(3)**2)
        if (gg <= gmax2) nv2 = nv2+1
        nv1 = nv1+1
      enddo
      enddo
      enddo
      if (ipr >= ipr0) call awrit3(' Reciprocal lattice: use sphere'//
