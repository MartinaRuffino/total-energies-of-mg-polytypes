...
Cu Updates
Cu   07 Nov 01 Adapted from Stuttgart lmto56
C ----------------------------------------------------------------------
      implicit none
C Passed variables:
      integer isym(*)
...
C --- set isym1 depending on number of valid symmetry operations
      isym1=0
      do  is1 = 1, 7
        if (nsymop == nsym0(is1))  isym1=is1
      enddo
      if (iprint() >= 100) write(stdo,300) nsymop

      if (isym1 == 0) call rx('BRAVSY: bad nsymop')
...
C Local variables:
     ...
     .  m,mn,n(7),nkd,isym32(5),isym33(6),isym34(2),isym37(3),stdo
      ...
      double precision cosvec,danrm2,d1mach,d2(ll1**3),ddd,ddot,det33,
...
C Statement functions:
     ...
     .         -mod(ikd+(ll1/2)*ll1**(m-1),ll1**(m-1)))/ll1**(m-1)-ltmax
        ...
        ddd(i,j)=ddot(3,dlat  (1,i),1,dlat  (1,j),1) ! dlat(i) . dlat(j)
      lra1(i,j)=dabs(pdp(i,j)) < tollat
      lsl1(i,j)=dabs(pdp(i,i)-pdp(j,j)) < tollat
      lra2(i,j)=dabs(ddd(i,j)) < tollat
      lsl2(i,j)=dabs(ddd(i,i)-ddd(j,j)) < tollat
      lsma1(i,k,j)=dabs(pdp(i,j)-pdp(j,k)) < tollat.and.
     .             dabs(pdp(j,k)-pdp(k,i)) < tollat
      lsma2(ikd1,ikd2,ikd3)=
     .          dabs(ddd(ikd1,ikd2)-ddd(ikd2,ikd3)) < tollat.and.
     .          dabs(ddd(ikd2,ikd3)-ddd(ikd3,ikd1)) < tollat
C Data statements:
...
C ----- triclinic
        ...
        call dvprm(3,3,platcv,platwk,iprm,1)
C       call bsort2(platcv)
        call dswap(3,platcv(1,1),1,platcv(1,2),1)
        call dswap(3,platcv(1,2),1,platcv(1,3),1)
        if (pdp(2,3) > 0.d0) call dscal(3,-1.d0,platcv(1,3),1)
        if (pdp(1,3) > 0.d0) call dscal(3,-1.d0,platcv(1,1),1)
        if (det33(platcv) < 0.d0) call dscal(9,-1.d0,platcv,1)
...
C ----- No standard choice  -> b-unique axis; fully reduced mesh
        do  i1 = 1, nkd
          do  i2 = i1+1, nkd
            if (lra2(i1,i2).and.d2(i1)*d2(i2) < dm2) then
              ikd1=i1
              ikd2=i2
              dm2=d2(i1)*d2(i2)
            endif
...
C ----- now dlat(1) and dlat(2) are the shortest orthogonal latice vec's
        do  ikd3 = 1, nkd
         ...
         if (idnint(volcv/volin) >= 1) then
         if (lra2(ikd3,ikd1))call dswap(3,dlat(1,ikd1),1,dlat(1,ikd2),1)
          if (lra2(ikd2,ikd3).and.d2(ikd1)*d2(ikd2)*d2(ikd3) < dm3)then
     ...
     .      call dswap(3,platcv(1,1),1,platcv(1,3),1)
        if (pdp(1,3) > 0.d0)      call dscal(3,-1.d0,platcv(1,3),1)
        if (det33(platcv) < 0.d0) call dscal(3,-1.d0,platcv(1,2),1)

C       No standard choice of unit cell ... find most orthogonal
        if (det33(platcv) == 0d0) then
          call cross(dlat(1,ikd1),dlat(1,ikd2),r)
          call dscal(3,1/sqrt(ddot(3,r,1,r,1)),r,1)
          call dcopy(3,dlat(1,ikd1),1,platcv(1,1),1)
          call dcopy(3,dlat(1,ikd2),1,platcv(1,2),1)
          dm3 = 0
          do  ikd3 = ikd2+1, nkd
            if (abs(ddot(3,dlat(1,ikd3),1,r,1)/sqrt(d2(ikd3))) > dm3) then
              call dcopy(3,dlat(1,ikd3),1,platcv(1,3),1)
              dm3 = abs(ddot(3,dlat(1,ikd3),1,r,1)/sqrt(d2(ikd3)))
            endif
          enddo
          warning = ' (approximate):'
        endif
...
C ----- No standard choice of unit cell
     ...
     .           d2(ikd1)*d2(ikd2)*d2(ikd3) < dm3)then
                call dcopy(3,dlat(1,ikd1),1,platcv(1,1),1)
                call dcopy(3,dlat(1,ikd2),1,platcv(1,2),1)
                call dcopy(3,dlat(1,ikd3),1,platcv(1,3),1)
                dm3=d2(ikd1)*d2(ikd2)*d2(ikd3)
            ...
            if (lsl2(ikd1,ikd2)) then
              ...
              do  ikd3 = ikd2+1, nkd
                if (lsl2(ikd2,ikd3).and.lsma2(ikd1,ikd2,ikd3)) then
                  volcv=dabs(ddot(3,dlat(1,ikd3),1,r,1))
                  if (idnint(volcv/volin) == 1) then
                    if (d2(ikd1)*d2(ikd2)*d2(ikd3) < dm3) then
                      call dcopy(3,dlat(1,ikd1),1,platcv(1,1),1)
                      call dcopy(3,dlat(1,ikd2),1,platcv(1,2),1)
                      call dcopy(3,dlat(1,ikd3),1,platcv(1,3),1)
                      dm3=d2(ikd1)*d2(ikd2)*d2(ikd3)
...
C ----- hexagonal
        ...
        if (pdp(1,2) > 0.d0)call daxpy(3,-1.d0,platcv,1,platcv(1,2),1)
        if (det33(platcv) < 0.d0)call dswap(3,platcv,1,platcv(1,2),1)
        ...
        do  ikd1 = 1, nkd
          do  ikd2 = ikd1+1, nkd
           if (lra2(ikd1,ikd2).and.lsl2(ikd1,ikd2)) then
              do  ikd3 = ikd2+1, nkd
                if (lra2(ikd2,ikd3).and.lra2(ikd3,ikd1).and.
     .       lsl2(ikd2,ikd3).and.d2(ikd1)*d2(ikd2)*d2(ikd3) < dm3) then
                  call dcopy(3,dlat(1,ikd1),1,platcv(1,1),1)
                  call dcopy(3,dlat(1,ikd2),1,platcv(1,2),1)
                  call dcopy(3,dlat(1,ikd3),1,platcv(1,3),1)
                  dm3=d2(ikd1)*d2(ikd2)*d2(ikd3)
...
C       call bsort2(platcv)
    ...
    8 volcv=det33(platcv)
      if (volcv < 0.d0) call dscal(3,-1.d0,platcv(1,3),1)
      if (idnint(dabs(volcv/volin)) < 1)
     .  call rx(' CVPLAT: could not calculate platcv')

      call dinv33(platcv,1,qlatcv,volcv)
      if (iprint() >= 50) write(stdo,301), trim(warning),
     .    ((platcv(m,l),m=1,3),(qlatcv(m,l),m=1,3),l=1,3)
      if (iprint() >= 60) write(stdo,303)
     .  ((cosvec(platcv(1,i),platcv(1,j)),i=1,3),j=1,3)

  301 format(/' CVPLAT: conventional unit cell',a,
...
C External calls:
      ...
      if (iprint() >= 30) write(stdo,300) a,b,c,alpha,beta,gamma
...
C Local variables:
      ...
      double precision d1mach,det33,qlatst(3,3),qlatcp(3,3),tb(9,7),
     .                 vol,volst
      logical latvec
C ... External calls
...
C Data statements:
     ...
     .          12,0,0,0,13,14,0/
      ...
      stdo = lgunit(1)
      if (isym(1) < 1.or.isym(1) > 7) call rx(' STPLAT: bad isym(1)')
      if (dabs(det33(platcp)) < d1mach(3))
     .  call rx('STPLAT: bad platcp')

      call dinv33(platcp,1,qlatcp,vol)
      do  isym3 = 7, 1,-1
        if (isym2(isym3,isym(1)) /= 0) then
    ...
    1 isym(3)=isym3
      ...
      if (isym(2) == 0) call rx('STPLAT: bad isym(3)')

      volst=det33(platst)
      if (volst < 0.d0) call dscal(3,-1.d0,platst(1,3),1)
      if (dabs(volst) < d1mach(3))call rx('STPLAT: bad vol')

      call dinv33(platst,1,qlatst,volst)
      if (iprint() >= 110) write(stdo,300)(isym(i),i=1,3)
      if (iprint() >= 50)  write(stdo,301)
...
C Local variables:
      ...
      double precision costbn,detop,det33,ddot,omcos,
     .                 sintbn,sinpb3,tiny,twopi,vfac
      character*144 messg
      parameter(twopi=6.28318530717958648d0)
      parameter(tiny=1.0d-5)
C External calls:
      external daxpy,dcopy,det33,dpzero,ddot,dscal,errmsg,idamax,
     .         nrmliz
C Intrinsic functions:
...
C ----- convert (nrot,vecg,li) into symopm
        ...
        elseif (in == 2.or.in == 3.or.in == 4.or.in == 6) then
          sintbn = dsin(twopi/nrot)
          costbn = dcos(twopi/nrot)
          omcos  = 1.d0-costbn
          if (ddot(3,vecg,1,vecg,1) < tiny)
...
C ------- get the right phases between the components.
          if (nrot == 2) then
            ...
            enddo
          elseif (nrot == 3.or.nrot == 4.or.nrot == 6) then
...
C      double precision function det33(matrix)
CC- Calculates the determinant of a 3X3 matrix
CC ----------------------------------------------------------------------
CCi Inputs:
CCi   matrix:input matrix
CCo Outputs:
CCo   det33:determinant
CC ----------------------------------------------------------------------
C      implicit none
CC Passed variables:
C      double precision matrix(9)
CC Local variables:
C      double precision ddot,m1cm2(3)
CC External calls:
C      external cross,ddot
C
C      call cross(matrix(4),matrix(7),m1cm2)
C      det33 = ddot(3,matrix(1),1,m1cm2,1)
C
C      end
