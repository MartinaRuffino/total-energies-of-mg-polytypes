...
Ci         :1's digit
Ci         :1 Around Mean-Field limit; see Remarks
Ci         :2 Fully Localized limit; see Remarks
Ci         :4 majority and minority shifted by U
Ci         :5 majority spin shifted by U, minority by J
...
C     .           Ueff*dmat4*dmat4 + (Ueff - U2)*dmat4*dmat4
              elseif (lvrs == 5) then
C               if (m == m1) Vnew(m,m1)=UH*(isp-1.5d0)*2
                if (m == m1 .and. isp == 1) Vnew(m,m1)=UH
                if (m == m1 .and. isp == 2) Vnew(m,m1)=JH
                Eldau = 0
              else
...
C             Second and third lines in Eq. 5, PRB 52, R5467
     ...
     .                  dmatu(m2,m3,isp,iblu)
              ...
              Eldau = Eldau +
     .          Vee(m,m2,m1,m3)*
     .          dmatu(m,m1,isp,iblu)*dmatu(m2,m3,iot(isp),iblu) +
     .          (Vee(m,m2,m1,m3) - Vee(m,m2,m3,m1))*
     .          dmatu(m,m1,isp,iblu)*dmatu(m2,m3,isp,iblu)
              endif
   10         continue
              enddo
            enddo
