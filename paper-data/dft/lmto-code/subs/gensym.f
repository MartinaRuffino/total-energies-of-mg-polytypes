      subroutine gensym(slabl,gens,usegen,lcar,lfix,pltol,fptol,lsmall,nbas,
     .  nspec,ngmx,plat,platcv,ldist,dist,bas,ips,nrspec,ng,g,ag,
...
Ci          1 Also, add extra basis atoms as needed to guarantee
...
Cio  nspec: On input, number of species
Cio         On output nspec may be reduced, (some species may not be used)
...
Cb   enlarged.
Cu Updates
Cu   03 Apr 16 New ability to render lattice vectors and symops consistent
...
C ... Passed parameters
     ...
     .        ng,nrspec(nspec),usegen,ldist,ips(*),nggen
C     integer istab2(nbas,*)
      double precision plat(9),platcv(9),g(9,*),ag(3,*),dist(3,3),
     .  bas(3,nbas),fptol,pltol
      character*8  slabl(*), gens*(*), nwgens*(*)
      logical lcar,lfix
C ... Dynamically allocated local arrays
      real(8), allocatable :: wk(:)
C ... Local parameters
      integer i,j,ibas,ic,iprint,ngnmx,igen,mxint,modes,nspec0
      integer mpipid,procid,master
      double precision qlat(3,3),vol,platt(3,3),xx
      logical lsmall,latvec
      parameter(ngnmx=10)
      double precision gen(9,ngnmx),agen(3,ngnmx)
C ... External calls
      external addbas,awrit2,dinv33,fexit,fixpos,groupg,grpprd,iinit,
     .         info0,lattdf,poppr,pshpr,psymop,rxx,sgroup,shoist,symcry,
     .         symlat,symtbl,words
...
C      if (pltol > 0) then
C        call fixplat(1,plat,pltol,0,g)
C      endif
...
C ... Make nrspec ... nspec0 will be nspec if all species are used
      nspec0 = mxint(nbas,ips)
      if (nspec0 /= nspec .and. iprint() > 0 .and. procid == master)
...
C   ... Symmetry of lattice without basis
        ...
        if (pltol > 0) call fixplat(10,plat,fptol,ng,g)
C   ... Symmetry of lattice with basis
        allocate(wk(3*nbas))
C       call prmx('pos',bas,3,3,nbas)
        call symcry(fptol,bas,wk,ips,nbas,nspec,nrspec,ng,plat,qlat,g,ag,istab)
C       Adjust site positions and remake space group (translation part may change)
        if (fptol > 0) then
...
C  ...  Adjust lattice, symops, and basis to conform with symops to numerical precision
        if (pltol > 0) call fixplat(10,plat,fptol,ng,g)
        if (fptol > 0) call fixpos(bas,nbas,fptol,ng,plat,g,ag,istab)
        nggen = 0
        call symtbl(0,fptol,nbas,xx,bas,g,ag,ng,qlat,istab,xx)
      endif
...
C      enddo
     ...
     .    ' GENSYM: site permutation table for group operations ...')
        call shoist(0,istab,nbas,ag,g,ng,0)
C       call shoist(0,istab2,nbas,ag,g,ng,0)
      endif

C ... return the possibly reduced number of species in nspec
      nspec = nspec0

C  ... debugging
C      call shoist(0,istab,nbas,ag,g,ng,0)
C      call symtbl(0,nbas,xx,bas,g,ag,ng,qlat,istab,w)
C      call shoist(0,istab,nbas,ag,g,ng,0)
C
C      call symtab(nbas,xx,bas,g,ag,-ng,plat,qlat,xx,istab)
C      call shoist(0,istab,nbas,ag,g,ng,0)
C      call symtbl(1,nbas,xx,bas,g,ag,ng,qlat,istab,w)
C      call shoist(0,istab,nbas,ag,g,ng,0)
C      stop
