...
C --- Find bottom and top of each band ---
      ...
      do  isp = 1, nsp
      do  iba = 1, nband
        if (eband(iba,isp,ikp) /= enull) then
...
C     Set all -enull to enull to float to top when sorted
      do  isp = 1, nsp
      do  iba = 1, nband
        if (ebtop(iba,isp) == -enull) ebtop(iba,isp) = enull
      enddo
      enddo
C     Sort bands irrespective of spin
      ...
      nfound = nband*nsp
   10 continue
      if (ebtop(nfound,1) == enull .or. ebbot(nfound,1) == enull) then
        nfound = nfound-1
        if (nfound == 0) call rx('efrng2: no bands')
        goto 10
      endif

C --- Find limits ---
      nbtop = (nval+2-nsp)/(3-nsp)
      if (zval > nval) nbtop = nbtop+1
      nbbot = nval/(3-nsp) + 1
      if (nbtop > nfound) nbtop = nfound
      if (nbbot > nfound) nbbot = nfound
      elo = ebbot(1,1)
      ehi = ebtop(nfound,1)
      if (elo == enull) call rx('efrng2: no bands')
      e1  = ebbot(nbbot,1)
      e2  = ebtop(nbtop,1)
      efrng2 = .false.
C     if (e1 > e2) then
      if (e1-e2 > d1mach(3)) then
        xx = e1
        e1 = e2
        e2 = xx
        efrng2 = .true.
