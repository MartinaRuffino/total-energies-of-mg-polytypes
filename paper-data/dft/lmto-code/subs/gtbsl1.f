...
Co Outputs
Co   ntab  :table of largest l for each orbital block
...
C ... Local parameters
      ...
      deq(x1,x2) = dabs(x1-x2) < 1d-8

      call sanrg(.true.,mode,1,31,'gtbsl1:','mode')
      lreqr = bittst(mode,1)
      lreqe = bittst(mode,2)
      lreql = bittst(mode,4)
...
C         Initial block size = size of this l
          ...
          do  jorb = iorb+1, norb
            ...
            if (lreql) then
              match = match .and. lj == lk+1
              match = match .and. kj == kk
            endif
            if ( .not. (match)) then
              exit
            endif
