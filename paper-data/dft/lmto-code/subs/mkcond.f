      subroutine mkcond(mode,plat,eval,qp,iq1,iq2,iq3,iq4,cvec,wt)
...
C ... Passed parameters
      ...
      double precision eval(4),qp(3,*),cvec(3,2),wt,plat(3,3)
C ... Local parameters
      integer i
      double precision a(3,3),b(3),grad(3),ainv(3,3),det,qlat(3,3)
...
C ... External calls
      ...
      elseif (mode == 1 .or. mode == 2 .or. mode == 3) then
        ...
        call mkqlat(plat,qlat,det) ! ... qlat = (plat^-1)^T so that qlat^T . plat = 1

C       OLD, no shortening
C        do  i = 1, 3
C          a(1,i) = qp(i,iq2) - qp(i,iq1)
C          a(2,i) = qp(i,iq3) - qp(i,iq1)
C          a(3,i) = qp(i,iq4) - qp(i,iq1)
C          b(i) = eval(i+1) - eval(1)
C        enddo

        ainv(:,1) = qp(:,iq2) - qp(:,iq1)
        ainv(:,2) = qp(:,iq3) - qp(:,iq1)
        ainv(:,3) = qp(:,iq4) - qp(:,iq1)
        do i = 1, 3
          call shorbz(ainv(1,i),ainv(1,i),qlat,plat)
          a(i,:) = ainv(:,i)
          b(i) = eval(i+1) - eval(1)
        enddo
...
C      if (sqrt(ddot(2,qbar,1,qbar,1)) > .25d0) wt=0
