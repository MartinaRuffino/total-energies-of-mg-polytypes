      subroutine gengrp(csym,dorig,gens,instr,iopt,iorig,isym,ngen,plat,qlat)
...
C ... External calls
      external chcase,gtcsym,isanrg,query,rx,rxs,words
C Data statements:
...
C       Parse for integer group id.  Failing that, assign to string
        ...
        if ((isym7 >= 1.and.isym7 <= 230) .or. csym7 /= ' ') then
...
C         Group found if integer ID or string ID matches
          if (isym7 == isym(7) .or. csym7 == csym(7)) then
...
C               Workaround gnu compiler bug
                symgrp = c(isym8+1)(15:)
                if (corig /= '2') call rxs(' SPCGRP: bad corig,',corig)
              else
                call sanrg(.true.,iorig,1,2,'SPCGRP:','iorig')
              endif
            elseif (corig /= ' ') then
...
C       ... Get isym(6-1) and csym(6-1)
            ...
            isym(4) = isym4(isym(5))
            do  isym3 = 1, 7
              if (csym(7)(1:1) == csym3(isym3)) isym(3)=isym3
            enddo
            isym(1) = isym1(isym(7))
...
C       ... Printout
            ...
            if (isym(5) == 5) then
              if (isym(1) == 5 .and. isym(3) == 1) write(*,303)
              if (isym(1) == 5 .and. isym(3) == 7) write(*,304)
              if (isym(1) == 6) write(*,305)
            endif
...
C --- Determine space group number ---
      elseif (iopt == 1) then
        do  isym8 = 1, nelts
          read(c(isym8),200) isym(7),corig,csym(7),symgrp
          if(isym(6) == isym6(isym(7)) .and.
     .       isym(1) == isym1(isym(7)) .and.
     .       csym(3) == csym(7)(1:1)) then
            iorig = 1
            if (corig == '2') iorig = 2
            if (iprint() >= 110) write(stdo,306) symgrp
...
C Data statements:
     ...
     .           '23','m-3','432','-432','m-3m'/
      ...
      do  i = 1, 6
        if (isym(i) > n(i) .or. isym(i) < 0) isym(i)=0
      enddo
