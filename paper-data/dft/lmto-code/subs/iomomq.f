...
C ----------------------------------------------------------------------
Ci Inputs:
Ci   ifi   :moments file logical unit, but >0 for read, <0 for write
...
Cio  nfstg :output, 1s digit mode = 0
Cio        :input, otherwise
Cio        :nsftg describes information contained in moments file; see below.
...
Cio        :Contents (from iomoms)
Cio        :1s digit =>  eb I/O
Cio        :10s digit => doswt I/O; digit's number specifies number of
Cio                      'spin' channels to I/O.
...
C --- Determine number of qp available ---
      ...
      do  jq = 1, nkp*(nsp/nspc)
        if (iomoms(ifi,nl,nsp,nspc,nkp,ldim,0,1,jq,1,nband*nspc,nband*nspc,
     .    nchan,nchan2,nev,eb,accwt,doswt,dosw2,efermi,vmtz) < 0) goto 14
        iq = jq-1
      enddo
C ... If read all qp, and another record also present, let iq=nkp
      read (ifi,err=14,end=14) xx ! This is normally the Fermi energy
      iq = iq+1
   14 continue
      iq = (nspc*iq)/nsp
...
C --- Read info spec'd by nfstg for each qp until error ---
      if (mod(mode/10,10) /= 1) then
        ...
        nevmx = 0
        do  jq = 1, iq*(nsp/nspc)
          if (iomoms(ifi,nl,nsp,nspc,nkp,ldim,nfstg,nschan,jq,1,nband*nspc,nband*nspc,
     .      nchan,nchan2,nev,eb,accwt,doswt,dosw2,efermi,vmtz) < 0) goto 999
          nevmx = max(nev,nevmx)
        enddo
...
C --- Error exit ---
  999 continue
      call info0(10,0,0,' IOMOMQ (warning): empty or bad moments file')
      iq = -1
      end
